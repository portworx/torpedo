...
func (r *Rancher) UpdateClusterWidePSA(clusterName string, psaName string) error {
	...
	for _, cluster := range clusterCollection.Data {
		log.Infof("The cluster is 1 %v", cluster.Name)
		log.Infof("The cluster is 2 %v", cluster.ServiceAccountTokenSecret)
		log.Infof("The cluster is 3 %v", cluster.AppliedSpec)
		log.Infof("The cluster is 4 %v", cluster.ID)
		log.Infof("The cluster is 5 %v", cluster.UUID)
		log.Infof("The cluster is 6 %v", cluster.DefaultPodSecurityAdmissionConfigurationTemplateName)
		log.Infof("The cluster object is %v", cluster)
		if cluster.Name == clusterName {
			log.Infof("The selected cluster")
