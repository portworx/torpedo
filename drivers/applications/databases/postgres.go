...
func (app *PostgresConfig) CopyBackupData(ctx context.Context, identifier string) error {

	log.InfoD("Running copy Query")
	log.Debugf("identifier [%s]", identifier)
	log.Debugf("copy command [%s]", app.SQLCommands[identifier]["copy"])
	_, err := app.ExecuteCommand([]string{app.SQLCommands[identifier]["copy"][0]}, ctx)
...
func (app *PostgresConfig) StartData(command <-chan string, ctx context.Context) error {
	...
	var tableName = "postgres_table_" + RandomString(4)
...
func (app *PostgresConfig) GetReplicaDetails(context.Context) (ReplicaDetails, error) {
	return ReplicaDetails{}, errdefs.NotImplemented(fmt.Errorf("GetReplicaDetails is not implemented for Postgres"))
}

// AddDatabaseToCluster Adds database to AAG Cluster for SQL Server
func (app *PostgresConfig) AddDatabaseToCluster(ctx context.Context, databaseName string, clusterName string) error {
	return errdefs.NotImplemented(fmt.Errorf("AddDatabaseToCluster is not implemented for Postgres"))
}

// CreateNewDatabase creates a new database on the given data service
func (app *PostgresConfig) CreateNewDatabase(ctx context.Context, databaseName string, databaseDetails DefaultDatabase) error {
	return errdefs.NotImplemented(fmt.Errorf("CreateNewDatabase is not implemented for Postgres"))
}

// GetDefaultNewDatabaseConfig will get default database config details
func (app *PostgresConfig) GetDefaultNewDatabaseConfig(databaseName string) DefaultDatabase {
	log.Warnf(fmt.Sprintf("GetDefaultNewDatabaseConfig is not implemented for Postgres"))
	return DefaultDatabase{}
}

// ExecuteCommandOnPrimary executes given command on the primary node
func (app *PostgresConfig) ExecuteCommandOnPrimary(commands []string, ctx context.Context) ([]string, error) {
	return nil, errdefs.NotImplemented(fmt.Errorf("ExecuteCommandOnPrimary is not implemented for MySql"))
}

// Failover is not implemented for Postgres
func (app *PostgresConfig) Failover(ctx context.Context) error {
	return errdefs.NotImplemented(fmt.Errorf("Failover is not implemented for Postgres"))
}

// GetDataserviceConfigurations returns the configuration of the dataservice
func (app *PostgresConfig) GetDataserviceConfigurations(ctx context.Context) (DatabaseConfiguration, error) {
	return DatabaseConfiguration{}, errdefs.NotImplemented(fmt.Errorf("GetDataserviceConfigurations is not implemented for Postgres"))
}

// GetDatabaseDetails returns the database details
func (app *PostgresConfig) GetDatabaseDetails() Database {
	return app.Database
}
