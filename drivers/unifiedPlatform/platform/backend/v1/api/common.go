...
type PLATFORM_API_V1 struct {
	...
	BackupPolicyV1Client             *backupPolicyV1.APIClient
	AccountID                        string
}
...
// GetClient updates the header with bearer token and returns the new client
func (backupPolicy *PLATFORM_API_V1) getbackupPolicyClient() (context.Context, *backupPolicyV1.BackupPolicyServiceAPIService, error) {
	log.Infof("Creating client from PLATFORM_API_V1 package")
	ctx, token, err := GetBearerToken()
	if err != nil {
		return nil, nil, fmt.Errorf("Error in getting bearer token: %v\n", err)
	...
	backupPolicy.BackupPolicyV1Client.GetConfig().DefaultHeader = GetDefaultHeader(token, backupPolicy.AccountID)

	client := backupPolicy.BackupPolicyV1Client.BackupPolicyServiceAPI
	return ctx, client, nil
}
