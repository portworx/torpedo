...
func (cloudCred *PLATFORM_API_V1) ListCloudCredentials(request *CloudCredentialsRequest) (*CloudCredentialsResponse, error) {
	...
	req := cloudCredsClient.CloudCredentialServiceListCloudCredentials(ctx)
	...
	cloudCredModel, res, err := req.Execute()
	...
	err = utilities.CopyStruct(cloudCredModel, &cloudCredsResponse.List)
	if err != nil {
		return nil, err
	}
