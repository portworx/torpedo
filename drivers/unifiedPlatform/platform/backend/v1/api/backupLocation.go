...
func (backuploc *PLATFORM_API_V1) ListBackupLocations(request *BackupLocationRequest) (*BackupLocationResponse, error) {
	...
	req := backupLocationClient.BackupLocationServiceListBackupLocations(ctx)
	...
	backupLocationModels, res, err := req.Execute()
	...
	for _, bkpLocation := range backupLocationModels.BackupLocations {
		...
		err = utilities.CopyStruct(backupLocationModels, &backupLocResp.List)
		if err != nil {
			return nil, err
		}
...
func (backuploc *PLATFORM_API_V1) DeleteBackupLocation(backupLocation *BackupLocationRequest) error {
	...
	_, res, err := backupLocationClient.BackupLocationServiceDeleteBackupLocation(ctx, backupLocation.Delete.Id).Execute()
	if err != nil {
