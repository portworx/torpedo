...
func (tc *PLATFORM_API_V1) DeleteTargetCluster(tcRequest *PlatformTargetClusterRequest, force bool) error {
	ctx, dtClient, err := tc.getTargetClusterClient()
	if err != nil {
		return fmt.Errorf("Error in getting context for api call: %v\n", err)
	}
	_, res, _ := dtClient.TargetClusterServiceDeleteTargetCluster(ctx, tcRequest.DeleteTargetCluster.Id).Force(force).Execute()
	if err != nil && res.StatusCode != status.StatusOK {
