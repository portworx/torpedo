...
func ResizeDataServiceStorage(deployment *automationModels.V1Deployment, ds dslibs.PDSDataService, namespaceId, newResConfigId string) (bool, error) {
	...
	if UpdateTemplate != "" {
		...
		log.Warnf("Update Template passed is empty")
		if ResiliencyFlag {
			ResiliencyCondition <- false
			CapturedErrors <- fmt.Errorf("%s\n", "Template is not passed properly to update the resources")
		}
	...
	newResourceTemp, err := dslibs.GetResourceTemplateConfigs(newResConfigId)
	if err != nil {
		if ResiliencyFlag {
			CapturedErrors <- err
		}
	...
	if len(UpdateTemplate) > 0 {
		if updatedCapacity > initialCapacity {
			...
			if ResiliencyFlag {
				CapturedErrors <- fmt.Errorf("Failed to verify Storage Resize at PV/PVC level: %s\n", "updatedCapacity should be higher than the initial capacity")
			}
			return false, err
		}
	}
