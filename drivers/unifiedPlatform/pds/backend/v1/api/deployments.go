...
func (ds *PDS_API_V1) CreateDeployment(createDeploymentRequest *automationModels.PDSDeploymentRequest) (*automationModels.PDSDeploymentResponse, error) {
	...
	//Debug Print
	log.Infof("Deployment Request [%+v]", *DeploymentRequest.Config)
	...
	fmt.Println("Is TLS Enabled: ", *DeploymentRequest.Config.TlsConfig.Enabled)
	fmt.Println("Cluster Issuer Name: ", *DeploymentRequest.Config.TlsConfig.IssuerName)
	...
	// Marshal depCreateRequest to JSON
	payload, err := json.MarshalIndent(DeploymentRequestBody, "", "  ")
	if err != nil {
		log.Fatalf("Error marshaling depCreateRequest: %v", err)
	}
