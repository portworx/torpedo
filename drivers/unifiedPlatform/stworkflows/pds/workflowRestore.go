...
func (restore WorkflowPDSRestore) CreateRestore(name string, backupUid string, namespace string, sourceDeploymentId string) (*automationModels.PDSRestoreResponse, error) {
	...
	log.Infof("Restore namespace - [%s]", namespace)
	...
	if restore.Source.DataServiceDeployment[sourceDeploymentId].DatabaseDriver != nil {
		restore.RestoredDeployments.DataServiceDeployment[createRestore.Create.Config.DestinationReferences.DataServiceDeploymentId].DatabaseName =
			restore.Source.DataServiceDeployment[sourceDeploymentId].DatabaseDriver.DefaultDBName()
	}
	...
	if value, ok := restore.SkipValidation[CheckDataAfterRestore]; ok {
		...
		if restore.Source.DataServiceDeployment[sourceDeploymentId].DatabaseDriver != nil {
			...
			databaseDriver, err := restore.RestoredDeployments.CreateDatabaseDriver(*deployment.Get.Meta.Uid, namespace, restore.Source.DataServiceDeployment[sourceDeploymentId].DSParams.Name)
			if err != nil {
				return nil, fmt.Errorf("unable to create Database driver - %s", err.Error())
			}

			// Setting the database driver for the deployment
			restore.RestoredDeployments.DataServiceDeployment[*deployment.Get.Meta.Uid].DatabaseDriver = databaseDriver

			if restore.Source.DataServiceDeployment[sourceDeploymentId].DSParams.RunContinuousData {
				// Starting data injection for the data service and adding to the error groups
				restore.RestoredDeployments.DataServiceDeployment[*deployment.Get.Meta.Uid].DatabaseErrorGroup.Go(func() error {
					err := databaseDriver.StartData(*restore.RestoredDeployments.DataServiceDeployment[*deployment.Get.Meta.Uid].DatabaseControlChannel, ctx)
					return err
				})

				// Setting IsContinoutDataSupportEnabled to true
				restore.RestoredDeployments.DataServiceDeployment[*deployment.Get.Meta.Uid].IsContinousDataSupportEnabled = true
			}

			if strings.ToLower(restore.RestoredDeployments.DataServiceDeployment[*deployment.Get.Meta.Uid].DSParams.Name) == neo4j {
				results, err := databaseDriver.ExecuteCommand([]string{"MATCH ()-[r:CONDUCTED_AT]->() RETURN COUNT(r) AS RelationshipCount;"}, ctx)
				if err != nil {
					return nil, fmt.Errorf("error while executing command on db - [%s]", err.Error())
				}
				restore.RestoredDeployments.Dash.VerifyFatal(results[0], "14446", "validating the relationship count after restore")
			}
		}
...
func (restore WorkflowPDSRestore) ValidateDataAfterRestore(destinationDeploymentId string, backupId, namespace string) error {
	...
	if slices.Contains(stworkflows.SKIPDATASERVICEFROMWORKLOAD, restore.RestoredDeployments.DataServiceDeployment[destinationDeploymentId].DSParams.Name) {
		log.Warnf("Skipping data validation for the data service [%s]", restore.RestoredDeployments.DataServiceDeployment[destinationDeploymentId].DSParams.Name)
		return nil
	}
	...
	dsInstances := *restore.RestoredDeployments.DataServiceDeployment[destinationDeploymentId].Deployment.Config.DataServiceDeploymentTopologies[0].Instances
	log.Debugf("dsInstances after restore [%s]", dsInstances)
	if dsInstances != "1" {
		log.Infof("Enabling cluster mode to read data after restore")
		restore.RestoredDeployments.WorkloadGenParams.ClusterMode = "true"
	} else {
		restore.RestoredDeployments.WorkloadGenParams.ClusterMode = "false"
	}
