...
type WorkflowPDSTemplates struct {
	...
	CustomTemplatesToBeDeleted []string
}
...
func (cusTemp *WorkflowPDSTemplates) CreateResourceTemplateWithCustomValue(templates *parameters.NewPDSParams) (string, error) {
	...
	cusTemp.CustomTemplatesToBeDeleted = append(cusTemp.CustomTemplatesToBeDeleted, *resNewConfigId)
...
func (cusTemp *WorkflowPDSTemplates) Purge(ignoreError bool) error {
	...
	if len(cusTemp.CustomTemplatesToBeDeleted) > 0 {
		log.InfoD("No of Custom Created Templates to be Deleted [%d]", len(cusTemp.CustomTemplatesToBeDeleted))
		err := cusTemp.DeleteCreatedCustomPdsTemplates(cusTemp.CustomTemplatesToBeDeleted)
		if err != nil {
