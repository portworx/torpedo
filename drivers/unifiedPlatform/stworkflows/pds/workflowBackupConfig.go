...
func (backupConfig WorkflowPDSBackupConfig) CreateScheduleBackup(name string, deploymentId string, schedule string) (*automationModels.PDSBackupConfigResponse, error) {
	var (
	...
	createBackup, err := pdslibs.CreateScheduleBackup(name,
		deploymentId,
...
func (backupConfig WorkflowPDSBackupConfig) CreateBackupConfig(name string, deploymentId string) (*automationModels.PDSBackupConfigResponse, error) {
	...
	//validate the health of the deployment after backup has been taken
	err = pdslibs.ValidateDataServiceDeploymentHealth(deploymentId, PDS_DEPLOYMENT_AVAILABLE)
	if err != nil {
		return nil, err
	}
