...
func (backup WorkflowPDSBackup) WaitForBackupToComplete(backupId string) error {
	...
	waitforBackupToComplete := func() (interface{}, bool, error) {
		...
		if *backupModel.Get.Status.Phase == stworkflows.FAILED {
			...
			backup.Backups[backupId] = &BackupDetails{
...
func (backup WorkflowPDSBackup) Purge() error {
	...
	for eachBackup, backupDetails := range backup.Backups {
		log.InfoD("Deleting [%s] ID [%s]", *backupDetails.Backup.Meta.Name, eachBackup)
...
// CheckFirstBackupTime checks the time taken for the first backup to trigger
func (backup WorkflowPDSBackup) CheckFirstBackupTime(deploymentID string, timeout time.Duration) (time.Duration, error) {
	...
	waitForFirstBackup := func() (interface{}, bool, error) {
		backupDetails, err := backup.GetLatestBackup(deploymentID)
		if err != nil && strings.Contains(err.Error(), "No backups found for backup config") {
			timeTaken = timeTaken + retryTime
			return nil, true, fmt.Errorf("No backups found yet")
		} else if err != nil {
			log.InfoD("Error occured while fetching backup details - [%s]", err.Error())
			return nil, false, err
		} else {
			log.Infof("Backup found - [%s] - [%s]", *backupDetails.Meta.Name, *backupDetails.Meta.Uid)
			return nil, false, nil
		}
	}

	_, err := task.DoRetryWithTimeout(waitForFirstBackup, timeout, retryTime)
