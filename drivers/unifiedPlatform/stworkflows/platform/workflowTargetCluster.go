...
// ValidatePlatformComponents used to validate all k8s object in px-system namespace
...
func (targetCluster *WorkflowTargetCluster) EnableTLSOnClusterAndApplicationLevel(ClusterIssuerName string) error {
	err := platformLibs.InstallCertManager(platformNamespace)
	if err != nil {
		return err
	}

	err = platformLibs.CreateSelfSignedClusterIssuer(ClusterIssuerName)
	if err != nil {
		return err
	}

	apps, err := platformLibs.ListAvailableApplicationsForCluster(targetCluster.ClusterUID)
	if err != nil {
		return err
	}
	for _, app := range apps.PDSApplication.List {
		log.Debugf("App Name: [%s]", *app.Meta.Name)
		log.Debugf("App Id: [%s]", *app.Meta.Uid)
		if *app.Meta.Name == "pds" {
			_, err := platformLibs.UpdateApplication(*app.Meta.Uid, true)
			if err != nil {
				return err
			}
		}
