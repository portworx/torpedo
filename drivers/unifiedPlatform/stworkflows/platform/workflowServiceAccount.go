...
// GrantIAMRole grants IAM role to a service account
func (svcUser *WorkflowServiceAccount) GrantIAMRole(saName string, roles []string, projectId string) error {

	actorID := *svcUser.UserRoles[saName].UserDetails.Meta.Uid
	...
	err := platformLibs.GrantIamAccess(actorID, allRoleBindings, projectId, svcUser.WorkflowProjects[0].Platform.TenantId)
	return err
}

// RevokeIAMRole revokes IAM role to a service account
func (svcUser *WorkflowServiceAccount) RevokeIAMRole(saName string, roles []string, projectId string) error {
	...
	err := platformLibs.RevokeIamAccess(actorID, allRoleBindings, projectId, svcUser.WorkflowProjects[0].Platform.TenantId)
	return err
}
