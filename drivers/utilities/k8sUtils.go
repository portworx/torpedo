...
const (
	removeFinalizerTimeout       = 5 * time.Minute
	removeFinalizerRetryInterval = 10 * time.Second
)
...
// GetK8sContext gets the k8s clientset and config
...
// CopyFileFromPod Copies file from pod
func CopyFileFromPod(namespace, pod, srcPath, destPath string) error {
	// Construct the kubectl cp command
	cmd := exec.Command("kubectl", "cp", fmt.Sprintf("%s/%s:%s", namespace, pod, srcPath), destPath)
	...
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("Error copying file from pod: %v\n", err)
	}

	return nil
}

// CopyFileToPod Copies file to the given pod
func CopyFileToPod(namespace, pod, srcPath, destPath string) error {
	// Construct the kubectl cp command
	cmd := exec.Command("kubectl", "cp", srcPath, fmt.Sprintf("%s/%s:%s", namespace, pod, destPath))
...
func RemoveFinalizersFromAllResources(namespace string) error {
	...
	log.Infof("Total number of endpoints found - [%d]", len(allEndpoints.Items))
	...
	for _, endpoint := range allEndpoints.Items {
		...
		log.Infof("Setting finalizer for endpoint [%s]", endpoint.Name)
	...
	removeFinalizerFromServices := func() (interface{}, bool, error) {

		allServices, err := k8sCore.ListServices(namespace, metav1.ListOptions{})
		if err != nil {
			return nil, false, fmt.Errorf("unable to list services. Error - [%s]", err.Error())
		}
		log.Infof("Total number of services found - [%d]", len(allServices.Items))

		log.Infof("Removing finalizers from services")
		for _, service := range allServices.Items {

			log.Infof("Setting finalizer for service [%s]", service.Name)
			service.SetFinalizers([]string{})

			_, err := k8sCore.UpdateService(&service)
			if err != nil {
				return nil, true, fmt.Errorf("error occurred while removing finalizer from service - [%s]", err.Error())
			}
		}
	...
	_, err = task.DoRetryWithTimeout(removeFinalizerFromServices, removeFinalizerTimeout, removeFinalizerRetryInterval)
	if err != nil {
...
// CreateNodePortServiceForDeployment creates a node port service for given deployment STS
func CreateNodePortServiceForDeployment(serviceName string, namespaceName string, selector map[string]string, port string) (*corev1.Service, error) {

	// Converting port to integert
	...
	portToInt := int32(tempPort)

	// Creating service specs
	serviceSpec := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      serviceName + "-" + RandomString(3), // Adding this fix as SQL already has a service with STS name
		...
		Spec: corev1.ServiceSpec{
			Ports: []corev1.ServicePort{
				{
					Port:       portToInt,
					TargetPort: intstr.FromInt(tempPort),
				},
			},
			Type:     "NodePort",
			Selector: selector,
	...
	svcDetails, err := k8sCore.CreateService(serviceSpec)
	if err != nil {
		return nil, err
	}
