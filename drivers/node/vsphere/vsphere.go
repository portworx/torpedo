...
func (v *vsphere) Init(nodeOpts node.InitOptions) error {
	...
	v.f = find.NewFinder(v.client.Client, true)
...
func (v *vsphere) connect() error {
	...
	vms, err := v.f.VirtualMachineList(v.ctx, "*")
...
func (v *vsphere) SetDatacenter(datacenterName string) error {
	t := func() (interface{}, bool, error) {
		...
		dc, err := v.f.Datacenter(v.ctx, datacenterName)
		if err != nil {
...
func (v *vsphere) AddMachine(vmName string) error {
	...
	vm, err := v.f.VirtualMachine(v.ctx, vmName)
	if err != nil {
		return err
	}
	dc, _ := v.f.Datacenter(v.ctx, vmName)
	v.f.SetDatacenter(dc)
	v.datacenter = dc
	var vmMo mo.VirtualMachine
...
func (v *vsphere) DeleteVm(vmName string) error {
	...
	if err != nil {
		log.Warnf("failed to get VM: %s, because of err: %+v", vmName, err)
		return nil
	}
...
func (v *vsphere) getVmObjectByName(vmName string) (*object.VirtualMachine, error) {
	t := func() (interface{}, bool, error) {
		...
		// Make future calls local to this datacenter
		if err != nil {
			return nil, false, fmt.Errorf("failed to find data center: %v", err)
		}
		err = v.AddMachine(vmName)
		if err != nil {
			return nil, false, fmt.Errorf("failed to find data center: %v", err)
		}
		dc, err := v.f.DefaultDatacenter(v.ctx)
		if err != nil {
			return nil, false, fmt.Errorf("failed to find data center: %v", err)
		}
		v.f.SetDatacenter(dc)
...
func (v *vsphere) destroy(vm *object.VirtualMachine) error {
	...
	if err != nil {
		log.Errorf("failed to delete VM %s. cause %v", vm.Name(), err)
	}
	if _, err := tsk.WaitForResult(v.ctx); err != nil {
		log.Errorf("failed to wait for VM %s to be deleted. cause %v", vm.Name(), err)
	}
