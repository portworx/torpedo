...
type NamespaceInfo struct {
	...
	forgottenContexts []*scheduler.Context
}

// DeepCopy returns a deep copy of NamespaceInfo
...
func (c *NamespaceConfig) Application(appKey string) *ApplicationConfig {
	return &ApplicationConfig{
		...
		ScheduleConfig: &ScheduleConfig{
			ScheduleOptions: &scheduler.ScheduleOptions{
				...
				Nodes:              c.controller.ClusterInfo.storageLessNodes,
				Labels:             c.controller.ClusterInfo.storageLessNodeLabels,
				Namespace:          c.namespace,
			},
...
func (c *ApplicationConfig) Schedule() error {
	//err := c.validate()
	//if err != nil {
	...
	if err != nil {
		debugMessage := fmt.Sprintf("namespace: [%s]; custom-apps-spec: [%v]; instance-id: [%s]; schedule-options: [%v]", c.Namespace, customAppsSpec, c.instanceID, *c.ScheduleOptions)
		return utils.ProcessError(err, debugMessage)
	}
	for _, ctx := range contexts {
		c.controller.saveContext(c.namespace, ctx)
	}
	return nil
}
