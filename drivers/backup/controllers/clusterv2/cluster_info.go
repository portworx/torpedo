...
// ClusterMetaData holds the metadata of the cluster
...
// ClusterInfo holds the information that can be used to create the ClusterController
...
func setStorageLessNodes(c *ClusterInfo) error {
	...
	for _, storageLessNode := range c.storageLessNodes {
		for labelKey, labelValue := range c.storageLessNodeLabels {
			...
			if err != nil {
				debugMessage := fmt.Sprintf("storage-less node: [%s]; label: key [%s], value [%s]", storageLessNode.Name, labelKey, labelValue)
...
// NewController returns new ClusterController based on the ClusterInfo
func (c *ClusterInfo) NewController() (*ClusterController, error) {
	newClusterInfo := c.DeepCopy()
	...
	if !clusterController.ClusterInfo.hyperConverged {
		...
		if err != nil {
			return nil, utils.ProcessError(err, c.String())
		}
	}
	return clusterController, nil
}

// String returns the string version of the ClusterInfo
func (c *ClusterInfo) String() string {
	return fmt.Sprintf("cluster-info: id [%s], name [%s], config-path [%s], "+
		"hyperconverged [%t], in-cluster [%t]", c.id, c.name, c.configPath, c.hyperConverged, c.inCluster)
}
