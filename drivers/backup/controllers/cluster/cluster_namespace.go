...
func (c *NamespaceConfig) validate() error {
	if !c.isNamespaceRecorded {
		...
		if !c.isAppKeyRecorded {
			err := fmt.Errorf("scheduler-contexts of the namespace [%s] is not in records", c.namespace)
			return utils.ProcessError(err)
		} else {
			err := fmt.Errorf("scheduler-contexts of the namespace [%s] with the app-key [%s] is not in records", c.namespace, c.appKey)
			return utils.ProcessError(err)
		}
	}
	return nil
...
func (c *NamespaceConfig) Application(appKey string) *NamespaceConfig {
	...
	c.appKey, c.isAppKeyRecorded = appKey, true
