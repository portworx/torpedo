...
const (
	// DefaultInClusterName is the default cluster where PX-Backup and Torpedo are running
	DefaultInClusterName = "in-cluster"
	// DefaultSourceClusterName is the default cluster where applications are installed
	DefaultSourceClusterName = "source-cluster"
	// DefaultDestinationClusterName is the default cluster where applications will be restored
	DefaultDestinationClusterName = "destination-cluster"
)

// ExecutionTime represents the start-time and total-duration of an execution
type ExecutionTime struct {
	StartTime     time.Time
	TotalDuration time.Duration
}

// NewExecutionTime returns new ExecutionTime based on the specified start and end time
func NewExecutionTime(startTime time.Time, endTime time.Time) ExecutionTime {
