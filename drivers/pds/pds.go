...
func GetK8sContext() (*kubernetes.Clientset, error) {
	kubeconfigPath := os.Getenv("KUBECONFIG")

	// Build the client configuration from the kubeconfig file.
	config, err := clientcmd.BuildConfigFromFlags("", kubeconfigPath)
	if err != nil {
		return nil, fmt.Errorf("Error creating client configuration from kubeconfig: %v\n", err)
	}
	// Create the Kubernetes client using the configuration.
	clientset, err := kubernetes.NewForConfig(config)
	if err != nil {
		return nil, fmt.Errorf("Error creating clientset: %v\n", err)
	}
