...
func (targetCluster *TargetCluster) ValidatePDSComponents() error {
	deployments, err := k8sApps.ListDeployments(PDSNamespace, metav1.ListOptions{})
	if err != nil {
		return err
	}
	replicaSets, err := k8sApps.ListReplicaSets(PDSNamespace, metav1.ListOptions{})
	if err != nil {
		return err
	}
	...
	for _, deploy := range deployments.Items {
		err = k8sApps.ValidateDeployment(&deploy, DefaultTimeout, DefaultRetryInterval)
		if err != nil {
			return err
		}
	}
	...
	for _, rs := range replicaSets {
		err = k8sApps.ValidateReplicaSet(rs.Name, PDSNamespace, DefaultTimeout)
		if err != nil {
			return err
		}
	}
