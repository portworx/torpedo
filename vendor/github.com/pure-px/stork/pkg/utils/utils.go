...
// DoesSCSupportDirectAccess returns true if storage class provisions direct access volumes like
// pure_block or pure_file
func DoesSCSupportDirectAccess(sc *storagev1.StorageClass) bool {
	if sc.Parameters != nil {
		if backend, ok := sc.Parameters[pureBackendParam]; ok {
			if backend == pureBlockParam || backend == pureFileParam {
				return true
			}
		}
	}
...
// IsSharedv4Volume returns pvc is sharedv4
func IsSharedv4Volume(pvcName string, namespace string) (bool, error) {
	var pvc *v1.PersistentVolumeClaim
	var err error
	if storkcache.Instance() != nil {
		pvc, err = storkcache.Instance().GetPersistentVolumeClaim(pvcName, namespace)
		if err != nil {
			logrus.Debugf("Error getting PVC %s from informer cache: %v", pvcName, err)
		}
	} else {
		pvc, err = core.Instance().GetPersistentVolumeClaim(pvcName, namespace)
	}
	if err != nil || pvc == nil {
		return false, fmt.Errorf("error getting PV name for PVC (%v/%v): %w", namespace, pvcName, err)
	}

	if pvc.Spec.VolumeMode == nil || *pvc.Spec.VolumeMode == v1.PersistentVolumeFilesystem {
		for _, mode := range pvc.Spec.AccessModes {
			if mode == v1.ReadWriteMany || mode == v1.ReadOnlyMany {
				return true, nil
			}
		}
	}
...
// CheckPodHasSharedv4Volume returns true if specified pod has sharedv4 volumes
func CheckPodHasSharedv4Volume(podSpec *v1.PodSpec, namespace string) (bool, error) {
	for _, volume := range podSpec.Volumes {
		if volume.PersistentVolumeClaim != nil {
			isSharedv4, err := IsSharedv4Volume(volume.PersistentVolumeClaim.ClaimName, namespace)
			if err != nil {
				return false, err
			}
			if isSharedv4 {
