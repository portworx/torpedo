...
func (r ApiDataServiceVersionServiceListCompatibleDataServiceVersionsRequest) DataServiceIds(dataServiceIds []string) ApiDataServiceVersionServiceListCompatibleDataServiceVersionsRequest {
	r.dataServiceIds = &dataServiceIds
	return r
}

// List of UID of the data service version for which compatible versions are requested.
func (r ApiDataServiceVersionServiceListCompatibleDataServiceVersionsRequest) DataServiceVersionIds(dataServiceVersionIds []string) ApiDataServiceVersionServiceListCompatibleDataServiceVersionsRequest {
	r.dataServiceVersionIds = &dataServiceVersionIds
	return r
}
...
func (a *DataServiceVersionServiceAPIService) DataServiceVersionServiceListCompatibleDataServiceVersionsExecute(r ApiDataServiceVersionServiceListCompatibleDataServiceVersionsRequest) (*V1ListCompatibleDataServiceVersionsResponse, *http.Response, error) {
	...
	if r.dataServiceIds != nil {
		t := *r.dataServiceIds
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "dataServiceIds", s.Index(i).Interface(), "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "dataServiceIds", t, "multi")
		}
	}
	if r.dataServiceVersionIds != nil {
		t := *r.dataServiceVersionIds
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "dataServiceVersionIds", s.Index(i).Interface(), "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "dataServiceVersionIds", t, "multi")
		}
	}
	if r.sortSortBy != nil {
...
// Filter data service versions based on enabled flag.   - ENABLED_UNSPECIFIED: List all versions.  - TRUE: List only enabled versions.  - FALSE: List only disabled versions.
func (r ApiDataServiceVersionServiceListDataServiceVersionsRequest) Enabled(enabled string) ApiDataServiceVersionServiceListDataServiceVersionsRequest {
	r.enabled = &enabled
	return r
}
