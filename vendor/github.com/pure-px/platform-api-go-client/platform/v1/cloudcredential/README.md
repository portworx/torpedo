# Go API client for cloudcredential
...
```go
import cloudcredential "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
For using other server than the one defined on index 0 set context value `cloudcredential.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), cloudcredential.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `cloudcredential.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), cloudcredential.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `cloudcredential.ContextOperationServerIndices` and `cloudcredential.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), cloudcredential.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), cloudcredential.ContextOperationServerVariables, map[string]map[string]string{
