...
func deleteAndWait(resource interface{}) error {
	if obj, ok := resource.(*corev1.Namespace); ok {
		t := func() (interface{}, bool, error) {
			...
			return nil, true, errors.New(fmt.Sprintf("namespace %s not deleted", obj.Name))
		}
		...
		t := func() (interface{}, bool, error) {
			...
			return nil, true, errors.New(fmt.Sprintf("job %s not deleted", obj.Name))
		}
...
func getSpecsToDeleteForCentralUpgrade(lsEnabled, monitorEnabled bool) []interface{} {
	...
	if lsEnabled {
		specs = append(specs,
			...
			&batchv1.Job{ObjectMeta:
				v1.ObjectMeta{Name: "pxcentral-license-ha-setup"},
			})
	}
	...
	if monitorEnabled {
		specs = append(specs,
			...
			&batchv1.Job{ObjectMeta:
				v1.ObjectMeta{Name: "pxcentral-monitor-post-install-setup"},
			},
			&appsapi.StatefulSet{
...
var _ = Describe("{InstallCentral}", func() {
	It("has to setup, validate and teardown apps", func() {
		...
		Step("Install px-central, px-license-server and px-monitor", func() {
			context = installPxcentral(&centralOptions, &lsOptions, &monitorOptions, false, false)
		})
...
var _ = Describe("{UpgradeCentralSingle}", func() {
	It("has to setup, upgrade, validate and teardown apps", func() {
		...
		Step("Install px-central using 1.2.x helm repo then validate", func() {
			context = installPxcentral(&centralOptions, &lsOptions, &monitorOptions, true, true)
		})
		...
		Step("Prepare for helm upgrade", func() {
			...
			extraValues = fmt.Sprintf("%s,%s,%s", extraValues, enableFlags, backupDBFlags)
