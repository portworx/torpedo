	...
	Step(stepLog, func() {
		...
		if isCloudDrive {
			...
			Step(stepLog, func() {
				...
				UpdateOutcome(event, fmt.Errorf("error refreshing driver end points,Err: %v", err))
				err = Inst().V.RefreshDriverEndpoints()
				UpdateOutcome(event, fmt.Errorf("error refreshing driver end points,Err: %v", err))
			...
			Step(stepLog, func() {
				...
				for _, nodeVol := range nodeVols {
					...
					err = Inst().V.SetReplicationFactor(tpVol, int64(len(replNodes)+1), nil, []string{poolToReplAdd.Uuid}, true)
