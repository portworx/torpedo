...
var _ = Describe("{MetroDRPerformFailoverFailbackWithNetworkDrop}", Label("staging", "p1", "negative", "MetroDR"), func() {
	/*
	   https://purestorage.atlassian.net/browse/HAZEL-1662
	   1. Deploy application on source cluster
	   2. Create cluster pair between source and destination cluster
	   3. Create migration schedule and start migration on all namespace
	   4. induce network packet drops
	   5. Perform and validate failback and failover
	*/
	var (
		testrailID          = 0
		runID               int
		contexts            []*scheduler.Context
		taskNamePrefix      = "dr-ipdrop"
		defaultNs           = "kube-system"
		migrationNamespaces []string
		migNamespaces       string
	...
	JustBeforeEach(func() {
		StartTorpedoTest("MetroDRPerformFailoverFailbackWithNetworkDrop", "perform strokctl metroDR failover and failback with network packet drops", nil, testrailID)
		runID = testrailuttils.AddRunsToMilestone(testrailID)
	})

	stepLog := "perform strokctl metroDR failover and failback with network packet drops"
	It(stepLog, func() {
		log.InfoD(stepLog)
		...
		stepLog = "Creating clusterpair between first and second cluster and create and validate migration"
		Step(stepLog, func() {
			log.InfoD(stepLog)
			...
			createMigSchdAndValidateMigration(migrationSchedName, cpName, defaultNs, kubeConfigPathSrc, extraArgs)
			log.InfoD("Migration schedule [%v] created and validated successfuly on namespace [%v]", migrationSchedName, defaultNs)
		})

		extraArgsFailoverFailback := map[string]string{
			"kubeconfig": kubeConfigPathDest,
		}
		stepLog = "Drop IP tables on all worker nodes and Perform failover on the destination cluster"
		Step(stepLog, func() {
			log.InfoD(stepLog)

			stepLog = "Drop IP Tables on all worker nodes and wait for 3 mins and bring it back"
			Step(stepLog, func() {
				log.InfoD(stepLog)
				storageNodes := node.GetStorageNodes()
				for _, eachNode := range storageNodes {
					log.Infof("Blocking IPAddress on Node [%v] before restart", eachNode.Name)
					err = BlockIPTabelOnNode(&eachNode, true)
					log.FailOnError(err, "failed to block IP tables on Node [%v]", eachNode.Name)
				}

				log.Infof("Waiting for 3 minute after drop IP tables on all worker nodes")
				time.Sleep(3 * time.Minute)

				for _, eachNode := range storageNodes {
					log.Infof("Unlocking IPAddress on Node [%v] before restart", eachNode.Name)
					err = BlockIPTabelOnNode(&eachNode, false)
					log.FailOnError(err, "failed to unblock IP tables on Node [%v]", eachNode.Name)
				...
				ValidateApplications(contexts)
				log.InfoD("Application validation done after network packet drop on all nodes")
			})
...
func BlockIPTabelOnNode(n *node.Node, block bool) error {

	command := fmt.Sprintf("iptables -A INPUT -p tcp -s %v -j DROP ", n.MgmtIp)
	if !block {
		command = fmt.Sprintf("iptables -D INPUT -p tcp -s %v -j DROP", n.MgmtIp)
	}
	...
	command = fmt.Sprintf("iptables -A OUTPUT -p tcp -d %v -j DROP ", n.MgmtIp)
	if !block {
		command = fmt.Sprintf("iptables -D OUTPUT -p tcp -d %v -j DROP", n.MgmtIp)
	}
	log.InfoD("Triggering command [%s] from Node [%v]", command, n.Name)
	err = runCommand(command, *n)
	if err != nil {
...
var _ = Describe("{AsyncDRMigrationWithKVDBRunFlatStateOnSource}", Label("staging", "p1", "negative", "AsyncDR", "kvdb_ops"), func() {
	/*
		https://purestorage.atlassian.net/browse/HAZEL-1943
		1. Deploy applications
		2. Create migration schedule
		3. while migration is in progress, px stop on two kvdb members and validate cluster is in runflat state on source
	...
	*/
	...
	JustBeforeEach(func() {
		StartTorpedoTest("AsyncDRMigrationWithKVDBRunFlatStateOnSource", "Perform failover with run-flat state on source cluster", nil, testrailID)
		runID = testrailuttils.AddRunsToMilestone(testrailID)
	})

	stepLog := "Bring cluster in to run-flat state on source, perform failover, and validate the operation"
	It(stepLog, func() {
		log.InfoD(stepLog)
