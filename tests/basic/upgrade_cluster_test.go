		...
		for _, version := range versions {
			...
			Step(fmt.Sprintf("start [%s] scheduler upgrade", Inst().S.String()), func() {
				...
				if Inst().S.String() == eks.SchedName {
					log.Warnf("This is [%s] scheduler, during Node Pool upgrades, EKS creates an extra node. "+
						"After the Node Pool upgrade is complete, EKS deletes this extra node, but it takes some time.", Inst().S.String())
					log.Infof("Sleeping for 30 minutes to let the cluster stabilize after the upgrade..")
					time.Sleep(30 * time.Minute)
				}
				printK8sCluterInfo()
