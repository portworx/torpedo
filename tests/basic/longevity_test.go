...
var _ = Describe("{UpgradeLongevity}", func() {
	...
	It("has to schedule app and register test triggers", func() {
		Step(fmt.Sprintf("Start watch on K8S configMap [%s/%s]", configMapNS, testTriggersConfigMap), func() {
			...
			err := watchConfigMap()
			if err != nil {
				log.Fatalf("failed to watch on K8S configMap [%s/%s]. Err: %v", configMapNS, testTriggersConfigMap, err)
			}
		})
			...
			for triggerType, triggerFunc := range upgradeTriggerFunction {
				go func(triggerType string, triggerFunc TriggerFunction) {
					...
					for {
						...
						shouldTrigger := minTestExecCount >= (currentEndpointIndex+1)*executionThreshold
						// Proceeding with upgrade if testExecSum is much higher than expected
						if !shouldTrigger && testExecSum >= (currentEndpointIndex+1)*(executionThreshold+1) {
							shouldTrigger = true
							// Logging a warning as TestExecutionCountMap might not be accurate
							log.Warnf("Triggering %s based on testExecSum %v. The TestExecutionCountMap may not be accurate: %+v", triggerType, testExecSum, TestExecutionCountMap)
						}
						if shouldTrigger {
