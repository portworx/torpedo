...
var _ = Describe("{CreateAndValidatePVCWithIopsAndBandwidthFA}", func() {
	/*
				https://purestorage.atlassian.net/browse/PTX-23995
		                1. Create storage class with max iops and max bandwidth for Normal Portworx Volumes , FADA and FBDA Pvc Deployment
				2. Create 10 PVC each with respective storage class parallely
	...
	*/
	JustBeforeEach(func() {
		StartTorpedoTest("CreateAndValidatePVCWithIopsAndBandwidthFA",
			"Create PVCs with updated MaxBandwidth / Max IOPS ( update the storage class )",
			nil, 0)
	})
	itLog := "CreateAndValidatePVCWithIopsAndBandwidth"
	It(itLog, func() {
		...
		listofFadaPvc := make([]string, 0)
		//Creating maps with namespaces as key and list of pvc names as values
		namespaces := []string{FadaAppNameSpace, BaseAppNameSpace}
		namespacePVCMap := make(map[string][]string)
		for _, ns := range namespaces {
			namespacePVCMap[ns] = []string{}
		}
	...
	*/
	...
	It(itLog, func() {
		...
		for _, fa := range flashArrays {
			...
			podName := RealmName + "::" + "test-automation-pod5"
	...
	It(itLog, func() {
		...
		Step(stepLog, func() {
			...
			for _, nodeToReboot := range selectedNodesForReboot {
				log.InfoD("Stopping Portworx Service on Node [%v]", nodeToReboot.Name)
				err := Inst().V.StopDriver([]node.Node{nodeToReboot}, false, nil)
				log.FailOnError(err, "Failed to stop portworx on node [%v]", nodeToReboot.Name)
				err = Inst().V.WaitDriverDownOnNode(nodeToReboot)
				log.FailOnError(err, fmt.Sprintf("Driver is up on node [%s]", nodeToReboot.Name))
				err = Inst().V.StartDriver(nodeToReboot)
				log.FailOnError(err, fmt.Sprintf("Failed to start portworx on node [%s]", nodeToReboot.Name))
				err = Inst().V.WaitDriverUpOnNode(nodeToReboot, addDriveUpTimeOut)
