	...
	It("Validates volume creation and deletion in parallel", func() {
		...
		deleteVolume := func(volType VolumeType, vol *api.Volume) error {
			...
			waitForVolumeDeletion := func() (interface{}, bool, error) {
				...
				return nil, true, fmt.Errorf("volume [%s/%s] still exists", vol.Id, vol.Locator.Name)
			}
			_, err = task.DoRetryWithTimeout(waitForVolumeDeletion, 5*time.Minute, 30*time.Second)
			if err != nil {
				return fmt.Errorf("failed to wait for volume [%s/%s] deletion. Err: [%v]", vol.Id, vol.Locator.Name, err)
			}
			return nil
		}
