...
var _ = Describe("{ScaleResizePVCDeleteApps}", Label("p1", "positive", "pure_ops"), func() {
	JustBeforeEach(func() {
		StartTorpedoTest("ScaleResizePVCDeleteApps", "Test creates multiple FADA volumes,resize pvc and checks if all the pods,pvc's are being deleted gracefully", nil, 302394)
	})

	It("schedules multiple nginx fada volumes, resize pvc's and checks if all the resources created are deleted gracefully", func() {
		var contexts = make([]*scheduler.Context, 0)
		requestedVols := make([]*volume.Volume, 0)
		rand.Seed(time.Now().Unix())
		...
		if len(workerNodes) < 2 {
			log.FailOnError(fmt.Errorf("need atleast 2 worker nodes to run this test"), "error")
		}
		// Select 2 random nodes
		selectedNode1 := workerNodes[rand.Intn(len(workerNodes))]
		selectedNode2 := workerNodes[rand.Intn(len(workerNodes))]

		// Ensure both nodes are different, if required (optional)
		for selectedNode1.Name == selectedNode2.Name {
			selectedNode2 = workerNodes[rand.Intn(len(workerNodes))]
		}
		log.InfoD("Selected Nodes: %v, %v", selectedNode1.Name, selectedNode2.Name)
		defer func() {
			Inst().AppList = applist
			err = Inst().S.RemoveLabelOnNode(selectedNode1, "apptype")
			log.FailOnError(err, "error removing label on node [%s]", selectedNode1.Name)
			err = Inst().S.RemoveLabelOnNode(selectedNode2, "apptype")
			log.FailOnError(err, "error removing label on node [%s]", selectedNode2.Name)
		}()
		Inst().AppList = []string{"nginx-fada-deploy"}
		err = Inst().S.AddLabelOnNode(selectedNode1, "apptype", k8s.PureDAVolumeLabelValueFA)
		log.FailOnError(err, fmt.Sprintf("Failed add label on node %s", selectedNode1.Name))
		err = Inst().S.AddLabelOnNode(selectedNode2, "apptype", k8s.PureDAVolumeLabelValueFA)
		log.FailOnError(err, fmt.Sprintf("Failed add label on node %s", selectedNode2.Name))
		Provisioner := fmt.Sprintf("%v", portworx.PortworxCsi)
		...
		Step("Schedule applications", func() {
			log.InfoD("Scheduling applications")
			for j := 0; j < NumberOfDeployments; j++ {
				taskName := fmt.Sprintf("resizetest-%v", j)
				context, err := Inst().S.Schedule(taskName, scheduler.ScheduleOptions{
					AppKeys:            Inst().AppList,
					StorageProvisioner: Provisioner,
					PvcSize:            6 * units.GiB,
		...
		log.FailOnError(err, "failed to get secret [%s]  in namespace [%s]", PureSecretName, volDriverNamespace)
		flashArrays := pxPureSecret.Arrays
		stepLog = fmt.Sprintf("Resize and validate pvc,destroy apps and check if the pvc's are deleted gracefully")
		Step(stepLog, func() {
			for _, ctx := range contexts {
				var appVolumes []*volume.Volume
				stepLog = fmt.Sprintf("get volumes for %s app", ctx.App.Key)
				Step(stepLog, func() {
					...
					log.Infof("len of app volumes is : %v", len(appVolumes))
					if len(appVolumes) == 0 {
						err = fmt.Errorf("no volumes found to perform the test")
						log.FailOnError(err, "found no volumes for app %s", ctx.App.Key)
					}
				})
			...
			stepLog = fmt.Sprintf("Destroy Applications")
			Step(stepLog, func() {
				var wg sync.WaitGroup
				opts := make(map[string]bool)
				opts[scheduler.OptionsWaitForResourceLeakCleanup] = true
				for j := 0; j < NumberOfDeployments; j++ {
					...
					go func(index int) {
						defer wg.Done()
						TearDownContext(contexts[index], opts)
					}(j)
				}
				wg.Wait()
				log.Infof("waiting for 5 mins allowing volumes to delete in backend")
				time.Sleep(5 * time.Minute)
			})
	...
	*/
	...
	It(stepLog, func() {
		...
		Step(stepLog, func() {
			...
			Step(stepLog, func() {
				// sleep for 15 minutes for all the entries to update
				time.Sleep(15 * 60 * time.Second)
				log.InfoD("Sleeping for 15 minutes for all the entries to update")
...
var _ = Describe("{CreateCsiSnapshotsforFADAandDelete}", Label("p0", "positive", "snapshot_ops", "px_vol_ops", "pure_ops"), func() {
	...
	*/
	...
	It(itLog, func() {
		...
		stepLog = "Validate volume snapshot classes are deleted"
		Step(stepLog, func() {
			log.InfoD(stepLog)
	...
	*/
	...
	It(itLog, func() {
		...
		Step(stepLog, func() {
			...
			clusterUUID, err := Inst().N.RunCommand(workerNodes[0], cmd, opts)
	...
	*/
	...
	It(itLog, func() {
		...
		Step(stepLog, func() {
			...
			for _, ns := range namespaces {
				err := core.Instance().DeleteNamespace(ns)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Delete namespace [%v]", ns))
...
var _ = Describe("{FastpathEncryptSharedVolumeDetachAfterPoolOfflineWithNodeRestart}", Label("staging", "p1", "pure_ops", "negative", "error_injection", "node_reboot"), func() {
	/*
	   https://purestorage.atlassian.net/browse/HAZEL-1560
	    1. Create a Fastpath secure shared volume, attach it, and verify that it is mounted and Fastpath Active.
	    2. Perform IO operations on the volume.
	    3. Verify that the pool is offline and the volume is not in quorum.
	    4. Reboot the node.
	    5. Ensure that detaching and deleting the volume is successful.
	*/
	...
	JustBeforeEach(func() {
		StartTorpedoTest("FastpathEncryptSharedVolumeDetachAfterPoolOfflineWithNodeRestart", "Create a pool and fast path encrypt shared volume and make pool offline. validate volume is detachable after node restart", nil, 0)
	...
	stepLog := "Create a pool and fast path encrypt shared volume and make pool offline. validate volume is detachable after node restart"
	It(stepLog, func() {
		log.InfoD(stepLog)
		...
		Step(stepLog, func() {
			...
			volumeId, err = CreateAndAttachFPVolume(*selectedStNode, volumeName, selectedPool, volSize, haLevel, true, true)
		...
		stepLog = "Reboot the volume attached node and validate whether the volume is detachable and deletable"
		Step(stepLog, func() {
			log.InfoD(stepLog)

			log.Infof("Restart the volume attached node [%s]", selectedStNode.Name)
			err = RebootNodeAndWaitForPxDown(*selectedStNode)
			log.FailOnError(err, "Failed to reboot node %s", selectedStNode.Name)

			t := func() (interface{}, bool, error) {
				if status := Inst().V.IsPxReadyOnNode(*selectedStNode); !status {
					return "", true, fmt.Errorf("PX is not ready on node %s", selectedStNode.Name)
				}
				return "", false, nil
			}
	...
	*/
	...
	It(stepLog, func() {
		...
		Step(stepLog, func() {
			...
			volSize = int(poolSizeInGiB + 50)
			volumeId, err = CreateAndAttachFPVolume(*selectedStNode, volumeName, selectedPool, volSize, haLevel, true, false)
			log.FailOnError(err, fmt.Sprintf("Failed to create or attach volume [%v]", volumeName))
...
var _ = Describe("{FastpathEncryptVolumeDetachAfterPoolOfflineWithNodeRestart}", Label("staging", "p1", "pure_ops", "negative", "error_injection", "node_reboot"), func() {
	/*
	   https://purestorage.atlassian.net/browse/HAZEL-1557
	...
	*/
	...
	JustBeforeEach(func() {
		StartTorpedoTest("FastpathEncryptVolumeDetachAfterPoolOfflineWithNodeRestart", "Create a pool and fast path encrypt volume and make pool offline. validate volume is detachable after node restart", nil, 0)
	...
	stepLog := "Create a pool and fast path encrypt volume and make pool offline. validate volume is detachable after node restart"
	It(stepLog, func() {
		log.InfoD(stepLog)
