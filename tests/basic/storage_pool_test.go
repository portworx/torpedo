...
var _ = Describe("{StorageFullPoolResize}", Label("p0", "positive", "px_ops", "pool_ops", "PoolExpand", "ResizeDisk", "Throttling"), func() {
	...
	stepLog := "Create vols and make pool full"
	It(stepLog, func() {
		storageFullPoolExpansion("StorageFullPoolResize")
...
func storageFullPoolExpansion(testName string) {
	...
	log.InfoD(fmt.Sprintf("Create vols and make pool full and expand using %s", operation))
...
var _ = Describe("{StorageFullPoolAddDisk}", Label("p0", "positive", "px_ops", "pool_ops", "PoolExpand", "AddDrive", "Throttling"), func() {
	...
	It(stepLog, func() {
		...
		It(stepLog, func() {
			storageFullPoolExpansion("StorageFullPoolAddDisk")
		...
		if strings.Contains(param, "size") {
			if sizeOfDisk == 0 {
				...
				for _, pool := range poolDetails {
					if pool.ID == poolID {
						poolSize = pool.TotalSize / units.GiB
						paramsArr = append(paramsArr, fmt.Sprintf("size=%d", poolSize))
					}
				}
			} else {
	...
	*/
	...
	It(stepLog, func() {
		...
		if err != nil {
			driveCompatibleErr := strings.Contains(err.Error(), "Drive not compatible with specified pool")
			if driveCompatibleErr {
				log.Infof("This is expected as we are trying to expand a pool using add-disk more than its pool size")

			} else {
				log.FailOnNoError(err, "Error while adding Disk to the pool")
			}
		}
...
var _ = Describe("{CreateNewPoolsOnClusterInParallel}", Label("p0", "positive", "pool_ops", "MiniScale"), func() {
	/*
				...
				for Automation : Trying to add Drives using legacy method to create new pools on 3 nodes in the cluster
	*/
	JustBeforeEach(func() {
	...
	It(stepLog, func() {
		...
		selectedNodeIds := make([]string, 0)
		excludeNodeIds := make([]string, 0)

		ctx0 := contexts[0]

		vols, err := Inst().S.GetVolumes(ctx0)
		...
		for _, eachNodeId := range selectedNodeIds {
			eachNode, err := node.GetNodeDetailsByNodeID(eachNodeId)
			log.FailOnError(err, "Failed to get node details by node id [%s]", eachNodeId)
			sPools, err := GetPoolsDetailsOnNode(&eachNode)
			log.FailOnError(err, "Failed to get pool details on node [%s]", eachNode.Name)
			if len(sPools) < 8 {
				nodesToUse = append(nodesToUse, eachNode)
			}
			if len(nodesToUse) == 3 {
				break
			}
...
func pickPoolToResize(contexts []*scheduler.Context, expandType api.SdkStoragePool_ResizeOperationType, targetIncrementInGiB uint64, excludeNodeIDs ...string) string {
	...
	if err != nil {
		log.Warnf("Error identifying pool with IOs, Error: %v", err)
	}
...
var _ = Describe("{HAIncreasePoolresizeAndAdddisk}", Label("p0", "positive", "px_vol_ops", "pool_ops", "PoolExpand", "ResizeDisk", "HA_Increase_Decrease"), func() {
	/*
		...
		https://portworx.testrail.net/index.php?/cases/view/57783
		 ...
		 2. Pool resize trigger while ha-inprogress
		 3. Add disk to the pool while ha-inprogress
	...
	*/
	...
	It(itLog, func() {
		...
		eachContext := contexts[rand.Intn(len(contexts))]
		...
		Step(stepLog, func() {
			log.InfoD(stepLog)
			//initiate pool expand on the pool where ha increase is happening with resize-disk
			...
			//initiate pool expand on the pool where ha increase with add-disk
			pool, err = GetStoragePoolByUUID(poolToBeUpdated)
			log.FailOnError(err, "Failed to get pool using UUID %s", poolToBeUpdated)
...
func PoolDeleteWithTimeInterval(testName, testDesc, testType string) {
	...
	It(ItLog, func() {
		...
		driveMap, err := Inst().V.GetPoolDrives(&selectedNode)
		log.FailOnError(err, fmt.Sprintf("error getting pool drive details for the node %s", selectedNode))

		if testName == "yank-drive-pool-delete-px-restart" || testName == "yank-drive-pool-delete-node-reboot" || testName == "yank-drive-pool-delete-node-maintenance-cycle" {
			poolDrive := ""
			for _, drive := range driveMap[poolIDToDelete] {
				if drive.Device != "" {
			...
			Step(stepLog, func() {
				log.InfoD(stepLog)
				busID, err = Inst().N.YankDrive(selectedNode, poolDrive, node.ConnectionOpts{
					Timeout:         dfDefaultTimeout,
					TimeBeforeRetry: dfDefaultRetryInterval,
				})
				log.FailOnError(err, fmt.Sprintf("failed to yank journal drive on node [%s]", selectedNode.Name))
			...
			Step(stepLog, func() {
				log.InfoD(stepLog)
				err = Inst().N.RecoverDrive(selectedNode, poolDrive, busID, node.ConnectionOpts{
					Timeout:         driveFailTimeout,
					TimeBeforeRetry: dfDefaultRetryInterval,
				})
				log.FailOnError(err, fmt.Sprintf("failed to recover yank journal drive on node [%s]", selectedNode.Name))
		...
		if testName == "delete-pool-yank-drive-px-restart" || testName == "delete-pool-yank-drive-node-reboot" || testName == "delete-pool-yank-drive-node-maintenance-cycle" {
			poolDrive := ""
			stepLog = "Select drive to yank"
			Step(stepLog, func() {
				log.InfoD(stepLog)
				driveMap, err := Inst().V.GetPoolDrives(&selectedNode)
				log.FailOnError(err, fmt.Sprintf("error getting pool drive details for the node %s", selectedNode))
				...
				for poolID, drives := range driveMap {
					if poolDrive == "" {
						for _, drive := range drives {
							if drive.Device == "" {
								log.InfoD("device path is empty for pool %s ", poolID)
								break
							}
							if !strings.Contains(drive.Device, jPath) {
								poolDrive = drive.Device
								break
							}
						}
		...
		if testType == "node_reboot" {
			//node restart
			stepLog = fmt.Sprintf("Verify reboot after [%d] seconds", sleepTime)
			Step(stepLog, func() {
				log.InfoD(stepLog)
				err = Inst().N.RebootNodeAndWait(*testNode)
				log.FailOnError(err, "Failed to reboot node and wait till it is up")
				log.Info("Verify reboot succeed")
				time.Sleep(time.Minute * 1)
			})
		} else if testType == "px_restart" {
			//Restart Portworx
		...
		} else if testType == "node_maintenance_cycle" {
...
var _ = Describe("{PoolDeleteWithNodeRebootWithTimeInterval}", Label("p1", "hal_ops_disruption", "node_reboot", "PoolDelete", "functional"), func() {
	testName = "delete-pool-node-reboot"
	testDescription = "Pool delete with node reboot with random time interval"
...
var _ = Describe("{PoolResizeWithNodeRebootWithTimeInterval}", Label("p0", "negative", "pool_ops", "node_ops", "PoolExpand", "node_reboot"), func() {
	...
	*/
	...
	testName = "PoolResizeWithNodeRebootWithTimeInterval"
	testDescription = "Pool resize with node reboot"
...
var _ = Describe("{AddNewPoolWithNodeReboot}", Label("p1", "hal_ops_disruption", "node_reboot", "AddNewPool", "functional"), func() {
	/*
	   Install portworx in a 5 node cluster
	   Select a node and add drive to create new pool (pxctl sv drive add --newpool -s "type=thin,size=128")
	   With some random delay reboot the node
	   After node comes up verify px status and check if pool is successfully added and apps are running.
	*/

	JustBeforeEach(func() {
		StartTorpedoTest("AddNewPoolWithNodeReboot", "Automate drive add newpool with node reboot", nil, 0)
	})
	...
	itLog := "AddNewPoolWithNodeReboot"
	It(itLog, func() {
...
var _ = Describe("{PoolExpandAddDriveWithNodeReboot}", func() {
	/*
		1. Create volume and do IOs / deploy apps to do IOs
		2. Expand pool by adding drive
		3. After some random delay reboot the node
		4. Wait for node to comeup and check PX status
		5. Verify pool expanded succesfully
		6. Delete the pool
		7. Receate the pool
	*/

	testName = "PoolExpandAddDriveWithNodeReboot"
	testDescription = "Pool Expand Add Drive With Node Reboot"
	ExpandPoolWithAddDrive(testName, testDescription)

})

var _ = Describe("{PoolDeleteWithPXRestart}", Label("p1", "hal_ops_disruption", "px_restart", "PoolDelete", "functional"), func() {
	testName = "delete-pool-px-restart"
	testDescription = "Pool delete with px restart with random time interval"
	PoolDeleteWithTimeInterval(testName, testDescription, "px_restart")
})

var _ = Describe("{AddDriveWithNodeMaintenance}", func() {
	/*
		1. Create volume and do IOs / deploy apps to do IOs
		2. Validate apps
		3. Expand pool by adding drive
		4. After some random issue node maintenance cycle
		5. Wait for node to comeup and check PX status
		6. Verify pool expanded succesfully
	*/

	testName = "AddDriveWithNodeMaintenance"
	testDescription = "Add Drive With Node Maintenance"
	ExpandPoolWithAddDrive(testName, testDescription)

})

var _ = Describe("{PoolResizeWithNodeMaintenanceCycleWithTimeInterval}", func() {

	/*
	   1.  Create volume and do IOs / deploy apps to do IOs
	   2.  Select a pool to resize
	   3.  Expand selected pool with resize-disk type
	   4.  Performing node maintenance cycle on selected node with some random delay
	   5.  Wait for pool to expand
	   6.  Verify pool resized
	   7.  Check px status
	   8.  Check if apps are running
	*/
	...
	JustBeforeEach(func() {
		StartTorpedoTest("PoolResizeWithNodeMaintenanceCycleWithTimeInterval", "Pool expand resize disk with node maintenance cycle with random delay", nil, 0)
	...
	testName = "PoolResizeWithNodeMaintenanceCycleWithTimeInterval"
	testDescription = "Pool expand resize disk with node maintenance cycle with random delay"
	PoolResize(testName, testDescription)
...
var _ = Describe("{PoolDeleteWithNodeMaintenanceCycleWithTimeInterval}", Label("p1", "hal_ops_disruption", "NodeMaintenance", "PoolDelete", "functional"), func() {
	testName = "delete-pool-node-maintenance-cycle"
	testDescription = "Pool delete with node maintenance cycle with random time interval"
			...
			go func() {
				...
				for _, selectedNode := range stNodes {
					...
					for i := 4; i <= 100; i += 10 {
						...
						err = Inst().V.ExpandPool(poolToBeResized.Uuid, api.SdkStoragePool_RESIZE_TYPE_RESIZE_DISK, expectedSize, true)
						if err != nil {
							err = fmt.Errorf("Failed to expand pool with resize disk in Node [%s] , err : %v", selectedNode.Name, err)
							errChan <- err
							return
						}
...
var _ = Describe("{RestartPxDuringPoolDeletion}", Label("p1", "hal_ops_disruption", "px_restart", "PoolDelete", "staging"), func() {
	/*
		Jira-ID : https://purestorage.atlassian.net/browse/HAZEL-1013
		Create apps on multiple pools
		Pick more than 2 Pools to delete across 2 different nodes where apps are running
		Start deletion of these 2 pools
		Restart Portworx on both nodes while deletion is in progress
		Once Portworx comes up, pool deletion should succeed on both nodes
		Validate apps
	*/
	var (
		contexts         = make([]*scheduler.Context, 0)
		...
		jrnlPartPoolID   string
		poolIDandNodeMap = make(map[string]string)
		jPath            string
		wg               sync.WaitGroup
		selectedNodes    = make(map[string]node.Node)
		selectedAppNodes = make([]node.Node, 0)
	)

	JustBeforeEach(func() {
		StartTorpedoTest("RestartPxDuringPoolDeletion", "Restart px during pool deletion process", nil, 0)
	})

	itLog := "RestartPxDuringPoolDeletion"
	It(itLog, func() {
		log.InfoD(itLog)
		for i := 0; i < Inst().GlobalScaleFactor; i++ {
			taskName := "restartpxduringpooldeletion"
		...
		for _, ctx := range contexts {
			...
			selectedAppNodes = append(selectedAppNodes, appNodes...)
		}

		selectedAppNodes = selectedAppNodes[:2] // 2 different nodes where apps are running
		for _, node := range selectedAppNodes {
		...
		Step(stepLog, func() {
			...
			for _, selectedNode := range selectedNodes {
				err = EnterPoolMaintenance(selectedNode)
				log.FailOnError(err, "Failed to enter in maintenance mode for the node %s", selectedNode.Name)
				log.Info("enter pool maintenance mode succeed for the node %s", selectedNode.Name)
			}
		})

		exitPoolMaintenanceMode := func() {
			for _, selectedNode := range selectedNodes {
				err = ExitPoolMaintenance(selectedNode)
				log.FailOnError(err, "Failed to exit maintenance mode for the node %s", selectedNode.Name)
				log.Info("exit pool maintenance mode succeed for the node %s", selectedNode.Name)
			}
		}
		defer exitPoolMaintenanceMode()

		stepLog = "Selecting pool to delete"
		for nodeID, selectedNode := range selectedNodes {
			Step(stepLog, func() {
				log.InfoD(stepLog)
				nodePools := selectedNode.StoragePools
				poolToDelete = selectedNode.StoragePools[0]
				isjournal, err := IsJournalEnabled()
				log.FailOnError(err, "Failed to check if Journal enabled")
				if isjournal && len(nodePools) > 1 {
					jDev, err := Inst().V.GetJournalDevicePath(&selectedNode)
				...
				poolIDandNodeMap[nodeID] = strconv.Itoa(int(poolToDelete.GetID()))
				log.Infof("pool selected for deletion [%v]", poolToDelete.ID)
			})
		}

		for seletedNodeID, selectedPoolIDToDelete := range poolIDandNodeMap {
			selectedNode := selectedNodes[seletedNodeID]
			stepLog = fmt.Sprintf("Delete the pool [%v] on node [%v]", selectedPoolIDToDelete, selectedNode.Name)
			Step(stepLog, func() {
				log.InfoD(stepLog)
				wg.Add(1)
				go func() {
					defer wg.Done()
					err := Inst().V.DeletePool(selectedNode, selectedPoolIDToDelete, false)
					log.InfoD(fmt.Sprintf("Failed to delete pool [%s], err : [%v]", selectedPoolIDToDelete, err))
					dash.VerifyFatal(err != nil, true, fmt.Sprintf("Pool Deletion have not succeed for the pool-ID : [%s]", selectedPoolIDToDelete))
				}()
			})
		}

		stepLog = "Restart Portworx Service on selected nodes"
		Step(stepLog, func() {
			log.InfoD(stepLog)
			for _, nodeToRestart := range selectedNodes {
				log.InfoD("Restarting portworx  Service on Node [%v]", nodeToRestart.Name)
				err := Inst().V.RestartDriver(nodeToRestart, nil)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Failed to restart portworx on node [%v]", nodeToRestart.Name))
				log.InfoD("Restarted portworx on node %s", nodeToRestart.Name)
			}
			wg.Wait() // wait for pool deletion to be completed
		})

		stepLog = "Exit pool maintenance mode"
		Step(stepLog, func() {
			log.InfoD(stepLog)
			exitPoolMaintenanceMode()
		})
		...
		for nodeID, selectedNode := range selectedNodes {
			...
			// Verify pool delete
			stepLog = "Verify pools are not deleted"
			Step(stepLog, func() {
				log.InfoD(stepLog)
				currentPoolsOnNode := selectedNode.StoragePools
				log.FailOnError(err, fmt.Sprintf("error getting pool drive from the node [%s], Err: %v", selectedNode.Name, err))
				for _, poolID := range currentPoolsOnNode {
					poolsMap[strconv.Itoa(int(poolID.ID))] = true
				}
				log.Infof("Pools on the Node %v and the existing poolsMap %v", selectedNode, poolsMap)
				if !poolsMap[poolIDandNodeMap[nodeID]] {
					log.FailOnError(fmt.Errorf("pool [%s] deleted on the node [%s]", poolIDandNodeMap[nodeID], selectedNode.Name), fmt.Sprintf("pool [%s] deleted on the node [%s]", poolIDandNodeMap[nodeID], selectedNode.Name))
				}
				log.InfoD("verify pool delete [%s] not succeed", poolIDandNodeMap[nodeID])
			})
		}
...
var _ = Describe("{AddDriveWithNodeRebootAndNodeMaintenanceMode}", Label("p1", "staging", "negative", "error_injection", "pool_ops", "PoolExpand", "node_reboot", "AddDrive"), func() {
	/*
		       ticket id: https://purestorage.atlassian.net/browse/HAZEL-1021
			   1. Expand a pool by adding a drive
			   2. While drive addition is in progress, reboot the node on which pool is created
			   3. When node comes up, validate what happened to pool expansion - it should still succeed
			   4. If it fails, fail the test
			   5. Put node in maintenance mode
			   6. Trigger pool expansion using add-disk
			   7. Exit maintenance mode
			   8. Validate pool expansion
			   9. Repeat Steps 2 to 8 at least 3 times for same pool
	*/
	...
	JustBeforeEach(func() {
		StartTorpedoTest("AddDriveWithNodeRebootAndNodeMaintenanceMode", "Expand the pool by adding a drive, reboot the node, and put the node in maintenance mode", nil, testrailID)
		runID = testrailuttils.AddRunsToMilestone(testrailID)
	})

	stepLog := "Should get an existing storage node, expand the pool by adding a drive, reboot the node, and put the node in maintenance mode"
	...
	It(stepLog, func() {
		...
		for iteration := 1; iteration <= 3; iteration++ {
			log.Infof("Iteration %d: Adding cloud drive with rebooting node and maintenancemode ", iteration)
			stNode, err := getRandomNodeWithPoolIOs(contexts)
			log.FailOnError(err, "error identifying node to run test")
			pools, err := Inst().V.ListStoragePools(metav1.LabelSelector{})
			log.FailOnError(err, "error getting pools list")
			...
			log.Infof("Cumulative current total pool size: %d GiB", currentTotalPoolSize)
			driveSpecs, err := GetCloudDriveDeviceSpecs()
			log.FailOnError(err, "Error getting cloud drive specs")
			deviceSpec := driveSpecs[0]
			deviceSpecParams := strings.Split(deviceSpec, ",")
			...
			expectedTotalPoolSize := currentTotalPoolSize + specSize
			log.Infof("Calculated expected total pool size: currentTotalPoolSize (%d GiB) + specSize (%d GiB) = expectedTotalPoolSize (%d GiB)",
				currentTotalPoolSize, specSize, expectedTotalPoolSize)
			stepLog := "Initiate add cloud drive and reboot node"
			Step(stepLog, func() {
				log.InfoD(stepLog)
				err = Inst().V.AddCloudDrive(&stNode, deviceSpec, -1)
				...
				log.Infof("After adding cloud drives total pool size: %d GiB", newTotalPoolSize)
				dash.VerifyFatal(newTotalPoolSize == expectedTotalPoolSize, true, fmt.Sprintf("Validate total pool size after add cloud drive on node %s", stNode.Name))
			})
			...
			stepLog = fmt.Sprintf("Entering maintenance mode on node %s", stNode.Name)
			Step(stepLog, func() {
				log.InfoD(stepLog)
				err = Inst().V.EnterMaintenance(stNode)
				log.FailOnError(err, fmt.Sprintf("fail to enter node %s in maintenance mode", stNode.Name))
				status, err := Inst().V.GetNodeStatus(stNode)
				log.FailOnError(err, "unable to get the node status")
				log.InfoD(fmt.Sprintf("Node %s status %s", stNode.Name, status.String()))

			})
			stepLog = "Expand pool while node in maintance mode"
			Step(stepLog, func() {
				log.InfoD(stepLog)
				drvSize, err := getPoolDiskSize(poolToBeResized)
				log.FailOnError(err, "error getting drive size for pool [%s]", poolToBeResized.Uuid)
			...
			stepLog = fmt.Sprintf("Exiting maintenance mode on node %s", stNode.Name)
			Step(stepLog, func() {
				log.InfoD(stepLog)
				t := func() (interface{}, bool, error) {
					status, err := Inst().V.GetNodeStatus(stNode)
...
var _ = Describe("{AddDriveWithPXRestartForMultipleIterations}", Label("p0", "staging", "negative", "error_injection", "px_ops", "pool_ops", "PoolExpand", "px_restart", "AddDrive"), func() {
	...
	*/
	...
	It(itLog, func() {
		...
		getCurrentPoolsize := func() uint64 {
		...
		Step(stepLog, func() {
			log.InfoD(stepLog)
			for i := 0; i < 5; i++ {
				currentTotalPoolSize := getCurrentPoolsize()
...
var _ = Describe("{HAIncreasePoolExpandAddDisk}", Label("p0", "positive", "px_vol_ops", "pool_ops", "PoolExpand", "HA_Increase_Decrease"), func() {
	/*
		...
		1. Create Apps
		2. Start HA Increase of mulitple volumes at once (maybe 20 volumes HA Increase in parallel)
		3. While HA Increase is happening, add Drive to atleast 3 different pools where HA INcrease is happening
		4. Validate apps
	*/
	...
	It(itLog, func() {
		...
		volHaIncreasePoolMap = make(map[string][]string, len(vols))
		errChan = make(chan error, 1)
		stepLog = "HA increase on all the app volumes"
		Step(stepLog, func() {
			log.InfoD(stepLog)
			for _, vol := range vols {
				...
				go func() {
					...
					curReplSet, err := Inst().V.GetReplicationFactor(vol)
					if err != nil {
						err = fmt.Errorf("failed to get repl factor for the volume %s , err - %v", vol.Name, err)
						errChan <- err
						return
					}
					...
					if curReplSet == 3 {
						...
						newRepl := int64(curReplSet - 1)
						err = Inst().V.SetReplicationFactor(vol, newRepl, nodesToBeUpdated, poolsToBeUpdated, true)
						if err != nil {
							err = fmt.Errorf("Failed to set Replicaiton factor for the vol %s to %d , err - %v", vol.Name, newRepl, err)
							errChan <- err
							return
						}
					} else {
						...
						poolsUuid, err := GetAllPoolsOnNode(nodeToBeUpdated.Id)
						if err != nil {
							err = fmt.Errorf("Failed to get pool using node %s , err - %v", nodeToBeUpdated.Id, err)
							errChan <- err
							return
						}
					...
					var maxReplicaFactor int64
					maxReplicaFactor = 3
					log.FailOnError(Inst().V.SetReplicationFactor(vol, maxReplicaFactor, nodesToBeUpdated, poolsToBeUpdated, false), fmt.Sprintf("Failed to set Replicaiton factor for the volume %s", vol.Name))
					volHaIncreasePoolMap[vol.ID] = poolsToBeUpdated
				}()
			}
		})
		...
		for err := range errChan {
			log.FailOnError(err, "ha increase failed")
		}
		stepLog = "Verify volume replica are in resync state"
		Step(stepLog, func() {
			log.InfoD(stepLog)
			poolsWithHAIncrease = make(map[string]bool, len(vols))
			for _, vol := range fioVolList {
				t := func() (interface{}, bool, error) {
					volDetails, err := Inst().V.InspectVolume(vol.ID)
					if err != nil {
				...
				_, err = task.DoRetryWithTimeout(t, 5*time.Minute, 10*time.Second)
				log.FailOnError(err, fmt.Sprintf("volume %s not in resync state", vol.ID))
				for _, poolID := range volHaIncreasePoolMap[vol.ID] {
					poolsWithHAIncrease[poolID] = true
				}
			}
		})

		log.Infof("Unique list of pools where HA increase is going on : %v", poolsWithHAIncrease)
		dash.VerifyFatal(len(poolsWithHAIncrease) >= 3, true, "verify atleast 3 different pools are available for add-disk pool expansion")

		stepLog = "Pool expand add disk trigger"
		Step(stepLog, func() {
			log.InfoD(stepLog)
			for poolID, _ := range poolsWithHAIncrease {
				pool, err := GetStoragePoolByUUID(poolID)
				log.FailOnError(err, "Failed to get pool using UUID %s", poolID)
