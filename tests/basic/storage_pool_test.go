...
var _ = Describe("{AddDriveBeyondMaxSupported}", func() {
	...
	*/
	...
	It(stepLog, func() {
		...
		poolList, err := GetPoolsDetailsOnNode(stNodes[0])
		for _, pool := range poolList {
			if pool.ID != 0 {
				err = Inst().V.DeletePool(stNodes[0], string(pool.ID), true)
	...
	It(stepLog, func() {
		...
		err = addNewPools(stNodes[0], poolLeft)
		err = addNewPools(stNodes[0], 1)
	...
	It(stepLog, func() {
		...
		poolLeft := 8 - len(poolList)
		err = addNewPools(stNodes[0], poolLeft)
		numberOfDrivesCanBeAdded = numberOfDrivesCanBeAdded - poolLeft
		for numberOfDrivesCanBeAdded != 0 {
			for _, pool := range poolList {
				log.FailOnError(err, "error getting node from pool.ID")
				err = addCloudDrive(stNodes[0], pool.ID)
				numberOfDrivesCanBeAdded = numberOfDrivesCanBeAdded - 1
			}
		}
		for _, pool := range poolList {
			...
			if len(drvs) < (POOL_MAX_CLOUD_DRIVES - 1) {
				selectedPool = pool
				break
			}
		}

		err = addCloudDrive(stNodes[0], selectedPool.ID)
		dash.VerifyFatal(err != nil, true, "Error expected as drive added more than allowed per node")
		poolList, err = GetPoolsDetailsOnNode(stNodes[0])
		for _, pool := range poolList {
			if pool.ID != 0 {
				err = Inst().V.DeletePool(selectedNode, string(pool.ID), true)
				log.FailOnError(err, "error in deleting pool with error %s", err)
			}
		}
	})
