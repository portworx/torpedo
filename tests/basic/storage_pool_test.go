...
var _ = Describe("{NetworkBlockWhilePoolExpand}", func() {
	/*
	...
	*/
	JustBeforeEach(func() {
		StartTorpedoTest("NetworkBlockWhilePoolExpand", "Network block while pool expand", nil, 0)
	})

	var contexts []*scheduler.Context
	var selectedNode node.Node

	itLog := "NetworkBlockWhilePoolExpand"
	It(itLog, func() {
		log.InfoD(itLog)
		stepLog := "schedule Application"
		Step(stepLog, func() {
		...
		stepLog = "Block network with the cloud provider while pool expand is in progress"
		Step(stepLog, func() {
			log.InfoD(stepLog)
			...
			defer func() {
				...
				err := Inst().N.InjectNetworkErrorWithRebootFallback([]node.Node{selectedNode}, "drop", "del", 100, Delay)
			...
			err := Inst().N.InjectNetworkErrorWithRebootFallback([]node.Node{selectedNode}, "drop", "add", 0, Delay)
