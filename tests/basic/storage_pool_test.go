...
var _ = Describe("{PoolCreateNegative}", func() {
	...
	*/
	...
	It(ItLog, func() {
		log.InfoD(ItLog)
		err = AddCloudDriveWithParams(*selectedNode, -1, "--max-thin-pool-size-tb 600", false)
		dash.VerifyFatal(err != nil, true, fmt.Sprintf("Expect pool create to fail: got [%v]", err))
	})
	...
	It(ItLog, func() {
		log.InfoD(ItLog)
		err = AddCloudDriveWithParams(*selectedNode, 0, "--max-thin-pool-size-tb 20", false)
		dash.VerifyFatal(err != nil, true, fmt.Sprintf("Expect pool create to fail: got [%v]", err))
	})
...
var _ = Describe("{PoolCreateNegativeOnPrem}", func() {
	...
	*/
	...
	It(ItLog, func() {
		log.InfoD(ItLog)
		err = Inst().V.AddBlockDrives(selectedNode, []string{}, "--newpool --max-thin-pool-size-tb 700", true, 1)
		dash.VerifyFatal(err != nil, true, fmt.Sprintf("Expect pool create to fail: got [%v]", err))
	})
...
var _ = Describe("{PoolCreateDriveListOnPrem}", func() {
	...
	It(ItLog, func() {
		...
		err = Inst().V.AddBlockDrives(selectedNode, []string{}, "--newpool --max-thin-pool-size-tb 75", true, 2)
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("Expect pool create to success: got [%v]", err))
		time.Sleep(time.Second * 15)
		err = Inst().V.WaitDriverUpOnNode(*selectedNode, addDriveUpTimeOut)
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("volume drive down on node %s", selectedNode.Name))
...
var _ = Describe("{PoolCreateMaxThinPoolSizeOnPrem}", func() {
	...
	It(ItLog, func() {
		...
		err = Inst().V.AddBlockDrives(selectedNode, []string{}, "--newpool --max-thin-pool-size-tb 480", true, 1)
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("Expect pool create to success: got [%v]", err))
		time.Sleep(time.Second * 15)
		err = Inst().V.WaitDriverUpOnNode(*selectedNode, addDriveUpTimeOut)
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("volume drive down on node %s", selectedNode.Name))
...
var _ = Describe("{PoolCreateInvalidMaxThinPoolSize}", func() {
	...
	*/
	...
	It(ItLog, func() {
		log.InfoD(ItLog)
		err = AddCloudDriveWithParams(*selectedNode, -1, "--max-thin-pool-size-tb 240 --drives-count 1", false)
		dash.VerifyFatal(err != nil, true, fmt.Sprintf("Expect pool create to fail: got [%v]", err))
		time.Sleep(time.Second * 15)
		err = Inst().V.WaitDriverUpOnNode(*selectedNode, addDriveUpTimeOut)
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("volume drive down on node %s", selectedNode.Name))
...
var _ = Describe("{PoolCreateValidMaxThinPoolSize}", func() {
	...
	*/
	...
	It(ItLog, func() {
		...
		err = AddCloudDriveWithParams(*selectedNode, -1, "--max-thin-pool-size-tb 15", false)
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("Expect pool create to success: got [%v]", err))
		time.Sleep(time.Second * 15)
		err = Inst().V.WaitDriverUpOnNode(*selectedNode, addDriveUpTimeOut)
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("volume drive down on node %s", selectedNode.Name))
...
var _ = Describe("{PoolCreateWithMultipleParams}", func() {
	...
	*/
	...
	It(ItLog, func() {
		...
		err = AddCloudDriveWithParams(*selectedNode, -1, "--drives-count 2 --max-thin-pool-size-tb 30", false)
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("Expect pool create to success: got [%v]", err))
		time.Sleep(time.Second * 15)
		err = Inst().V.WaitDriverUpOnNode(*selectedNode, addDriveUpTimeOut)
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("volume drive down on node %s", selectedNode.Name))
...
var _ = Describe("{PoolCreateWithMultipleParamsFails}", func() {
	...
	*/
	...
	It(ItLog, func() {
		log.InfoD(ItLog)
		err = AddCloudDriveWithParams(*selectedNode, -1, "--drives-count 1 --max-thin-pool-size-tb 480", false)
		dash.VerifyFatal(err != nil, true, fmt.Sprintf("Expect pool create to fail: got [%v]", err))
	})
...
var _ = Describe("{PoolCreateMaxThinPoolSizePrecedence}", func() {
	...
	*/
	...
	It(ItLog, func() {
		...
		err = Inst().V.SetClusterRunTimeOpts(*selectedNode, map[string]string{
			"max_thin_pool_size_tb": "480",
		})
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("set cluster runtime opts fail: got [%v]", err))
		err = AddCloudDriveWithParams(*selectedNode, -1, "--drives-count 3 --max-thin-pool-size-tb 45", false)
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("Expect pool create to success: got [%v]", err))
		time.Sleep(time.Second * 15)
		err = Inst().V.WaitDriverUpOnNode(*selectedNode, addDriveUpTimeOut)
		dash.VerifyFatal(err == nil, true, fmt.Sprintf("volume drive down on node %s", selectedNode.Name))
