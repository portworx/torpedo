...
var _ = Describe("{PoolResizeWithPXRestartWithTimeInterval}", Label("p1", "hal_ops_disruption", "px_restart", "PoolExpand", "functional"), func() {
	...
	*/
	...
	JustBeforeEach(func() {
		StartTorpedoTest("PoolResizeWithPXRestartWithTimeInterval", "Pool resize with px restart with random delay", nil, 0)
	...
	itLog := "PoolResizeWithPXRestartWithTimeInterval"
	It(itLog, func() {
		...
		Step(stepLog, func() {
			...
			log.FailOnError(err, "failed to restart portworx on node [%v]", selectedNode.Name)
