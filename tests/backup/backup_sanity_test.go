	...
	It("Basic Backup Creation", func() {
		...
		Step("Taking backup of application from source cluster", func() {
			...
			for _, appCtx := range scheduledAppContexts {
				...
				err := CreateBackupWithCRValidation(backupName, SourceClusterName, backupLocationName, backupLocationUID, []string{scheduledNamespace}, labelSelectors, BackupOrgID, sourceClusterUid, "", "", "", "", ctx)
		...
		Step("Restoring the backed up namespaces", func() {
			...
			for i, appCtx := range scheduledAppContexts {
				...
				err = CreateRestoreWithCRValidation(restoreName, backupNames[i], make(map[string]string), DestinationClusterName, BackupOrgID, ctx, make(map[string]string))
