...
var _ = Describe("{BasicBackupCreationDummyTest}", func() {
	...
	It("Basic Backup Creation", func() {
		...
		Step(fmt.Sprintf("Creating schedule backup for multiple provisioner with forced kdmp option"), func() {
			...
			err = DeleteScheduleWithUIDAndWait(kdmpScheduleName, scheduleUid, SourceClusterName, sourceClusterUid, BackupOrgID, ctx)
			log.FailOnError(err, "Fetching px-central-admin")
		})

		Step("Taking backup of application from source cluster with vsc", func() {
		...
		Step("Taking backup of application from source cluster without vsc", func() {
			log.InfoD("taking backup of applications")
			ctx, err := backup.GetAdminCtxFromSecret()
			log.FailOnError(err, "Fetching px-central-admin ctx")
