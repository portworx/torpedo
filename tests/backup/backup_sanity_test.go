	...
	It("Basic Backup Creation", func() {
		...
		Step("Taking backup of application from source cluster", func() {
			...
			for i, appCtx := range scheduledAppContexts {
				...
				err := CreateBackupWithValidationWithVscMapping(ctx, backupName, SourceClusterName, backupLocationName, backupLocationUID, scheduledAppContexts[i:i+1], labelSelectors, BackupOrgID, sourceClusterUid, "", "", "", "", provisionerVolumeSnapshotClassMap)
