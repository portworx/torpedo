...
var _ = Describe("{ValidateForBackupWithPartialSuccess}", func() {
	...
	It("Validating app with multiple vols", func() {
		...
		Step("Fail backup for CSI", func() {
			log.InfoD("Fail backup for CSI")
			ctx, err := backup.GetAdminCtxFromSecret()
			log.FailOnError(err, "Fetching px-central-admin ctx")
			...
			log.Infof("The pvc list is %v AND COUNT IS %v", pvcList, len(pvcList.Items))
			//dash.VerifyFatal(err, nil, "failed to get pvcList")
			if len(pvcList.Items) == 0 {
				log.Fatalf("No PVCs found with the specified label selector")
			}
			singlePVC := pvcList.Items[0]
			scProvisioner, err = k8sCore.GetStorageProvisionerForPVC(&singlePVC)
			...
			log.Infof("The pvc name list is %v", pvcNames)
