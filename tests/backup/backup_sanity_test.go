...
// This testcase verifies if the backup installation is passing
var _ = Describe("{PxBackupInstallation}", func() {
	var (
		backupNames          []string
		restoreNames         []string
		scheduledAppContexts []*scheduler.Context
	...
	JustBeforeEach(func() {
		...
		monthlyName = fmt.Sprintf("%s-%v", "monthly", time.Now().Unix())
		log.Infof("No pre-setup required for this testcase")
		StartPxBackupTorpedoTest("PxBackupInstallation", "Installing Px Backup", nil, 0, Mkoppal, Q4FY25)
		log.InfoD("Time to delete px-backup namespace")
		err := UninstallPxBackup()
		log.FailOnError(err, "Failed to delete px-backup")
	})
	It("Installing Px Backup", func() {
		Step("Installing Px Backup", func() {
			log.InfoD("Check the status of backup pods")
			valuesMap, err := ParseValuesFromFile("values1")
			log.FailOnError(err, "Failed to parse values file")
			_, err = InstallPxBackup("2.6.0", "master", "backup-test", valuesMap)
