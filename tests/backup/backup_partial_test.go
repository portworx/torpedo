...
var _ = Describe("{PartialBackupSuccessWithAzureEndpoint}", func() {
	...
	var (
		...
		backupLocationMap          = make(map[string]string)
		labelSelectors             = make(map[string]string)
		provisionerVscMap          = make(map[string]string)
	...
	It("verifies the partial backup success when few Px volumes backups failed while taking backup to Azure Global Location when env variable is set to non-global location", func() {
		...
		Step("Fetch the provisioner and volume snapshot class map for each namespaces to be backed up", func() {
			log.InfoD("Fetch the provisioner and volume snapshot class map for each namespaces to be backed up")
			for _, appContext := range scheduledAppContexts {
				ProvisionerToSnapshotClass, err := FindProvisionerToSnapshotClassByNamespace(appContext.ScheduleOptions.Namespace)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Fetching provisioner to volumesnapshot class map for namespace [%s]", appContext.ScheduleOptions.Namespace))
				for provisioner, snapshotClass := range ProvisionerToSnapshotClass {
					provisionerVscMap[provisioner] = snapshotClass
				}
			}
		})
