		...
		Step("Creating a locked bucket and backup location", func() {
			...
			for _, provider := range providers {
				for _, mode := range modes {
					...
					backupLocation = fmt.Sprintf("%s-%s-%s-lock", getGlobalLockedBucketName(provider), strings.ToLower(mode), RandomString(5))
			...
			Step(fmt.Sprintf("Create cloud credential and locked bucket backup location from the user %s", user), func() {
				...
				for _, provider := range providers {
					...
					for _, mode := range modes {
						...
						lockedBucketName := fmt.Sprintf("%s-%s-%s-%s-locked", provider, getGlobalLockedBucketName(provider), strings.ToLower(mode), RandomString(5))
