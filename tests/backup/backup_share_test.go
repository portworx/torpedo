...
// DeleteUserBackupsAndRestoresOfDeletedClusterFromAdmin deletes user backups and restores of the deleted cluster from the admin
var _ = Describe("{DeleteUserBackupsAndRestoresOfDeletedClusterFromAdmin}", func() {
	...
	JustBeforeEach(func() {
		StartTorpedoTest("DeleteUserBackupsAndRestoresOfDeletedClusterFromAdmin", "Delete user backups and restores of the deleted cluster from the admin", nil, 87560)
		for i := 0; i < Inst().GlobalScaleFactor; i++ {
			taskName := fmt.Sprintf("%s-%d", taskNamePrefix, i)
			appContexts := ScheduleApplications(taskName)
			for _, ctx := range appContexts {
	...
	It("Deletes user backups and restores of the deleted cluster from the admin", func() {
		Step("Validate applications", func() {
			log.InfoD("Validating applications")
			ValidateApplications(scheduledAppContexts)
		})
		...
		Step(fmt.Sprintf("Delete source and destination cluster from the user %s", infraAdminUser), func() {
			log.InfoD(fmt.Sprintf("Deleting source and destination cluster from the user %s", infraAdminUser))
			nonAdminCtx, err := backup.GetNonAdminCtx(infraAdminUser, commonPassword)
			log.FailOnError(err, "failed to fetch user %s ctx", infraAdminUser)
