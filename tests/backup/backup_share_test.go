...
var _ = Describe("{SwapShareBackup}", func() {
	...
	var backupName string
	var userBackupLocationName string
	var userBackupLocationUID string
	var scheduledAppContexts []*scheduler.Context
	var bkpNamespaces []string
	var clusterUid string
	var clusterStatus api.ClusterInfo_StatusInfo_Status
		...
		Step(fmt.Sprintf("Adding Credentials and Registering Backup Location for %s and %s", users[0], users[1]), func() {
			...
			for _, provider := range providers {
				for _, user := range users {
					...
					err = CreateBackupLocationWithContext(provider, userBackupLocationName, userBackupLocationUID, credName, cloudCredUID, userBucketName, BackupOrgID, "", ctxNonAdmin, true)
					log.FailOnError(err, "Failed to add backup location %s using provider %s to user - %s", userBackupLocationName, provider, user)
					userBackupLocationMapping[user] = userBackupLocationName
					userBackupLocationUidMapping[user] = userBackupLocationUID
