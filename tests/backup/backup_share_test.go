...
// DeleteUserBackupsAndRestoresOfInActiveClusterFromAdmin deletes user backups and restores of the inactive cluster from the admin
var _ = Describe("{DeleteUserBackupsAndRestoresOfInActiveClusterFromAdmin}", func() {
	...
	var (
		...
		userCloudCredentialName    string
		userCloudCredentialUID     = uuid.New()
		userBackupLocationName     string
		userBackupLocationUID      = uuid.New()
		userBackupLocationMap      = make(map[string]string)
		userSourceClusterUID       string
		providers                  = getProviders()
		userBackupMap              = make(map[string]string)
		userSchedulePolicyName     string
		userSchedulePolicyUID      string
		userSchedulePolicyInterval = int64(15)
		userScheduleName           string
		userRestoreNames           = make([]string, 0)
		numberOfBackups            = 2
		invalidKubeconfig          = "\"\""
	)

	JustBeforeEach(func() {
		StartTorpedoTest("DeleteUserBackupsAndRestoresOfInActiveClusterFromAdmin", "Delete user backups and restores of the inactive cluster from the admin", nil, 87560)
		for i := 0; i < Inst().GlobalScaleFactor; i++ {
			taskName := fmt.Sprintf("%s-%d", taskNamePrefix, i)
			appContexts := ScheduleApplications(taskName)
			for _, ctx := range appContexts {
	...
	It("Deletes user backups and restores of the inactive cluster from the admin", func() {
		Step("Validate applications", func() {
			log.InfoD("Validating applications")
			ValidateApplications(scheduledAppContexts)
		})
		Step(fmt.Sprintf("Create user with %s role", infraAdminRole), func() {
			log.InfoD(fmt.Sprintf("Creating user with %s role", infraAdminRole))
		...
		Step(fmt.Sprintf("Create cloud credential and backup location from the user %s", infraAdminUser), func() {
			log.InfoD(fmt.Sprintf("Creating cloud credential and backup location from the user %s", infraAdminUser))
		...
		Step(fmt.Sprintf("Take backup of applications from the user %s", infraAdminUser), func() {
			log.InfoD(fmt.Sprintf("Taking backup of applications from the user %s", infraAdminUser))
			nonAdminCtx, err := backup.GetNonAdminCtx(infraAdminUser, commonPassword)
			log.FailOnError(err, "failed to fetch user %s ctx", infraAdminUser)
			var wg sync.WaitGroup
			var mu sync.RWMutex
			createBackup := func(backupName string, namespace string) {
				...
				defer wg.Done()
				appContextsToBackup := FilterAppContextsByNamespace(scheduledAppContexts, []string{namespace})
				err := CreateBackupWithValidation(nonAdminCtx, backupName, SourceClusterName, userBackupLocationName, userBackupLocationUID, appContextsToBackup, make(map[string]string), orgID, userSourceClusterUID, "", "", "", "")
				...
				mu.Lock()
				defer mu.Unlock()
				userBackupMap[backupName] = namespace
			}
		...
		Step(fmt.Sprintf("Create schedule policy from the user %s", infraAdminUser), func() {
			log.InfoD(fmt.Sprintf("Creating schedule policy from the user %s", infraAdminUser))
			nonAdminCtx, err := backup.GetNonAdminCtx(infraAdminUser, commonPassword)
			log.FailOnError(err, "failed to fetch user %s ctx", infraAdminUser)
			...
			log.FailOnError(err, "failed to fetch schedule policy uid %s", userSchedulePolicyName)
		})
		Step(fmt.Sprintf("Take schedule backup of applications from the user %s", infraAdminUser), func() {
			log.InfoD(fmt.Sprintf("Taking schedule backup of applications from the user %s", infraAdminUser))
			nonAdminCtx, err := backup.GetNonAdminCtx(infraAdminUser, commonPassword)
			log.FailOnError(err, "failed to fetch user %s ctx", infraAdminUser)
			userScheduleName = fmt.Sprintf("backup-schedule-%v", time.Now().Unix())
			_, err = CreateScheduleBackupWithValidation(nonAdminCtx, userScheduleName, SourceClusterName, userBackupLocationName, userBackupLocationUID, scheduledAppContexts, make(map[string]string), orgID, "", "", "", "", userSchedulePolicyName, userSchedulePolicyUID)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation and validation of schedule backup with schedule name [%s]", userSchedulePolicyName))
		})
		Step(fmt.Sprintf("Take restore of backups from the user %s", infraAdminUser), func() {
			log.InfoD(fmt.Sprintf("Taking restore of backups from the user %s", infraAdminUser))
			nonAdminCtx, err := backup.GetNonAdminCtx(infraAdminUser, commonPassword)
			log.FailOnError(err, "failed to fetch user %s ctx", infraAdminUser)
			var wg sync.WaitGroup
			var mu sync.RWMutex
			createRestore := func(backupName string, restoreName string, namespace string) {
				...
				mu.Lock()
				defer mu.Unlock()
				userRestoreNames = append(userRestoreNames, restoreName)
			}
			for backupName, namespace := range userBackupMap {
				wg.Add(1)
				restoreName := fmt.Sprintf("%s-%s", restoreNamePrefix, backupName)
				go createRestore(backupName, restoreName, namespace)
			}
			wg.Wait()
			log.Infof("The list of user restores taken are: %v", userRestoreNames)
		})
		Step(fmt.Sprintf("Make source and destination cluster inactive from the user %s", infraAdminUser), func() {
			log.InfoD(fmt.Sprintf("Making source and destination cluster inactive from the user %s", infraAdminUser))
			nonAdminCtx, err := backup.GetNonAdminCtx(infraAdminUser, commonPassword)
			log.FailOnError(err, "failed to fetch user %s ctx", infraAdminUser)
		...
		Step(fmt.Sprintf("Delete user %s backups from the admin", infraAdminUser), func() {
			log.InfoD(fmt.Sprintf("Deleting user %s backups from the admin", infraAdminUser))
			ctx, err := backup.GetAdminCtxFromSecret()
			log.FailOnError(err, "Fetching px-central-admin ctx")
			for backupName := range userBackupMap {
				backupUid, err := Inst().Backup.GetBackupUID(ctx, backupName, orgID)
		...
		Step(fmt.Sprintf("Delete user %s schedule backups, backup schedule and schedule policy from the admin", infraAdminUser), func() {
			log.InfoD(fmt.Sprintf("Deleting user %s schedule backups, backup schedule and schedule policy from the admin", infraAdminUser))
			nonAdminCtx, err := backup.GetNonAdminCtx(infraAdminUser, commonPassword)
			log.FailOnError(err, "failed to fetch user %s ctx", infraAdminUser)
			allScheduleBackupNames, err := Inst().Backup.GetAllScheduleBackupNames(nonAdminCtx, userScheduleName, orgID)
			log.FailOnError(err, "failed to get all schedule backup names with schedule name %s", userScheduleName)
			ctx, err := backup.GetAdminCtxFromSecret()
			log.FailOnError(err, "Fetching px-central-admin ctx")
			for i := len(allScheduleBackupNames) - 1; i >= 0; i-- {
				backupName := allScheduleBackupNames[0]
				backupUid, err := Inst().Backup.GetBackupUID(ctx, backupName, orgID)
				log.FailOnError(err, "failed to fetch schedule backup %s uid", backupName)
				_, err = DeleteBackup(backupName, backupUid, orgID, ctx)
				log.FailOnError(err, "failed to delete schedule backup %s", backupName)
			}
			err = DeleteSchedule(userScheduleName, SourceClusterName, orgID, ctx)
			log.FailOnError(err, "failed to delete schedule %s", userScheduleName)
			err = Inst().Backup.DeleteBackupSchedulePolicy(orgID, []string{userSchedulePolicyName})
			log.FailOnError(err, "failed to delete schedule policy %s", userSchedulePolicyName)
		})
		Step(fmt.Sprintf("Delete user %s restores from the admin", infraAdminUser), func() {
			log.InfoD(fmt.Sprintf("Deleting user %s restores from the admin", infraAdminUser))
			ctx, err := backup.GetAdminCtxFromSecret()
			log.FailOnError(err, "Fetching px-central-admin ctx")
	...
	JustAfterEach(func() {
		...
		opts[SkipClusterScopedObjects] = true
		DestroyApps(scheduledAppContexts, opts)
		CleanupCloudSettingsAndClusters(userBackupLocationMap, userCloudCredentialName, userCloudCredentialUID, nonAdminCtx)
	})
})
