...
// VerifyRBACOperationForAppUser Verify RBAC operation for app-user
var _ = Describe("{VerifyRBACOperationForAppUser}", func() {
	...
	JustBeforeEach(func() {
		StartTorpedoTest("CreateUserShareRBAC",
			"Share RBAC object from adminn to user", nil, 82940)
		log.InfoD("Deploy applications")
		scheduledAppContexts = make([]*scheduler.Context, 0)
		for i := 0; i < Inst().GlobalScaleFactor; i++ {
			taskName := fmt.Sprintf("%s-%d", taskNamePrefix, i)
	...
	It("Share the backup with same name", func() {
		...
		Step("Create Users", func() {
			userName := fmt.Sprintf("testautouser%v", time.Now().Unix())
			firstName := fmt.Sprintf("test-app.user")
			lastName := fmt.Sprintf("1")
			email := fmt.Sprintf("testuser%v@cnbu.com", time.Now().Unix())
			err := backup.AddUser(userName, firstName, lastName, email, commonPassword)
			log.FailOnError(err, "Failed to create user - %s", userName)
		...
		Step(fmt.Sprintf("Adding Credentials and Registering Backup Location for admin"), func() {
			log.InfoD(fmt.Sprintf("Adding Credentials and Registering Backup Location for admin"))
			for _, provider := range providers {
				cloudCredUID = uuid.New()
				backupLocationUID = uuid.New()
				credName = fmt.Sprintf("test-cloud-credential-%v", time.Now().Unix())
				ctx, err := backup.GetAdminCtxFromSecret()
				log.FailOnError(err, "Fetching non admin ctx")
				err = CreateCloudCredential(provider, credName, cloudCredUID, orgID, ctx)
				log.FailOnError(err, "Failed to create cloud credential - %s", err)
				backupLocationName = fmt.Sprintf("autogenerated-backup-location-%v", time.Now().Unix())
				err = CreateBackupLocation(provider, backupLocationName, backupLocationUID, credName, cloudCredUID, getGlobalBucketName(provider), orgID, "")
				log.FailOnError(err, "Failed to add backup location")
			}
		})
		...
		Step("share RBAC object with created user", func() {
			ctx, err := backup.GetAdminCtxFromSecret()
			log.FailOnError(err, "Fetching px-central-admin ctx")

			log.InfoD("update ownership for cloud credential - %s", credName)
			err = UpdateCloudCredentialOwnership(credName, cloudCredUID, userNames, nil, Read, Invalid, ctx, orgID)
			log.FailOnError(err, "updating cloud credential for user")

			log.InfoD("update ownership for backup location - %s", backupLocationName)
			err = UpdateBackupLocationOwnership(backupLocationName, backupLocationUID, userNames, nil, Read, Invalid, ctx)
			log.FailOnError(err, "updating backup location for user")

			log.InfoD("update ownership for schedule policy - %s", periodicSchedulePolicyName)
			err = UpdateSchedulePolicyOwnership(periodicSchedulePolicyName, periodicSchedulePolicyUid, userNames, nil, Read, Invalid, ctx)
			log.FailOnError(err, "updating schedule policy for user")

			log.InfoD("update ownership for pre-rule - %s", preRuleName)
			err = UpdateRuleOwnership(preRuleName, preRuleUid, userNames, nil, Read, Invalid, ctx)
			log.FailOnError(err, "updating pre-rule for user")

			log.Infof("succesfully updated rbac objects owernship, waiting for 60 seconds")
			time.Sleep(60 * time.Second)
		})

		Step("Create backup with the non-admin ctx", func() {
			for _, user := range userNames {
				Step(fmt.Sprintf("Register source and destination cluster for backup on %s ", user), func() {
					log.InfoD("Registering Source and Destination clusters as user : %s and verifying the status", user)
					ctx, err := backup.GetNonAdminCtx(user, commonPassword)
				...
				Step("Create schedule backup from the non-admin context", func() {
					for _, namespace := range bkpNamespaces {
						log.InfoD("Create schedule backup after initializing")
						ctxNonAdmin, err := backup.GetNonAdminCtx(user, commonPassword)
						log.FailOnError(err, "Unable to px-central-admin ctx")
						//preRuleUid, err := Inst().Backup.GetRuleUid(orgID, ctx, preRuleName)
						log.FailOnError(err, "Unable to fetch pre rule Uid")
						postRuleUid, err := Inst().Backup.GetRuleUid(orgID, ctxNonAdmin, postRuleName)
						log.FailOnError(err, "Unable to fetch post rule Uid")
						scheduleName = fmt.Sprintf("%s-schedule-%v", BackupNamePrefix, time.Now().Unix())
						appContextsToBackup := FilterAppContextsByNamespace(scheduledAppContexts, []string{namespace})
						_, err = CreateScheduleBackupWithValidation(ctxNonAdmin, scheduleName, SourceClusterName, backupLocationName, backupLocationUID, appContextsToBackup, make(map[string]string), orgID, preRuleName, preRuleUid, postRuleName, postRuleUid, periodicSchedulePolicyName, periodicSchedulePolicyUid)
						dash.VerifyFatal(err, nil, fmt.Sprintf("Creation and Validation of schedule backup with schedule name [%s]", scheduleName))
						scheduleNames = append(scheduleNames, scheduleName)
					}
				})
		...
		Step(fmt.Sprintf("Restore the backup %s from the non-admin context", userNames[0]), func() {
			for _, user := range userNames {
				log.InfoD(fmt.Sprintf("Restore the shared backup  %s with user context %s", backupName, user))
				ctxNonAdmin, err := backup.GetNonAdminCtx(user, commonPassword)
				log.FailOnError(err, "Fetching non admin ctx")
				restoreName := fmt.Sprintf("%s-%v", RestoreNamePrefix, time.Now().Unix())
				err = CreateRestoreWithUID(restoreName, backupName, nil, destinationClusterName, orgID, ctxNonAdmin, nil, backupUIDList[1])
