...
var _ = Describe("{DeleteFailedInProgressBackupAndRestoreOfUserFromAdmin}", func() {
	...
	It("Delete failed and in-progress backups and restores of user from the admin", func() {
		...
		//		defer GinkgoRecover()
		//		defer wg.Done()
		//		_, err = CreateBackupByNamespacesWithoutCheck(backupName, SourceClusterName, userBackupLocationName, userBackupLocationUID,
		...
		//})
		//Step(fmt.Sprintf("Delete failed taken by user %s from the admin", infraAdminUser), func() {
		//	log.InfoD(fmt.Sprintf("Deleting failed taken by user %s from the admin", infraAdminUser))
		//	ctx, err := backup.GetAdminCtxFromSecret()
		//	log.FailOnError(err, "Fetching px-central-admin ctx")
		...
		//	ctx, err := backup.GetNonAdminCtx(masterUser, commonPassword)
		//	log.FailOnError(err, "Fetching non-admin ctx")
	...
	JustAfterEach(func() {
		...
		////Delete Schedule Backup-
		//log.Infof("Deleting backup schedule")
		//err = DeleteSchedule(userBackupScheduleName, SourceClusterName, orgID, ctx)
		//dash.VerifySafely(err, nil, fmt.Sprintf("Verifying the deletion of  the backup schedule - %s", userBackupScheduleName))
		//log.Infof("Deleting backup schedule policy")
		//err = Inst().Backup.DeleteBackupSchedulePolicy(orgID, []string{userSchedulePolicyName})
		//dash.VerifySafely(err, nil, fmt.Sprintf("Deleting backup schedule policies %s ", []string{userSchedulePolicyName}))
		//for _, user := range users {
		//	err = backup.DeleteUser(user)
		//	dash.VerifySafely(err, nil, fmt.Sprintf("Verifying the deletion of the user [%s]", user))
		//}
		//CleanupCloudSettingsAndClusters(userBackupLocationMap, userCloudCredentialName, userCloudCredentialUID, ctx)
	})
})
