	...
	It("Validate shared backup is not duplicated without cluster", func() {
		...
		Step("Taking backup of applications", func() {
			...
			err := CreateBackup(backupName, SourceClusterName, backupLocationName, backupLocationUID, []string{bkpNamespaces[0]},
				nil, orgID, clusterUid, "", "", "", "", PxBackupAdminContext)
		...
		Step("Share backup with user", func() {
			...
			err := ShareBackup(backupName, nil, []string{userName}, FullAccess, PxBackupAdminContext)
...
var _ = Describe("{DifferentAccessSameUser}", func() {
	...
	It("Different Access Same User", func() {
		...
		Step("Share backup with user having viewonly access", func() {
			...
			err = ShareBackup(backupName, nil, userNames, ViewOnlyAccess, PxBackupAdminContext)
			log.FailOnError(err, "Failed to share backup %s", backupName)
		})
		Step("Share backup with group having full access", func() {
			log.InfoD("Share backup with group having full access")
			err = ShareBackup(backupName, []string{groupName}, nil, FullAccess, PxBackupAdminContext)
			log.FailOnError(err, "Failed to share backup %s", backupName)
		})
	...
	It("Share large number of backups", func() {
		...
		Step("Sharing backup with groups", func() {
			...
			for i, backupName := range backupNames {
				...
				switch groupIndex {
				case 0:
					err = ShareBackup(backupName, []string{groups[groupIndex]}, nil, ViewOnlyAccess, PxBackupAdminContext)
					log.FailOnError(err, "Failed to share backup %s", backupName)
				case 1:
					err = ShareBackup(backupName, []string{groups[groupIndex]}, nil, RestoreAccess, PxBackupAdminContext)
					log.FailOnError(err, "Failed to share backup %s", backupName)
				case 2:
					err = ShareBackup(backupName, []string{groups[groupIndex]}, nil, FullAccess, PxBackupAdminContext)
					log.FailOnError(err, "Failed to share backup %s", backupName)
				default:
					err = ShareBackup(backupName, []string{groups[0]}, nil, ViewOnlyAccess, PxBackupAdminContext)
					log.FailOnError(err, "Failed to share backup %s", backupName)
				}
			}
		})
		...
		Step("Share Backup with Full access to a user of View Only access group and Validate", func() {
			...
			err = ShareBackup(backupName, nil, []string{chosenUser}, FullAccess, PxBackupAdminContext)
			log.FailOnError(err, "Failed to share backup %s", backupName)
		...
		Step("Share Backup with View Only access to a user of Full access group and Validate", func() {
			...
			err = ShareBackup(backupName, nil, []string{username}, ViewOnlyAccess, PxBackupAdminContext)
			log.FailOnError(err, "Failed to share backup %s", backupName)
		...
		Step("Share Backup with Restore access to a user of View Only access group and Validate", func() {
			...
			err = ShareBackup(backupName, nil, []string{username}, RestoreAccess, PxBackupAdminContext)
			log.FailOnError(err, "Failed to share backup %s", backupName)
...
var _ = Describe("{ShareLargeNumberOfBackupsWithLargeNumberOfUsers}", func() {
	...
	It("Share all backups at cluster level with a user group and revoke it and validate", func() {
		...
		Step("Share all backups with Full Access in source cluster with a group", func() {
			...
			err = ClusterUpdateBackupShare(SourceClusterName, groups, nil, FullAccess, true, PxBackupAdminContext)
		...
		Step("Share all backups with Restore Access in source cluster with a group", func() {
			log.InfoD("Share all backups with Full Access in source cluster with a group")
			err = ClusterUpdateBackupShare(SourceClusterName, groups, nil, RestoreAccess, true, PxBackupAdminContext)
			log.FailOnError(err, "Failed sharing all backups for cluster [%s]", SourceClusterName)
		})
		...
		Step("Share all backups with View Only Access in source cluster with a group", func() {
			log.InfoD("Share all backups with Full Access in source cluster with a group")
			err = ClusterUpdateBackupShare(SourceClusterName, groups, nil, ViewOnlyAccess, true, PxBackupAdminContext)
			log.FailOnError(err, "Failed sharing all backups for cluster [%s]", SourceClusterName)
		})
	...
	JustAfterEach(func() {
		...
		err := DeleteCluster(SourceClusterName, orgID, PxBackupAdminContext)
		dash.VerifySafely(err, nil, fmt.Sprintf("Deleting cluster %s", SourceClusterName))
		err = DeleteCluster(destinationClusterName, orgID, PxBackupAdminContext)
...
var _ = Describe("{CancelClusterBackupShare}", func() {
	...
	It("Share all backups at cluster level with a user group and revoke it and validate", func() {
		...
		Step("Share all backups with Full Access in source cluster with a group and a user who is not part of the group", func() {
			...
			err = ClusterUpdateBackupShare(SourceClusterName, []string{groups[0]}, []string{individualUser}, FullAccess, true, PxBackupAdminContext)
			log.FailOnError(err, "Failed sharing all backups for cluster [%s]", SourceClusterName)
		})
		...
		Step("Share all backups with Restore Access in source cluster with a group and a user who is not part of the group", func() {
			...
			err = ClusterUpdateBackupShare(SourceClusterName, []string{groups[0]}, []string{"autogenerated-user"}, RestoreAccess, true, PxBackupAdminContext)
			log.FailOnError(err, "Failed sharing all backups for cluster [%s]", SourceClusterName)
		})
		...
		Step("Share all backups with View Only Access in source cluster with a group and a user who is not part of the group", func() {
			...
			err = ClusterUpdateBackupShare(SourceClusterName, []string{groups[0]}, []string{individualUser}, ViewOnlyAccess, true, PxBackupAdminContext)
			log.FailOnError(err, "Failed sharing all backups for cluster [%s]", SourceClusterName)
		})
		...
		Step("Revoke all the shared backups in source cluster", func() {
			...
			err = ClusterUpdateBackupShare(SourceClusterName, []string{groups[0]}, []string{individualUser}, ViewOnlyAccess, false, PxBackupAdminContext)
			log.FailOnError(err, "Failed sharing all backups for cluster [%s]", SourceClusterName)
		})
	...
	It("Share the backup and edit", func() {
		...
		Step("Adding Credentials and Registering Backup Location", func() {
			...
			for _, provider := range providers {
				...
				err = CreateCloudCredential(provider, newCredName, newCloudCredUID, orgID, PxBackupAdminContext)
		...
		Step("Share backup with user restore mode and validate", func() {
			...
			// Share backup with the user
			err = ShareBackup(backupNames[0], nil, []string{users[0]}, RestoreAccess, PxBackupAdminContext)
			...
			backupDriver := Inst().Backup
			backupUID, err := backupDriver.GetBackupUID(PxBackupAdminContext, backupNames[0], orgID)
			log.FailOnError(err, "Failed while trying to get backup UID for - %s", backupNames[0])
			status, err := UpdateBackup(backupNames[0], backupUID, orgID, newCredName, newCloudCredUID, PxBackupAdminContext)
		...
		Step("Share backup with user restore mode and validate", func() {
			...
			// Share backup with the user
			err = ShareBackup(backupNames[0], nil, []string{users[1]}, FullAccess, PxBackupAdminContext)
			log.FailOnError(err, "Failed to share backup %s", backupNames[0])
	...
	It("Share the backups and delete", func() {
		...
		Step("Share backup with multiple users", func() {
			...
			for _, backup := range backupNames {
				err = ShareBackup(backup, nil, users, ViewOnlyAccess, PxBackupAdminContext)
			...
			for _, user := range users {
				...
				for _, backup := range backupNames {
					// Get Backup UID
					backupDriver := Inst().Backup
					backupUID, err := backupDriver.GetBackupUID(PxBackupAdminContext, backup, orgID)
		...
		Step("Delete the backups and validate", func() {
			...
			for _, backup := range backupNames {
				...
				go func(backup string) {
					defer GinkgoRecover()
					defer wg.Done()
					_, err = DeleteBackup(backup, backupMap[backup], orgID, PxBackupAdminContext)
					log.FailOnError(err, "Failed to delete backup - %s", backup)
					err = backupDriver.WaitForBackupDeletion(PxBackupAdminContext, backup, orgID, time.Minute*30, time.Minute*1)
...
var _ = Describe("{ClusterBackupShareToggle}", func() {
	...
	It("Validates that the user is able to perform operations on a shared backup after toggling the access", func() {
		...
		Step("Configure source and destination clusters with px-central-admin and user ctx", func() {
			...
			clusterStatus, err := Inst().Backup.GetClusterStatus(orgID, backupClusterName, PxBackupAdminContext)
			...
			clusterUid, err := Inst().Backup.GetClusterUID(PxBackupAdminContext, orgID, backupClusterName)
		...
		Step("Validate the Access toggle", func() {
			...
			for _, accessLevel := range accesses {
				...
				err := ClusterUpdateBackupShare(backupClusterName, nil, []string{username}, accessLevel, true, PxBackupAdminContext)
				...
				err = backupSuccessCheck(recentBackupName, orgID, maxWaitPeriodForBackupCompletionInMinutes*time.Minute, 30*time.Second, PxBackupAdminContext)
...
var _ = Describe("{ShareBackupsAndClusterWithUser}", func() {
	...
	It("Share Backup With Full Access Users and try to duplicate the backup", func() {
		...
		Step("Share backup with user having full access", func() {
			log.InfoD("Share backup with user having full access")
			err = ShareBackup(backupName, nil, userNames, FullAccess, PxBackupAdminContext)
			log.FailOnError(err, "Failed to share backup %s", backupName)
		})
...
var _ = Describe("{DeleteSharedBackup}", func() {
	...
	It("Validate shared backups are deleted from owner of backup ", func() {
		...
		Step("Share backup with user", func() {
			...
			for _, backup := range backupNames {
				err = ShareBackup(backup, nil, []string{userName}, FullAccess, PxBackupAdminContext)
				log.FailOnError(err, "Failed to share backup %s", backup)
...
var _ = Describe("{ShareAndRemoveBackupLocation}", func() {
	...
	It("Share and remove backup location and add it back and check from other users if they show up", func() {
		...
		Step("Register source and destination cluster for backup", func() {
			...
			srcClusterUid, err = Inst().Backup.GetClusterUID(PxBackupAdminContext, orgID, SourceClusterName)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Fetching [%s] cluster uid", SourceClusterName))
		...
		Step("Taking backups of application for each user", func() {
			...
			for i := 0; i < numberOfUsers; i++ {
				...
				go func(backupName string) {
					...
					defer func() { <-sem }()
					err = CreateBackup(backupName, SourceClusterName, bkpLocationName, backupLocationUID, []string{bkpNamespaces[0]},
						labelSelectors, orgID, srcClusterUid, "", "", "", "", PxBackupAdminContext)
		...
		Step("Share backup with users with different access level", func() {
			...
			_, err = ShareBackupWithUsersAndAccessAssignment(backupNames, users, PxBackupAdminContext)
		...
		Step("Removing backup location after sharing backup with all the users", func() {
			...
			backupLocationDeleteStatusCheck := func() (interface{}, bool, error) {
				status, err := IsBackupLocationPresent(bkpLocationName, PxBackupAdminContext, orgID)
				if err != nil {
		...
		Step("Share backup with users again with different access level", func() {
			...
			accessUserBackupContext, err = ShareBackupWithUsersAndAccessAssignment(newBackupNames, users, PxBackupAdminContext)
	...
	JustAfterEach(func() {
		...
		wg.Wait()
		CleanupCloudSettingsAndClusters(newBackupLocationMap, cloudCredName, cloudCredUID, PxBackupAdminContext)
...
var _ = Describe("{ViewOnlyFullBackupRestoreIncrementalBackup}", func() {
	...
	It("Full backup view only and incremental backup restore access", func() {
		...
		Step(fmt.Sprintf("Sharing full backup with view only access and incremental backup with full access with user [%s]", individualUser), func() {
			...
			err = ShareBackup(fullBackupName, nil, []string{individualUser}, ViewOnlyAccess, PxBackupAdminContext)
			err = ShareBackup(incrementalBackupName, nil, []string{individualUser}, FullAccess, PxBackupAdminContext)
		})
		...
		Step("Validate that user with View Only access cannot restore or delete the backup", func() {
			...
			// Get Backup UID
			backupDriver := Inst().Backup
			backupUID, err := backupDriver.GetBackupUID(PxBackupAdminContext, fullBackupName, orgID)
		...
		Step("Validate that user with View Only access on full backup and full access to incremental backup can restore", func() {
			...
			// Get Backup UID
			backupDriver := Inst().Backup
			backupUID, err := backupDriver.GetBackupUID(PxBackupAdminContext, incrementalBackupName, orgID)
...
var _ = Describe("{IssueMultipleRestoresWithNamespaceAndStorageClassMapping}", func() {
	...
	It("Issue multiple restores with namespace and storage class mapping", func() {
		...
		Step("Share backup with user with FullAccess", func() {
			...
			err = ShareBackup(backupName, nil, userName, FullAccess, PxBackupAdminContext)
		...
		Step("Share backup with user with RestoreAccess", func() {
			err = ShareBackup(backupName, nil, userName, RestoreAccess, PxBackupAdminContext)
...
var _ = Describe("{IssueMultipleDeletesForSharedBackup}", func() {
	...
	It("Share the backups and delete", func() {
		...
		Step("Share backup with multiple users", func() {
			...
			// Share backups with all the users
			err = ShareBackup(backupName, nil, users, FullAccess, PxBackupAdminContext)
			log.FailOnError(err, "Failed to share backup %s", backupName)
		...
		Step("Delete the backups and validate", func() {
			...
			for _, user := range users {
				...
				go func(user string) {
					...
					err = backupDriver.WaitForBackupDeletion(PxBackupAdminContext, backupName, orgID, time.Minute*10, time.Minute*1)
