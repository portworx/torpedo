...
var _ = Describe("{VerfiyRBACforAppUser}", func() {
	...
	It("To verify all the RBAC operations for an app-user", func() {
		...
		Step(fmt.Sprintf("Verify if the app-user %s can create cloud credentials and backup location", appUser), func() {
			log.InfoD(fmt.Sprintf("Creating cloud credentials and backup location for %s", appUser))
			ctxNonAdmin, err := backup.GetNonAdminCtx(appUser, commonPassword)
			log.FailOnError(err, "Fetching non admin ctx")
			for _, provider := range providers {
				cloudCredUID = uuid.New()
				backupLocationUID = uuid.New()
				credName := fmt.Sprintf("autogenerated-cred-%v", RandomString(6))
				...
				backupLocationName := fmt.Sprintf("autogenerated-backup-location-%v", RandomString(4))
		...
		Step(fmt.Sprintf("Verify if App-User has permission to create a schedule policy"), func() {
			log.InfoD("Verify if App-User has permission to create a schedule policy")
		...
		Step(fmt.Sprintf("Verfiy if App-User has permission to create pre and post exec rules for applications"), func() {
			log.InfoD("Verify if App-User has permission to create pre and post exec rules for applications")
			nonAdminCtx, err := backup.GetNonAdminCtx(appUser, commonPassword)
			log.FailOnError(err, "failed to fetch user %s ctx", appUser)
			preRuleName, postRuleName, err := CreateRuleForBackupWithMultipleApplications(orgID, Inst().AppList, nonAdminCtx)
			log.InfoD(preRuleName, postRuleName, err.Error())
		...
		Step(fmt.Sprintf("Create backup objects by px-admin"), func() {
			log.InfoD("Create backup objects by px-admin")
		})
		Step(fmt.Sprintf("Share backup objects created by px-admin to app-user"), func() {
			log.InfoD("Share backup objects created by px-admin to app-user")
		})
