...
// Verify PxRestore size from metrics
var _ = Describe("{VerifyPxRestoreSize}", Label(TestCaseLabelsMap[ValidateMetrics]...), func() {
	var (
		contexts             []*scheduler.Context
		pxbNamespace         string
		scheduledAppContexts []*scheduler.Context
	...
	JustBeforeEach(func() {
		StartPxBackupTorpedoTest("VerifyPxRestoreSize", "Verify px-restore size from metrics.", nil, 91956, Prikumar, Q2FY25)
		scheduledAppContexts = make([]*scheduler.Context, 0)
		bkpNamespaces = make([]string, 0)
	...
	// Validate px-restore total size from metrics
	It("Validate px-restore size from metrics", func() {
		ctx, err := backup.GetAdminCtxFromSecret()
		log.FailOnError(err, "Fetching admin ctx")
		numOfBackup := 1
		restoreCount := 2
		...
		// Fetch total restore size from cluster
		Step("Get total restore size volumes from cluster", func() {
			restoreEnumerateRequest := &api.RestoreEnumerateRequest{
				OrgId: BackupOrgID,
			}
			restoreResponse, err := Inst().Backup.EnumerateRestore(ctx, restoreEnumerateRequest)
			...
			for i, restoreObj := range restoreResponse.GetRestores() {
				for _, volume := range restoreObj.GetVolumes() {
					totRestoreSize = totRestoreSize + volume.TotalSize
					log.InfoD("verify the restore size [%v][%d]", i, totRestoreSize)
				}
			}
			log.InfoD("Total restore size from cluster [%v]", totRestoreSize)
		})

		// Check and validate the pxbackup_restore_size_bytes with actual restore size from cluster
		Step("Verify the pxbackup restore size from metrics endpoint", func() {
			metricsName := "pxbackup_restore_size_bytes"
			actual := totRestoreSize
			log.InfoD("started restore count")
			var expected uint64
			for i := 0; i < restoreCount*clusterCount; i++ {
				totRestoreSizeFromMetrics := GetMetricValue(allMetricsData, metricsName, restoreNames[i])
				currentMetricsRestoreSize, err := ConvertSciNotationFloat64ToUint64(totRestoreSizeFromMetrics)
				log.FailOnError(err, "Error while converting value to uint64")
				expected = expected + currentMetricsRestoreSize
			}
			log.InfoD("total restore size from multiple cluster metrics: [%v]", expected)
			dash.VerifyFatal(actual, expected, "Verify the cluster restore size with metrics size from metrics endpoint")
		})
	})
