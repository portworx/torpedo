			...
			for _, namespace := range appNamespaces {
				for i := 0; i < numberOfBackups; i++ {
					...
					go func(backupName string, namespace string) {
						...
						defer wg.Done()
						defer func() { <-sem }()
						_, err = CreateBackupByNamespacesWithoutCheck(backupName, SourceClusterName, bkpLocationName, backupLocationUID, []string{namespace}, labelSelectors, BackupOrgID, srcClusterUid, "", "", "", "", ctx)
