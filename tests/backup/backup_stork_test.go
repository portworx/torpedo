...
var _ = Describe("{BackupAndRestoreWithNonExistingAdminNamespace}", func() {
	...
	It("Selective Restore From a Basic Backup", func() {
		...
		// 	}
		// })
		Step("Printing Stork related details", func() {
			log.InfoD("Printing stork related details")
			stc, err := Inst().V.GetDriver()
			log.FailOnError(err, "Unable to fetch config data")
			log.InfoD("Storage Cluster Details stork - %v", stc.Spec.Stork.Args)
			log.Info("Adding custom admin namespace")
			stc.Spec.Stork.Args["admin_namespace"] = "stork"
			stc, err = operator.Instance().UpdateStorageCluster(stc)
			log.Info("Storage Cluster Details After Update %v")
			...
			log.InfoD("Admin NameSpace - %v", adminNamespace)
			log.InfoD("Config Map Details - %v", configData)
		})
