	...
	It("Create backup and restore for all combinations of SSE-S3 form px-backup side and deny policy configured on AWS S3 bucket", func() {
		...
		for _, provider := range providers {
			...
			Step("Create BackupLocation with bucketWithPolicy and SSE set to true", func() {
				...
				err = CreateS3BackupLocationWithSseType(customBackupLocationWithSse, backupLocationUID, credName, cloudCredUID, customBucketsWithDenyPolicy[0], orgID, "", api.S3Config_SSE_S3, true)
			...
			Step("Create BackupLocation with bucketWithPolicy and SSE set to true post px-backup and stork restart", func() {
				...
				err = CreateS3BackupLocationWithSseType(newBackupLocationWithSseAfterRestart, backupLocationUID, credName, cloudCredUID, customBucketsWithDenyPolicy[1], orgID, "", api.S3Config_SSE_S3, true)
