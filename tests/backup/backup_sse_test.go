	...
	It("sse-s3 encryption", func() {
		...
		Step("Adding Credentials and Registering Backup Location", func() {
			...
			for _, provider := range providers {
				...
				customBucketsWithOutPolicy = append(customBuckets, customBucket)
			}
			...
			for _, provider := range providers {
				for i := 0; i < scCount; i++ {
					...
					customBucketsWithPolicy = append(customBuckets, customBucket)
			...
			for _, provider := range providers {
				...
				err = CreateS3BackupLocationWithSseType(customBackupLocationName, backupLocationUID, credName, cloudCredUID, customBucketsWithPolicy[0], orgID, "", api.S3Config_SSE_S3)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Creating backup location %s", customBackupLocationName))
				log.Infof("created backup location successfully")
			}
