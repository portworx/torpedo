	...
	It("sse-s3 encryption", func() {
		...
		for _, provider := range providers {
			...
			Step("Create BackupLocation with bucketWithPolicy and SSE set to false", func() {
				...
				dash.VerifyFatal(strings.Contains(err.Error(), "AccessDenied"), nil, fmt.Sprintf("Verifing backup location creation fail in case of bucket with deny policy and SSE set to false"))
			})
