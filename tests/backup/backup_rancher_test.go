	...
	It("Basic Backup Creation", func() {
		...
		Step("Creating backup location and cloud setting", func() {
			...
			for _, provider := range backupLocationProviders {
				...
				dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation of cloud credential named [%s] for org [%s] %s]  as provider\", clusterCredName, orgID)as provider for backup location", credName, orgID))
		...
		Step("Registering clusters for backup", func() {
			...
			for _, provider := range clusterProviders {
				for _, kubeconfig := range kubeconfigList {
					...
					AddApplicationCluster(kubeconfig, orgID, clusterCredName, clusterUid, ctx)
				}
			}
		})
