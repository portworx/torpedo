	...
	It("VerifyRestoreObjectsAreDeletedCreatedByNonSuperAdmin", func() {
		...
		Step("Create backup location and cloud setting with User2", func() {
			...
			for _, provider := range providers {
				...
				err := CreateCloudCredential(provider, cloudCredName, cloudCredUID, BackupOrgID, testUsers[2].ctx)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation of cloud credential named [%s] for org [%s] with [%s] as provider", cloudCredName, BackupOrgID, provider))
				err = CreateBackupLocationWithContext(provider, bkpLocationName, backupLocationUID, cloudCredName, cloudCredUID, getGlobalBucketName(provider), BackupOrgID, "", testUsers[2].ctx, true)
