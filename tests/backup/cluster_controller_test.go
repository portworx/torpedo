...
var _ = Describe("{ClusterControllerDemo}", func() {
	...
	It("Cluster Controller Demo", func() {
		...
		//
		//		namespaces, err := helper.TestCaseControllerCollectionMap[testRailId].GetClusterController("source-cluster").App(appKey).ScheduleOnMultipleNamespaces(namespacePrefix, 2)
		...
		//
		//		err := helper.TestCaseControllerCollectionMap[testRailId].GetClusterController("destination-cluster").App(appKey).ScheduleOnNamespace(namespace)
		//		log.FailOnError(err, fmt.Sprintf("failed to schedule the application %s on the namespace %s", appKey, namespace))
		//
