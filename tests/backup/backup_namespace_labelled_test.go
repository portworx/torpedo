...
var _ = Describe("{BackupScheduleForOldAndNewNS}", func() {
	...
	It("Schedule backup using namespace label for old and new namespaces", func() {
		...
		Step("Restoring scheduled backups with old namespaces only", func() {
			log.InfoD("Restoring scheduled backups with old namespaces only")
			ctx, err := backup.GetAdminCtxFromSecret()
			dash.VerifyFatal(err, nil, "Fetching px-central-admin ctx")
		...
		Step("Schedule applications to create new namespaces", func() {
			...
			for i := 0; i < newNSCount; i++ {
				...
				allNamespaces = append(newNamespaces, bkpNamespaces...)
			}
		})
		...
		Step("Restoring scheduled backups", func() {
			...
			for _, backupName := range allScheduleBackupNames {
				restoreName = fmt.Sprintf("%s-%s-%v", restoreNamePrefix, scheduleName, time.Now().Unix())
				// Here allScheduleBackupNames contains backup with both old and old+new namespaces,so  did not change CreateRestore to CreateRestoreWithValidation
		...
		Step("Restoring the latest scheduled backups to see if both old and new namespaces have been restored", func() {
			log.InfoD("Restoring the latest scheduled backups to see if both old and new namespaces have been restored")
			ctx, err := backup.GetAdminCtxFromSecret()
			dash.VerifyFatal(err, nil, "Fetching px-central-admin ctx")
			latestScheduleBkpName, err := GetLatestScheduleBackupName(ctx, scheduleName, orgID)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Fetching name of latest schedule backups of schedule named [%s]", scheduleName))
			restoreName = fmt.Sprintf("%s-%s-%v", restoreNamePrefix, scheduleName, time.Now().Unix())
			appContextsToBackup := FilterAppContextsByNamespace(scheduledAppContexts, allNamespaces)
			err = CreateRestoreWithValidation(ctx, restoreName, latestScheduleBkpName, namespaceMapping, make(map[string]string), destinationClusterName, orgID, appContextsToBackup)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verification of restoring scheduled backups with both old and new namespaces- %s", restoreName))
			restoreNames = append(restoreNames, restoreName)
		})
