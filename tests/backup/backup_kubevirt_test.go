...
var _ = Describe("{DefaultBackupRestoreWithKubevirtAndNonKubevirtNS}", Label(TestCaseLabelsMap[DefaultBackupRestoreWithKubevirtAndNonKubevirtNS]...), func() {
	...
	It("Verify default backup & restore with both Kubevirt and Non-Kubevirt namespaces", func() {
		...
		Step("Creating schedule backup with single namespace contains both kubevirt and non-kubevirt using exec rules", func() {
			...
			log.InfoD("Creating a backup [%s] in [%s] with namespace [%s] in backup location [%s]", backupName, SourceClusterName, appNamespaces[0], backupLocationName)
			scheduleBackupName, err = CreateScheduleBackupWithValidation(ctx, backupName, SourceClusterName, sourceClusterUID, backupLocationName, backupLocationUID, singleScheduledAppContexts, labelSelectors, BackupOrgID, preRuleName, preRuleUid, postRuleName, postRuleUid, periodicPolicyName, schPolicyUid, false)
		...
		Step("Creating single schedule backup with all namespaces", func() {
			...
			scheduleBackupName, err = CreateScheduleBackupWithValidation(ctx, backupName, SourceClusterName, sourceClusterUID, backupLocationName, backupLocationUID, scheduledAppContexts, labelSelectors, BackupOrgID, preRuleName, preRuleUid, postRuleName, postRuleUid, periodicPolicyName, schPolicyUid, false)
			backupNames = append(backupNames, scheduleBackupName)
		...
		Step("Creating schedule backup with single namespace contains both kubevirt and non-kubevirt apps", func() {
			...
			scheduleNames = append(scheduleNames, backupName)
			log.InfoD("Creating a backup [%s] in [%s] with namespace [%s] in backup location [%s]", backupName, SourceClusterName, appNamespaces[0], backupLocationName)
			scheduleBackupName, err = CreateScheduleBackupWithValidation(ctx, backupName, SourceClusterName, sourceClusterUID, backupLocationName, backupLocationUID, singleScheduledAppContexts, labelSelectors, BackupOrgID, "", "", "", "", periodicPolicyName, schPolicyUid, false)
			backupNames = append(backupNames, scheduleBackupName)
		...
		Step("Creating single schedule backup with all the namespaces", func() {
			...
			scheduleNames = append(scheduleNames, backupName)
			log.InfoD("Creating a backup [%s] in [%s] with namespaces %s in backup location [%s]", backupName, SourceClusterName, appNamespaces, backupLocationName)
			scheduleBackupName, err = CreateScheduleBackupWithValidation(ctx, backupName, SourceClusterName, sourceClusterUID, backupLocationName, backupLocationUID, scheduledAppContexts, labelSelectors, BackupOrgID, "", "", "", "", periodicPolicyName, schPolicyUid, false)
			backupNames = append(backupNames, scheduleBackupName)
		...
		Step("Creating multiple schedule backups with each namespace", func() {
			...
			for i, appCtx := range multiScheduledAppContexts {
				...
				scheduleNames = append(scheduleNames, backupName)
				log.InfoD("Creating backup [%s] in [%s] with namespace [%s] in backup location [%s]", backupName, SourceClusterName, scheduledNamespace, backupLocationName)
				scheduleBackupName, err = CreateScheduleBackupWithValidation(ctx, backupName, SourceClusterName, sourceClusterUID, backupLocationName, backupLocationUID, multiScheduledAppContexts[i:i+1], labelSelectors, BackupOrgID, "", "", "", "", periodicPolicyName, schPolicyUid, false)
				backupNames = append(backupNames, scheduleBackupName)
		...
		Step("Taking first schedule backup of kubevirt VMs without namespace label", func() {
			...
			for _, namespace := range bkpNamespaces {
				...
				_, err = CreateVMScheduledBackupWithValidation(nonLabelScheduleName, vms, SourceClusterName, backupLocationName, backupLocationUID, scheduledAppContexts,
					labelSelectors, BackupOrgID, sourceClusterUid, "", "", "", "", false, periodicSchedulePolicyName, periodicSchedulePolicyUid, false, ctx)
		...
		Step("Taking first schedule backup of kubevirt VMs with namespace label", func() {
			...
			for _, namespace := range bkpNamespaces {
				...
				_, err = CreateVMScheduleBackupWithNamespaceLabelWithValidation(ctx, labelScheduleName, vms, SourceClusterName, backupLocationName, backupLocationUID, scheduledAppContexts,
					labelSelectors, BackupOrgID, "", "", "", "", nsLabelString, periodicSchedulePolicyName, periodicSchedulePolicyUid, false, false)
...
var _ = Describe("{CustomBackupRestoreWithKubevirtAndNonKubevirtNS}", Label(TestCaseLabelsMap[CustomBackupRestoreWithKubevirtAndNonKubevirtNS]...), func() {
	...
	It("Verify custom backup & restore with both Kubevirt and Non-Kubevirt namespaces", func() {
		...
		Step("Creating schedule backup with single namespace contains both kubevirt and non-kubevirt using exec rules", func() {
			...
			scheduleNames = append(scheduleNames, backupName)
			log.InfoD("Creating a backup [%s] in [%s] with namespace [%s] in backup location [%s]", backupName, SourceClusterName, appNamespaces[0], backupLocationName)
			scheduleBackupName, err = CreateScheduleBackupWithValidation(ctx, backupName, SourceClusterName, sourceClusterUID, backupLocationName, backupLocationUID, singleScheduledAppContexts, labelSelectors, BackupOrgID, preRuleName, preRuleUid, postRuleName, postRuleUid, periodicPolicyName, schPolicyUid, false, []string{"PersistentVolumeClaim"}...)
			backupNames = append(backupNames, scheduleBackupName)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Creation of single schedule backup [%s] with single namespace contains multiple apps using exec rules.", scheduleBackupName))
		})
		...
		Step("Creating single schedule backup with all namespaces", func() {
			...
			scheduleNames = append(scheduleNames, backupName)
			log.InfoD("Creating a backup [%s] in [%s] with namespaces %s in backup location [%s]", backupName, SourceClusterName, appNamespaces, backupLocationName)
			scheduleBackupName, err = CreateScheduleBackupWithValidation(ctx, backupName, SourceClusterName, sourceClusterUID, backupLocationName, backupLocationUID, scheduledAppContexts, labelSelectors, BackupOrgID, preRuleName, preRuleUid, postRuleName, postRuleUid, periodicPolicyName, schPolicyUid, false, []string{"PersistentVolumeClaim"}...)
			backupNames = append(backupNames, scheduleBackupName)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Creation of single schedule backup [%s] with all namespaces %s using exec rules", scheduleBackupName, appNamespaces))
		})
		...
		Step("Creating schedule backup with single namespace contains both kubevirt and non-kubevirt apps", func() {
			...
			scheduleNames = append(scheduleNames, backupName)
			log.InfoD("Creating a backup [%s] in [%s] with namespace [%s] in backup location [%s]", backupName, SourceClusterName, appNamespaces[0], backupLocationName)
			scheduleBackupName, err = CreateScheduleBackupWithValidation(ctx, backupName, SourceClusterName, sourceClusterUID, backupLocationName, backupLocationUID, singleScheduledAppContexts, labelSelectors, BackupOrgID, "", "", "", "", periodicPolicyName, schPolicyUid, false, []string{"PersistentVolumeClaim"}...)
			backupNames = append(backupNames, scheduleBackupName)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Creation of single schedule backup [%s] with single namespace contains multiple apps.", scheduleBackupName))
		})
		...
		Step("Creating single schedule backup with all the namespaces", func() {
			...
			scheduleNames = append(scheduleNames, backupName)
			log.InfoD("Creating a backup [%s] in [%s] with namespaces %s in backup location [%s]", backupName, SourceClusterName, appNamespaces, backupLocationName)
			scheduleBackupName, err = CreateScheduleBackupWithValidation(ctx, backupName, SourceClusterName, sourceClusterUID, backupLocationName, backupLocationUID, scheduledAppContexts, labelSelectors, BackupOrgID, "", "", "", "", periodicPolicyName, schPolicyUid, false, []string{"PersistentVolumeClaim"}...)
			backupNames = append(backupNames, scheduleBackupName)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Creation of single schedule backup [%s] with all namespaces %s", scheduleBackupName, appNamespaces))
		})
		...
		Step("Creating multiple schedule backups with each namespace", func() {
			...
			for i, appCtx := range multiScheduledAppContexts {
				...
				scheduleNames = append(scheduleNames, backupName)
				log.InfoD("Creating backup [%s] in [%s] with namespace [%s] in backup location [%s]", backupName, SourceClusterName, scheduledNamespace, backupLocationName)
				scheduleBackupName, err = CreateScheduleBackupWithValidation(ctx, backupName, SourceClusterName, sourceClusterUID, backupLocationName, backupLocationUID, multiScheduledAppContexts[i:i+1], labelSelectors, BackupOrgID, "", "", "", "", periodicPolicyName, schPolicyUid, false, []string{"PersistentVolumeClaim"}...)
				backupNames = append(backupNames, scheduleBackupName)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Creation of multiple schedule backup [%s] with each namespace [%s]", scheduleBackupName, scheduledNamespace))
			}
		})
