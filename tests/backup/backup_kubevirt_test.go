...
var _ = Describe("{DummyTest1}", func() {
	...
	JustBeforeEach(func() {
		StartPxBackupTorpedoTest("DummyTest1", "Verify the restore of VM backup on the destination cluster in a namespace where the same VM is already running with Replace/Retain option by changing the number of disks",
			nil, 296421, Mkoppal, Q1FY25)
	...
	JustAfterEach(func() {
		log.Infof("DummyTest1 Test execution completed")
		defer EndPxBackupTorpedoTest(scheduledAppContexts)
	})
})
...
var _ = Describe("{DummyTest2}", func() {
	...
	JustBeforeEach(func() {
		StartPxBackupTorpedoTest("DummyTest2", "Verify the restore of VM backup on the destination cluster in a namespace where the same VM is already running with Replace/Retain option by changing the number of disks",
			nil, 296421, Mkoppal, Q1FY25)

		log.InfoD("scheduling applications")
		scheduledAppContexts = make([]*scheduler.Context, 0)
		taskName := fmt.Sprintf("%s", "test2")
		appContexts := ScheduleApplications(taskName)
		for _, appCtx := range appContexts {
			appCtx.ReadinessTimeout = AppReadinessTimeout
			scheduledAppContexts = append(scheduledAppContexts, appCtx)
...
var _ = Describe("{DummyTest0}", func() {
	var (
		// TODO: Need to uncomment the below code once we have data validation for kubevirt VMs implemented
		//controlChannel       chan string
		//errorGroup           *errgroup.Group
	...
	JustBeforeEach(func() {
		StartPxBackupTorpedoTest("DummyTest0", "Verify the restore of VM backup on the destination cluster in a namespace where the same VM is already running with Replace/Retain option by changing the number of disks",
			nil, 296421, Mkoppal, Q1FY25)

		log.InfoD("scheduling applications")
		scheduledAppContexts = make([]*scheduler.Context, 0)
		taskName := fmt.Sprintf("%s", "test0")
		appContexts := ScheduleApplications(taskName)
		for _, appCtx := range appContexts {
			appCtx.ReadinessTimeout = AppReadinessTimeout
			scheduledAppContexts = append(scheduledAppContexts, appCtx)
