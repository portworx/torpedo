...
var _ = Describe("{testParseConfig}", func() {
	...
	provider, _ := getCloudProviderCred("aws", "default")
	log.InfoD("Access Key IDN: %s", provider.AccessKeyID)
	log.Infof("Access Key IDN:", provider.Region)
	backupTarget, _ := getBackupTargets("s3", "default")
	log.InfoD("Name: %s", backupTarget.Name)
