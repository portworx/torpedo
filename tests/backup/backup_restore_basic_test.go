		...
		Step(fmt.Sprintf("Validate deleting backups and restores"), func() {
			...
			for _, backupName := range backupNames {
				...
				go func(backupName string) {
					...
					defer wg.Done()
					backupUid, err := Inst().Backup.GetBackupUID(ctx, backupName, orgID)
					dash.VerifyFatal(err, nil, fmt.Sprintf("Failed to fetch the backup %s uid", backupName))
					_, err = DeleteBackup(backupName, backupUid, orgID, ctx)
					dash.VerifyFatal(err, nil, fmt.Sprintf("Failed to delete the backup %s", backupName))
