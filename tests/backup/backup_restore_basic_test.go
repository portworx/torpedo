...
var _ = Describe("{NamespaceLabelScheduleBackup}", func() {
	var (
		err                        error
		contexts                   []*scheduler.Context
		appContexts                []*scheduler.Context
	...
	JustBeforeEach(func() {
		StartTorpedoTest("NamespaceLabelScheduleBackup", "Filter out namespace using label and perform backup and restore", nil, 58041)
		log.InfoD("Deploy applications")
		contexts = make([]*scheduler.Context, 0)
		for i := 0; i < Inst().GlobalScaleFactor; i++ {
			taskName := fmt.Sprintf("%s-%d", taskNamePrefix, i)
	...
	It("Backup Schedule using labels", func() {
		providers := getProviders()
		Step("Validate applications", func() {
			log.InfoD("Validate applications")
			ValidateApplications(contexts)
		...
		Step("Adding labels to namespaces", func() {
			log.InfoD("Adding labels to group of namespaces and resources")
			nsLabelsGroup1, err = AddMultipleLabelsToNS(1, bkpNamespaces, "nsGroup1")
		...
		Step("Adding labels to resources", func() {
			...
			log.InfoD("All label selectors added on resources - %v", labelSelectors)
		})
		Step("Selecting random label", func() {
			log.InfoD("Selecting random label")
			for _, key := range nsLabelsGroup1 {
				keys = append(keys, key)
			}
			randomKey = keys[rand.Intn(len(keys))]
			log.InfoD("Random selected label %v", randomKey)
		})
		Step("Creating credentials and backup location", func() {
			log.InfoD("Creating cloud credentials and backup location")
			for _, provider := range providers {
		...
		Step("Taking one generic backup of applications", func() {
			log.InfoD("Taking one generic backup of applications")
			ctx, err := backup.GetAdminCtxFromSecret()
			dash.VerifyFatal(err, nil, "Fetching px-admin context")
			backupName = fmt.Sprintf("%s-%v", "backup", time.Now().Unix())
			backupNames = append(backupNames, backupName)
			err = CreateBackupUsingLabels(backupName, SourceClusterName, backupLocationName, backupLocationUID,
				labelSelectors, orgID, clusterUid, "", "", "", "", ctx, randomKey)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying backup [%s] creation", backupName))
		})
		Step("Restoring the generic backup", func() {
			log.InfoD("Restoring scheduled the generic backup")
			ctx, err := backup.GetAdminCtxFromSecret()
			log.FailOnError(err, "Fetching px-central-admin ctx")
		...
		Step("Create schedule backup", func() {
			...
			dash.VerifyFatal(err, nil, "Fetching px-central-admin ctx")
			scheduleName = fmt.Sprintf("%s-schedule-%v", BackupNamePrefix, time.Now().Unix())
			err = CreateScheduleBackupUsingLabel(scheduleName, SourceClusterName, backupLocationName, backupLocationUID,
				labelSelectors, orgID, "", "", "", "", periodicSchedulePolicyName, periodicSchedulePolicyUid, ctx, randomKey)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation of schedule backup with schedule name [%s]", scheduleName))
			firstScheduleBackupName, err := GetFirstScheduleBackupName(ctx, scheduleName, orgID)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Fetching the name of the first schedule backup [%s]", firstScheduleBackupName))
		})
		Step("Restoring scheduled backups", func() {
			...
			err = CreateRestore(restoreName, scheduleName, namespaceMapping, destinationClusterName, orgID, ctx, nil)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verification of restoring scheduled backups - %s", restoreName))
			restoreNames = append(restoreNames, restoreName)
		})
