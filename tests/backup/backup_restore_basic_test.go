...
var _ = Describe("{BackupWithResourceLabel}", func() {
	...
	It("Taking backup and restoring multiple namespace having same labels", func() {
		Step("Adding labels to all namespaces", func() {
			...
			dash.VerifyFatal(err, nil, fmt.Sprintf("delete labels [%v] from pvc [%v]", nsLabelsMap, "bkpNamespaces"))
			err = Inst().S.RemoveLabelFromConfigMap("nsLabelsMap", nsLabelsMap)
			dash.VerifyFatal(err, nil, fmt.Sprintf("delete labels [%v] to configmap [%v]", nsLabelsMap, "bkpNamespaces"))
			//err = Inst().S.RemoveLabelFromSecret("nsLabelsMap", nsLabelsMap)
			//dash.VerifyFatal(err, nil, fmt.Sprintf("delete labels [%v] from secret [%v]", nsLabelsMap, "bkpNamespaces"))
		})
	})
})
