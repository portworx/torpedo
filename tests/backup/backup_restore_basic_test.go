...
var _ = Describe("{BasicSelectiveRestore}", func() {
	...
	It("Selective Restore From a Basic Backup", func() {
		...
		Step("Selecting random backed-up apps and restoring them", func() {
			...
			err = CreateRestore(restoreName, backupName, selectedBkpNamespaceMapping, destinationClusterName, orgID, PxBackupAdminContext, make(map[string]string))
...
var _ = Describe("{CustomResourceBackupAndRestore}", func() {
	...
	It("Create custom resource backup and restore", func() {
		...
		Step("Taking backup of applications", func() {
			...
			for _, namespace := range bkpNamespaces {
				...
				err = CreateBackupWithCustomResourceType(backupName, SourceClusterName, backupLocation, backupLocationUID, []string{namespace}, nil, orgID, clusterUid, "", "", "", "", []string{"PersistentVolumeClaim"}, PxBackupAdminContext)
		...
		Step("Restoring the backed up application", func() {
			...
			for _, namespace := range bkpNamespaces {
				...
				err = CreateRestore(restoreName, backupName, namespaceMapping, SourceClusterName, orgID, PxBackupAdminContext, make(map[string]string))
	...
	JustAfterEach(func() {
		...
		for _, restore := range restoreNames {
			err = DeleteRestore(restore, orgID, PxBackupAdminContext)
	...
	It("Create backup objects and delete", func() {
		...
		Step("Taking backup of applications", func() {
			...
			if len(preRuleNameList) > 0 {
				preRuleUid, err = Inst().Backup.GetRuleUid(orgID, PxBackupAdminContext, preRuleNameList[0])
			...
			if len(postRuleNameList) > 0 {
				postRuleUid, err = Inst().Backup.GetRuleUid(orgID, PxBackupAdminContext, postRuleNameList[0])
			...
			for _, namespace := range bkpNamespaces {
				...
				err = CreateBackup(backupName, SourceClusterName, bkpLocationName, backupLocationUID, []string{namespace},
					labelSelectors, orgID, clusterUid, preRuleName, preRuleUid, postRuleName, postRuleUid, PxBackupAdminContext)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying %s backup creation", backupName))
			}
		})
		Step("Restoring the backed up applications", func() {
			...
			err = CreateRestore(restoreName, backupName, namespaceMapping, destinationClusterName, orgID, PxBackupAdminContext, make(map[string]string))
...
var _ = Describe("{ScheduleBackupCreationSingleNS}", func() {
	...
	It("Schedule Backup Creation with single namespace", func() {
		...
		Step("Creating schedule backups", func() {
			...
			schPolicyUid, _ = Inst().Backup.GetSchedulePolicyUid(orgID, PxBackupAdminContext, periodicPolicyName)
			for _, namespace := range bkpNamespaces {
				...
				schBackupName, err = GetFirstScheduleBackupName(PxBackupAdminContext, backupName, orgID)
		...
		Step("Restoring scheduled backups", func() {
			...
			err = CreateRestore(restoreName, schBackupName, namespaceMapping, destinationClusterName, orgID, PxBackupAdminContext, nil)
...
var _ = Describe("{CustomResourceRestore}", func() {
	...
	It("Create custom resource restore", func() {
		...
		Step("Creating credentials and backup location", func() {
			...
			for _, provider := range providers {
				...
				err = CreateCloudCredential(provider, credName, cloudCredUID, orgID, PxBackupAdminContext)
		...
		Step("Taking backup of applications", func() {
			...
			for _, namespace := range bkpNamespaces {
				...
				err = CreateBackup(backupName, SourceClusterName, backupLocationName, backupLocationUID, []string{namespace}, labelSelectors, orgID, clusterUid, "", "", "", "", PxBackupAdminContext)
		...
		Step("Restoring the backed up application", func() {
			...
			for _, namespace := range bkpNamespaces {
				...
				restoredNameSpace := fmt.Sprintf("%s-%s", namespace, "restored")
				namespaceMapping[namespace] = restoredNameSpace
				deploymentName, err = CreateCustomRestoreWithPVCs(restoreName, backupName, namespaceMapping, SourceClusterName, orgID, PxBackupAdminContext, make(map[string]string), namespace)
	...
	JustAfterEach(func() {
		...
		opts[SkipClusterScopedObjects] = true
		ValidateAndDestroy(contexts, opts)
		CleanupCloudSettingsAndClusters(newBackupLocationMap, credName, cloudCredUID, PxBackupAdminContext)
...
var _ = Describe("{AllNSBackupWithIncludeNewNSOption}", func() {
	...
	It("Validates schedule backups created with include new namespaces option includes newly created namespaces", func() {
		...
		Step("Configure source and destination clusters with px-central-admin ctx", func() {
			...
			clusterStatus, err := Inst().Backup.GetClusterStatus(orgID, appClusterName, PxBackupAdminContext)
			...
			clusterUid, err := Inst().Backup.GetClusterUID(PxBackupAdminContext, orgID, appClusterName)
		...
		Step("Create schedule backups", func() {
			...
			err = CreateScheduleBackup(scheduleName, appClusterName, backupLocationName, backupLocationUID, namespaces,
				labelSelectors, orgID, "", "", "", "", periodicSchedulePolicyName, periodicSchedulePolicyUid, PxBackupAdminContext)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation of schedule backup with schedule name [%s]", scheduleName))
			firstScheduleBackupName, err := GetFirstScheduleBackupName(PxBackupAdminContext, scheduleName, orgID)
		...
		Step("Verify new application namespaces inclusion in next schedule backup", func() {
			...
			checkOrdinalScheduleBackupCreation := func() (interface{}, bool, error) {
				ordinalScheduleBackupName, err := GetOrdinalScheduleBackupName(PxBackupAdminContext, scheduleName, nextScheduleBackupOrdinal, orgID)
				if err != nil {
					return "", true, err
				}
			...
			nextScheduleBackupUid, err := Inst().Backup.GetBackupUID(PxBackupAdminContext, nextScheduleBackupName.(string), orgID)
		...
		Step("Restore new application namespaces from next schedule backup", func() {
			...
			err = CreateRestore(restoreName, nextScheduleBackupName.(string), namespaceMapping, appClusterName, orgID, PxBackupAdminContext, make(map[string]string))
			dash.VerifyFatal(err, nil, fmt.Sprintf("Creating restore [%s]", restoreName))
		})
	})
...
var _ = Describe("{BackupSyncBasicTest}", func() {
	...
	It("Validate that the backup sync syncs all the backups present in bucket", func() {
		...
		Step("Remove the backup location where backups were taken", func() {
			...
			backupLocationDeleteStatusCheck := func() (interface{}, bool, error) {
				status, err := IsBackupLocationPresent(customBackupLocationName, PxBackupAdminContext, orgID)
				if err != nil {
		...
		Step("Taking backup of applications to trigger BackupSync goroutine", func() {
			...
			for _, namespace := range bkpNamespaces {
				backupName = fmt.Sprintf("%s-%s-%v", BackupNamePrefix, namespace, time.Now().Unix())
				backupNamespaceMap[namespace] = backupName
				err = CreateBackup(backupName, SourceClusterName, customBackupLocationName, backupLocationUID, []string{namespace}, labelSelectors, orgID, clusterUid, "", "", "", "", PxBackupAdminContext)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying backup creation %s", backupName))
				backupNames = append(backupNames, backupName)
			}
		})
		...
		Step("Check if all backups are synced or not", func() {
			...
			for _, bkp = range curBackups.GetBackups() {
				...
				resp, err := backupDriver.InspectBackup(PxBackupAdminContext, backupInspectRequest)
	...
	JustAfterEach(func() {
		...
		CleanupCloudSettingsAndClusters(backupLocationMap, credName, cloudCredUID, PxBackupAdminContext)
	})
})
	...
	It("Taking backup and restoring multiple namespace having same labels", func() {
		...
		Step("Taking a backup of multiple applications with namespace label filter", func() {
			...
			err = CreateBackupWithNamespaceLabel(multipleNamespaceBackupName, SourceClusterName, backupLocationName, backupLocationUID,
				nil, orgID, clusterUid, "", "", "", "", nsLabelString, PxBackupAdminContext)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying backup [%s] creation with label [%s]", multipleNamespaceBackupName, nsLabelString))
			err = NamespaceLabelBackupSuccessCheck(multipleNamespaceBackupName, PxBackupAdminContext, bkpNamespaces, nsLabelString)
		...
		Step("Restoring multiple applications backup", func() {
			...
			restoreName = fmt.Sprintf("%s-%v", restoreNamePrefix, time.Now().Unix())
			err = CreateRestore(restoreName, multipleNamespaceBackupName, namespaceMapping, destinationClusterName, orgID, PxBackupAdminContext, nil)
		...
		Step("Multiple restore for same backup in different storage class in same cluster at the same time", func() {
			...
			for _, scName := range scNames {
				...
				go func(scName string) {
					defer GinkgoRecover()
					defer wg.Done()
					err = CreateRestore(restoreName, backupName, namespaceMap, SourceClusterName, orgID, PxBackupAdminContext, storageClassMapping)
	...
	It("Add multiple labels to namespaces, perform manual backup, schedule backup using namespace label and restore", func() {
		...
		Step("Taking a manual backup of application using namespace labels", func() {
			...
			err = CreateBackupWithNamespaceLabel(backupName, SourceClusterName, backupLocationName, backupLocationUID,
				nil, orgID, clusterUid, "", "", "", "", namespaceLabel, PxBackupAdminContext)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying backup [%s] creation with label [%s]", backupName, namespaceLabel))
			err = NamespaceLabelBackupSuccessCheck(backupName, PxBackupAdminContext, bkpNamespaces, namespaceLabel)
		...
		Step("Creating a schedule backup with namespace label filter", func() {
			...
			err = CreateScheduleBackupWithNamespaceLabel(scheduleName, SourceClusterName, backupLocationName, backupLocationUID,
				nil, orgID, "", "", "", "", namespaceLabel, periodicSchedulePolicyName, periodicSchedulePolicyUid, PxBackupAdminContext)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation of schedule backup [%s] with labels [%v]", scheduleName, namespaceLabel))
			firstScheduleBackupName, err = GetFirstScheduleBackupName(PxBackupAdminContext, scheduleName, orgID)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Fetching the name of the first schedule backup [%s]", firstScheduleBackupName))
			err = NamespaceLabelBackupSuccessCheck(firstScheduleBackupName, PxBackupAdminContext, bkpNamespaces, namespaceLabel)
		...
		Step("Restoring manual backup of application", func() {
			...
			err = CreateRestore(restoreName, backupName, nil, destinationClusterName, orgID, PxBackupAdminContext, nil)
		...
		Step("Restoring first schedule backup of application", func() {
			...
			scheduleRestoreMapping = make(map[string]string)
			backupScheduleNamespace, err := FetchNamespacesFromBackup(PxBackupAdminContext, firstScheduleBackupName, orgID)
			...
			err = CreateRestore(customRestoreName, firstScheduleBackupName, scheduleRestoreMapping, destinationClusterName, orgID, PxBackupAdminContext, nil)
	...
	It("Namespace labeled manual and schedule backup of single and multiple namespaces along with default and custom restore", func() {
		...
		Step("Taking a manual backup of single application with namespace label filter", func() {
			...
			err = CreateBackupWithNamespaceLabel(manualBkpSingleNS, SourceClusterName, backupLocationName, backupLocationUID,
				nil, orgID, clusterUid, "", "", "", "", labelForSingleNamespace, PxBackupAdminContext)
			...
			err = NamespaceLabelBackupSuccessCheck(manualBkpSingleNS, PxBackupAdminContext, singleNamespace, labelForSingleNamespace)
		...
		Step("Taking a manual backup of multiple applications with namespace label filter", func() {
			...
			err = CreateBackupWithNamespaceLabel(manualBkpMultipleNS, SourceClusterName, backupLocationName, backupLocationUID,
				nil, orgID, clusterUid, "", "", "", "", labelForMultipleNamespace, PxBackupAdminContext)
			...
			err = NamespaceLabelBackupSuccessCheck(manualBkpMultipleNS, PxBackupAdminContext, multipleNamespace, labelForMultipleNamespace)
		...
		Step("Creating a schedule backup for single namespace with namespace label filter", func() {
			...
			err = CreateScheduleBackupWithNamespaceLabel(scheduleBkpSingleNs, SourceClusterName, backupLocationName, backupLocationUID,
				nil, orgID, "", "", "", "", labelForSingleNamespace, periodicSchedulePolicyName, periodicSchedulePolicyUid, PxBackupAdminContext)
			...
			firstScheduleBackupName, err = GetFirstScheduleBackupName(PxBackupAdminContext, scheduleBkpSingleNs, orgID)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Fetching the name of the first schedule backup [%s]", firstScheduleBackupName))
			err = NamespaceLabelBackupSuccessCheck(firstScheduleBackupName, PxBackupAdminContext, singleNamespace, labelForSingleNamespace)
		...
		Step("Creating a schedule backup for multiple applications with namespace label filter", func() {
			...
			err = CreateScheduleBackupWithNamespaceLabel(scheduleBkpMultipleNs, SourceClusterName, backupLocationName, backupLocationUID,
				nil, orgID, "", "", "", "", labelForMultipleNamespace, periodicSchedulePolicyName, periodicSchedulePolicyUid, PxBackupAdminContext)
			...
			firstScheduleBackupForMultipleNs, err = GetFirstScheduleBackupName(PxBackupAdminContext, scheduleBkpMultipleNs, orgID)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Fetching the name of the first schedule backup [%s]", firstScheduleBackupForMultipleNs))
			err = NamespaceLabelBackupSuccessCheck(firstScheduleBackupForMultipleNs, PxBackupAdminContext, multipleNamespace, labelForMultipleNamespace)
		...
		Step("Restoring manual backup of single application", func() {
			...
			err = CreateRestore(restoreName, manualBkpSingleNS, nil, destinationClusterName, orgID, PxBackupAdminContext, nil)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying backup restore with name [%s] in default namespace", restoreName))
			// Restore backup to custom namespace
			backupNamespace, err := FetchNamespacesFromBackup(PxBackupAdminContext, manualBkpSingleNS, orgID)
			...
			err = CreateRestore(customRestoreName, manualBkpSingleNS, namespaceMapping, destinationClusterName, orgID, PxBackupAdminContext, nil)
		...
		Step("Restoring manual backup of multiple applications backup", func() {
			...
			err = CreateRestore(restoreName, manualBkpMultipleNS, nil, destinationClusterName, orgID, PxBackupAdminContext, nil)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying multiple backups [%s] restore in default namespace", restoreName))
			multipleBackupNamespace, err := FetchNamespacesFromBackup(PxBackupAdminContext, manualBkpMultipleNS, orgID)
			...
			err = CreateRestore(customRestoreName, manualBkpMultipleNS, multipleRestoreMapping, destinationClusterName, orgID, PxBackupAdminContext, nil)
		...
		Step("Restoring the incremental scheduled backup of single namespace", func() {
			...
			secondScheduleBackupName, err = GetOrdinalScheduleBackupName(PxBackupAdminContext, scheduleBkpSingleNs, 2, orgID)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Fetching the name of the second schedule backup [%s]", secondScheduleBackupName))
			err = NamespaceLabelBackupSuccessCheck(secondScheduleBackupName, PxBackupAdminContext, singleNamespace, labelForSingleNamespace)
			...
			// Restore to default namespace
			restoreName = fmt.Sprintf("%s-%v", restoreNamePrefix, time.Now().Unix())
			err = CreateRestore(restoreName, secondScheduleBackupName, nil, destinationClusterName, orgID, PxBackupAdminContext, nil)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verification of restoring scheduled backups - %s", restoreName))
			// Restore to custom namespace
			backupScheduleNamespace, err := FetchNamespacesFromBackup(PxBackupAdminContext, secondScheduleBackupName, orgID)
			...
			err = CreateRestore(customRestoreName, secondScheduleBackupName, scheduleRestoreMapping, destinationClusterName, orgID, PxBackupAdminContext, nil)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verification of restoring scheduled backups %s in custom namespace %v", customRestoreName, scheduleRestoreMapping))
			restoreNames = append(restoreNames, restoreName, customRestoreName)
		})
		Step("Restoring the incremental backups for multiple applications", func() {
			...
			secondScheduleBackupForMultipleNs, err = GetOrdinalScheduleBackupName(PxBackupAdminContext, scheduleBkpMultipleNs, 2, orgID)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Fetching the name of the second schedule backup [%s]", secondScheduleBackupForMultipleNs))
			err = NamespaceLabelBackupSuccessCheck(secondScheduleBackupForMultipleNs, PxBackupAdminContext, multipleNamespace, labelForMultipleNamespace)
			...
			// Restore to default namespace
			restoreName = fmt.Sprintf("%s-%v", RestoreNamePrefix, time.Now().Unix())
			err = CreateRestore(restoreName, secondScheduleBackupForMultipleNs, nil, destinationClusterName, orgID, PxBackupAdminContext, nil)
			...
			// Restore to custom namespace
			multipleBackupScheduleNamespace, err := FetchNamespacesFromBackup(PxBackupAdminContext, secondScheduleBackupForMultipleNs, orgID)
			...
			err = CreateRestore(customRestoreName, secondScheduleBackupForMultipleNs, scheduleMultipleRestoreMapping, SourceClusterName, orgID, PxBackupAdminContext, nil)
		...
		Step("Issuing multiple in-place restore at the same time", func() {
			...
			for bkpNameSpace, backupName := range backupNamespaceMapping {
				...
				go func(bkpNameSpace string, backupName string) {
					defer GinkgoRecover()
					defer wg.Done()
					err = CreateRestore(restoreName, backupName, make(map[string]string), SourceClusterName, orgID, PxBackupAdminContext, make(map[string]string))
		...
		Step("Issuing multiple in-place restore at the same time with replace existing resources", func() {
			...
			for bkpNameSpace, backupName := range backupNamespaceMapping {
				...
				go func(bkpNameSpace string, backupName string) {
					defer GinkgoRecover()
					defer wg.Done()
					err = CreateRestoreWithReplacePolicy(restoreName, backupName, make(map[string]string), SourceClusterName, orgID, PxBackupAdminContext, make(map[string]string), ReplacePolicy_Delete)
					dash.VerifyFatal(err, nil, fmt.Sprintf("Restoring backup %v into namespce %v with replacing existing resources", backupName, bkpNameSpace))
				}(bkpNameSpace, backupName)
			}
			wg.Wait()
