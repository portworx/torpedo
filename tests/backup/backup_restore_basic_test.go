...
var _ = Describe("{ScheduleBackupUsingNamespaceLabel}", func() {
	var (
		err                        error
		contexts                   []*scheduler.Context
		appContexts                []*scheduler.Context
	...
	It("Schedule backup and restore, using namespace label and resource label", func() {
		providers := getProviders()
		Step("Validate applications", func() {
			log.InfoD("Validate applications")
			ValidateApplications(contexts)
		...
		Step("Adding labels to namespaces", func() {
			...
			nsLabelsGroup1, err = AddMultipleLabelsToNS(1, bkpNamespaces, "nsGroup0")
			dash.VerifyFatal(err, nil, fmt.Sprintf("Adding labels [%v] to namespaces [%v]", nsLabelsGroup1, bkpNamespaces))
		})
		Step("Adding labels to resources", func() {
			log.InfoD("Adding labels to resources")
			for _, namespace := range bkpNamespaces {
				namespaceMapping[namespace] = namespace
				labelSelectors, err = addLabelsToResources(namespace)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Adding labels to resources [%v]", labelSelectors))
			}
		})
		Step("Selecting random label", func() {
			log.InfoD("Selecting random label")
			keys := make([]string, 0, len(nsLabelsGroup1))
			for k := range nsLabelsGroup1 {
				keys = append(keys, k)
			}
			randomLabel = keys[rand.Intn(len(keys))]
			log.InfoD("Random selected label %v", randomLabel)
		})
		...
		Step("Configure source and destination clusters with px-central-admin", func() {
			...
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation of source [%s] and destination [%s] clusters with px-central-admin ctx", SourceClusterName, destinationClusterName))
			clusterStatus, clusterUid = Inst().Backup.RegisterBackupCluster(orgID, SourceClusterName, "")
			dash.VerifyFatal(clusterStatus, api.ClusterInfo_StatusInfo_Online, fmt.Sprintf("Verifying registration of cluster named [%s] with uid [%s] as backup cluster", SourceClusterName, clusterUid))

		})
		Step("Taking a backup of applications", func() {
		...
		Step("Restoring backup", func() {
			log.InfoD("Restoring backup")
			ctx, err := backup.GetAdminCtxFromSecret()
			dash.VerifyFatal(err, nil, "Fetching px-admin context")
		...
		Step("Creating schedule backup", func() {
			...
			err = CreateScheduleBackupUsingNamespaceLabel(scheduleName, SourceClusterName, backupLocationName, backupLocationUID,
				labelSelectors, orgID, "", "", "", "", periodicSchedulePolicyName, periodicSchedulePolicyUid, ctx, randomLabel)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation of schedule backup with schedule name [%s]", scheduleName))
			firstScheduleBackupName, err = GetFirstScheduleBackupName(ctx, scheduleName, orgID)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Fetching the name of the first schedule backup [%s]", firstScheduleBackupName))
		})
...
var _ = Describe("{FilterNamespaceAndTakeMultipleBackups}", func() {
	var (
		err                              error
		contexts                         []*scheduler.Context
		appContexts                      []*scheduler.Context
	...
	bkpNamespaces = make([]string, 0)
	JustBeforeEach(func() {
		StartTorpedoTest("FilterNamespaceAndTakeMultipleBackups", "Filter using namespace label and take backups in multiple manner", nil, 84842)
		log.InfoD("Deploy applications")
		contexts = make([]*scheduler.Context, 0)
		for i := 0; i < 3; i++ {
			taskName := fmt.Sprintf("%s-%d", taskNamePrefix, i)
	...
	It("Filter using namespace label and take backups in multiple manner", func() {
		...
		Step("Adding labels to namespaces", func() {
			log.InfoD("Adding labels to namespaces")
			nsLabelsGroup1, err = AddMultipleLabelsToNS(2, bkpNamespaces, "nsGroup0")
			dash.VerifyFatal(err, nil, fmt.Sprintf("Adding labels [%v] to namespaces [%v]", nsLabelsGroup1, bkpNamespaces))
		})
		Step("Selecting random label", func() {
			log.InfoD("Selecting random label")
		...
		Step("Taking a backup of application", func() {
			log.InfoD("Taking a backup of application")
			uniqueLabel = fmt.Sprintf("%v=%v", randomLabel, nsLabelsGroup1[randomLabel])
			log.InfoD("Unique label to backup single application %v", uniqueLabel)
			ctx, err := backup.GetAdminCtxFromSecret()
			dash.VerifyFatal(err, nil, "Fetching px-admin context")
			backupName = fmt.Sprintf("%s-%v", "backup", time.Now().Unix())
			err = CreateBackupUsingNamespaceLabels(backupName, SourceClusterName, backupLocationName, backupLocationUID,
				labelSelectors, orgID, clusterUid, "", "", "", "", ctx, uniqueLabel)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying backup [%s] creation", backupName))
			backupNames = append(backupNames, backupName)
		})
		Step("Restoring backup", func() {
		...
		Step("Creating a schedule backup for multiple applications", func() {
			log.InfoD("Creating a schedule backup for multiple applications")
			ctx, err := backup.GetAdminCtxFromSecret()
			dash.VerifyFatal(err, nil, "Fetching px-central-admin ctx")
