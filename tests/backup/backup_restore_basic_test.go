...
var _ = Describe("{SetUnsetNSLabelDuringScheduleBackup}", func() {
	...
	bkpNamespaces = make([]string, 0)
	JustBeforeEach(func() {
		StartTorpedoTest("SetUnsetNSLabelDuringScheduleBackup", "Create multiple namespaces and set unset namespace labels", nil, 84849)
		log.InfoD("Deploy applications")
		contexts = make([]*scheduler.Context, 0)
		for i := 0; i < 3; i++ {
			taskName := fmt.Sprintf("%s-%d", taskNamePrefix, i)
	...
	It("Create multiple namespaces and set unset namespace labels", func() {
		...
		Step("Adding labels to namespaces", func() {
			...
			nsLabelsMap, err = AddLabelsToMultipleNS(1, bkpNamespaces)
		...
		Step("Selecting random label", func() {
			...
			keys := make([]string, 0, len(nsLabelsMap))
			for k := range nsLabelsMap {
				keys = append(keys, k)
			}
		...
		Step("Verify namespace with removed labels is not present in next schedule backup", func() {
			...
			backedUpNamespaces := resp.GetBackup().GetNamespaces()
			log.InfoD("Namespaces in next schedule backup named [%s] and uid [%s] are [%v]", nextScheduleBackupName, nextScheduleBackupUid, backedUpNamespaces)
			dash.VerifyFatal(strings.Contains(strings.Join(backedUpNamespaces, ","), bkpNamespaces[0]), false, fmt.Sprintf("Checking the application namespace [%s] is present in the next scheduled backup named [%s]", bkpNamespaces[0], nextScheduleBackupName))
		})
		...
		Step("Verify namespace inclusion in next schedule backup after setting the namespace labels back", func() {
			...
			backedUpNamespaces := resp.GetBackup().GetNamespaces()
			log.InfoD("Namespaces in next schedule backup named [%s] and uid [%s] are [%v]", nextScheduleBackupName, nextScheduleBackupUid, backedUpNamespaces)
			dash.VerifyFatal(strings.Contains(strings.Join(backedUpNamespaces, ","), bkpNamespaces[0]), true, fmt.Sprintf("Checking the application namespace [%s] is present in the next scheduled backup named [%s]", bkpNamespaces[0], nextScheduleBackupName))
