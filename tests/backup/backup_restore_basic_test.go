...
// CloudSnapsSafeWhenBackupLocationDeleteTest take a good number of backups check if cloud snaps are removed
// if backup location is removed
var _ = Describe("{CloudSnapsSafeWhenBackupLocationDeleteTest}", func() {
	...
	JustBeforeEach(func() {
		StartTorpedoTest("CloudSnapsSafeWhenBackupLocationDeleteTest",
			"Validate if cloud snaps presence if backup location is deleted", nil, 58069)
		log.InfoD("Deploy applications")
		contexts = make([]*scheduler.Context, 0)
		for i := 0; i < Inst().GlobalScaleFactor; i++ {
			taskName := fmt.Sprintf("%s-%d", taskNamePrefix, i)
