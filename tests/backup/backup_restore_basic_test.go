...
import (
	...
	task "github.com/portworx/sched-ops/task"
)
...
var _ = Describe("{ScheduleBackupCreationSingleNS}", func() {
	...
	JustBeforeEach(func() {
		testrailID := 58014
		StartTorpedoTest("ScheduleBackupCreationSingleNS", "Verification of schedule backup creation with single namespace", nil, testrailID)
	})

	It("Validates schedule backups created with single namespace", func() {
		...
		Step("Create schedule backups", func() {
			...
			for _, namespace := range appNamespaces {
				...
				labelSelectors := make(map[string]string)
				err = CreateScheduleBackup(scheduleName, backupClusterName, backupLocationName, backupLocationUID, namespaces,
					labelSelectors, orgID, "", "", "", "", periodicSchedulePolicyName, periodicSchedulePolicyUid, ctx)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation of schedule backup with schedule name [%s] of the namespace [%s]", scheduleName, namespace))
				scheduleNames = append(scheduleNames, scheduleName)
				firstScheduleBackupName, err := GetFirstScheduleBackupName(ctx, scheduleName, orgID)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Fetching the name of the first schedule backup [%s] of the namespace [%s]", firstScheduleBackupName, namespace))
			}
		})
		Step("Restore application namespaces from their second schedule backup", func() {
			...
			for index, namespace := range appNamespaces {
				...
				dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying success of second schedule backup named [%s] of schedule named [%s] of the namespace [%s]", secondScheduleBackupName.(string), scheduleNames[index], namespace))
				...
				log.InfoD("Restoring [%s] namespace from the [%s] backup with restore name [%s]", namespace, secondScheduleBackupName, restoreName)
				...
				dash.VerifyFatal(err, nil, fmt.Sprintf("Creating restore [%s] of the namespace [%s]", restoreName, namespace))
				restoreNames = append(restoreNames, restoreName)
			}
		})
	})
	...
	JustAfterEach(func() {
		...
		for index, namespace := range appNamespaces {
			err = DeleteRestore(restoreNames[index], orgID, ctx)
			dash.VerifySafely(err, nil, fmt.Sprintf("Deleting restore [%s] of the namespace [%s]", restoreNames[index], namespace))
		}
		opts := make(map[string]bool)
		opts[SkipClusterScopedObjects] = true
...
var _ = Describe("{ScheduleBackupCreationAllNS}", func() {
	...
	JustBeforeEach(func() {
		testrailID := 58015
		StartTorpedoTest("ScheduleBackupCreationAllNS", "Verification of schedule backup creation with all namespaces", nil, testrailID)
	})
...
var _ = Describe("{AllNSBackupWithIncludeNewNSOption}", func() {
	...
	JustBeforeEach(func() {
		testrailID := 84760
		StartTorpedoTest("AllNSBackupWithIncludeNewNSOption", "Verification of schedule backups created with include new namespaces option", nil, testrailID)
	})
	...
	It("Validates schedule backups created with include new namespaces option includes newly created namespaces", func() {
		...
		Step("Configure source and destination clusters with px-central-admin ctx", func() {
			...
			err = CreateSourceAndDestClusters(orgID, "", "", ctx)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation of source [%s] and destination [%s] clusters with px-central-admin ctx", SourceClusterName, destinationClusterName))
			backupClusterName = destinationClusterName
			clusterStatus, clusterUid := Inst().Backup.RegisterBackupCluster(orgID, backupClusterName, "")
			dash.VerifyFatal(clusterStatus, api.ClusterInfo_StatusInfo_Online, fmt.Sprintf("Verifying registration of cluster named [%s] with uid [%s] as backup cluster", backupClusterName, clusterUid))
		})
		Step("Create schedule backups", func() {
		...
		Step("Restore new application namespaces from next schedule backup", func() {
			...
			restoreName = fmt.Sprintf("%s-%s", "test-restore", RandomString(4))
			err = CreateRestore(restoreName, nextScheduleBackupName.(string), namespaceMapping, destinationClusterName, orgID, ctx, make(map[string]string))
			dash.VerifyFatal(err, nil, fmt.Sprintf("Creating restore [%s]", restoreName))
		})
	})
