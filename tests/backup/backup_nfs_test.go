...
// DeleteNfsExecutorPodWhileBackupAndRestoreInProgress deletes the nfs executor pod while backup and restore are in progress and validates the status
var _ = Describe("{DeleteNfsExecutorPodWhileBackupAndRestoreInProgress}", func() {
	...
	It("To validate backup and restore status after deleting the respective nfs executor pod while in progress", func() {
		...
		Step("Validating backup status after deleting the respective nfs executor pod while backup is in progress", func() {
			...
			for backupType, namespace := range backupToNfsExecutorPodNamespaceMapping {
				...
				case "scheduleBackup":
					...
					scheduleName = fmt.Sprintf("schedule-backup-%v", RandomString(5))
				...
				case "customResourceBackup":
					...
					customResourceBackupName = fmt.Sprintf("%s-custom-resource-%v", BackupNamePrefix, RandomString(10))
				...
				dash.VerifyFatal(err, nil, fmt.Sprintf("Delete nfs executor pod while backup %s of type %sis in progress", currentBackupName, backupType))
		...
		Step("Validating restore status after deleting the respective nfs executor pod while restore is in progress", func() {
			...
			multiNamespaceRestore = fmt.Sprintf("multi-namespace-%s-%s-%v", restoreNamePrefix, firstSchBackupName, RandomString(5))
			singleNamespaceRestore = fmt.Sprintf("single-namespace-%s-%s-%v", restoreNamePrefix, singleNamespaceBackup, RandomString(5))
