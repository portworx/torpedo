...
var _ = Describe("{CloudSnapshotMissingValidationForNFSLocation}", Label(TestCaseLabelsMap[CloudSnapshotMissingValidationForNFSLocation]...), func() {
	...
	It("To validate the backup status when the cloud snapshots are deleted", func() {
		...
		Step(fmt.Sprintf("Taking a scheduled backup of single namespaces"), func() {
			...
			singleNSScheduledBackup, err = CreateScheduleBackupWithValidation(ctx, singleNSScheduleName, SourceClusterName, clusterUid, bkpLocationName, backupLocationUID, scheduledAppContexts, make(map[string]string), BackupOrgID, "", "", "", "", periodicSchedulePolicyName, periodicSchedulePolicyUid, false)
		...
		Step(fmt.Sprintf("Taking a scheduled backup of multiple namespaces"), func() {
			...
			multipleNSScheduledBackup, err = CreateScheduleBackupWithValidation(ctx, multipleNSScheduleName, SourceClusterName, clusterUid, bkpLocationName, backupLocationUID, scheduledAppContexts, make(map[string]string), BackupOrgID, "", "", "", "", periodicSchedulePolicyName, periodicSchedulePolicyUid, false)
		...
		Step(fmt.Sprintf("Taking a new scheduled backup of multiple namespaces"), func() {
			...
			newScheduledBackup, err = CreateScheduleBackupWithValidation(ctx, newScheduleName, SourceClusterName, clusterUid, bkpLocationName, backupLocationUID, scheduledAppContexts, make(map[string]string), BackupOrgID, "", "", "", "", periodicSchedulePolicyName, periodicSchedulePolicyUid, false)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation and validation of schedule backup with schedule name [%s]", multipleNSScheduleName))
