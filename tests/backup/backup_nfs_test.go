		...
		Step("Validating backup status after deleting the respective nfs executor pod while backup is in progress", func() {
			...
			for backupType, namespace := range backupToNfsExecutorPodNamespaceMapping {
				...
				case "scheduleBackup":
					...
					_, err = CreateScheduleBackupWithoutCheck(scheduleName, SourceClusterName, bkpLocationName, backupLocationUID, appNamespaces, make(map[string]string), BackupOrgID, "", "", "", "", schedulePolicyName, schedulePolicyUID, ctx, nil)
