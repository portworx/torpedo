...
// This TC takes 30 generic backups, deletes intermittent backups and validates the restores
var _ = Describe("{IssueGenericBackupsAndRestoreInterleavedCopies}", func() {
	var (
		scheduledAppContexts []*scheduler.Context
		...
		restoreNames         []string
		genericBackupCount   = 5
		backupDeleteCount    = 3
		backupRestoreCount   = 2
	)

	JustBeforeEach(func() {
		StartPxBackupTorpedoTest("IssueGenericBackupsAndRestoreInterleavedCopies", "To verify the restores for generic backups when intermittent backups are deleted", nil, 58049, Sabrarhussaini, Q3FY24)
		backupLocationMap = make(map[string]string)
	...
	It("To verify the restores for generic backups when intermittent backups are deleted", func() {
		...
		Step("Validate creation of cloud credentials and backup location", func() {
			...
			for _, provider := range providers {
				...
				err = CreateBackupLocation(provider, bkpLocationName, backupLocationUID, cloudAccountName, cloudCredUID, bucketName, orgID, "", true)
		...
		Step("Taking generic backups of application on source cluster", func() {
		...
		Step("Deleting few intermittent backups", func() {
			...
			for i := 0; i < backupDeleteCount; i++ {
				...
				_, err = DeleteBackup(backupNameList[backupIndexToDelete], backupUID, orgID, ctx)
