...
var _ = Describe("{PsaTakeBackupInLowerPrivilegeRestoreInHigherPrivilege}", Label(TestCaseLabelsMap[PsaTakeBackupInLowerPrivilegeRestoreInHigherPrivilege]...), func() {
	...
	It("Take backup in restricted PSA set at a NS and restore in privilege PSA set to new NS", func() {
		...
		Step("Taking schedule backup of namespaces with restricted label and restore it on namespace with privileged label", func() {
			...
			scheduleBackupName, err := CreateScheduleBackupWithValidation(ctx, scheduleName, SourceClusterName, srcClusterUid, backupLocationName, backupLocationUID, restrictedScheduledAppContexts, nil, BackupOrgID, preRuleName, preRuleUid, postRuleName, postRuleUid, periodicSchedulePolicyName, periodicSchedulePolicyUid, false)
		...
		Step("Taking schedule backup of namespaces with baseline label and do default restore on destination cluster without namespace deployed already", func() {
			...
			log.InfoD("Taking schedule backup of applications with both volume and resources on namespace with baseline label set")
			scheduleName := fmt.Sprintf("%s-schedule-with-rules-%s", BackupNamePrefix, RandomString(4))
			scheduleBackupName, err := CreateScheduleBackupWithValidation(ctx, scheduleName, SourceClusterName, srcClusterUid, backupLocationName, backupLocationUID, baselineScheduledAppContexts, nil, BackupOrgID, preRuleName, preRuleUid, postRuleName, postRuleUid, periodicSchedulePolicyName, periodicSchedulePolicyUid, false)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation of backup [%s]", scheduleBackupName))
			dash.VerifyFatal(err, nil, fmt.Sprintf("Creation and Validation of backup of namespaces with volume and resources [%s]", scheduleBackupName))
			err = SuspendBackupSchedule(scheduleName, periodicSchedulePolicyName, BackupOrgID, ctx)
			dash.VerifyFatal(err, nil, fmt.Sprintf("Suspending Backup Schedule [%s] ", scheduleName))
...
var _ = Describe("{PSALowerPrivilegeToHigherPrivilegeWithProjectMapping}", Label(TestCaseLabelsMap[PSALowerPrivilegeToHigherPrivilegeWithProjectMapping]...), func() {
	...
	It("Take backup with restricted PSA namespace and restore it in privilege PSA namespace", func() {
		...
		Step("Creating schedule backup with all restricted & baseline namespaces", func() {
			...
			allNSScheduleBackup, err = CreateScheduleBackupWithValidation(ctx, schBackupName, SourceClusterName, srcClusterUid, backupLocationName, backupLocationUID, scheduledAppContexts, labelSelectors, BackupOrgID, preRuleName, preRuleUid, postRuleName, postRuleUid, periodicPolicyName, schPolicyUid, false)
		...
		Step("Creating schedule backup with restricted & baseline namespaces", func() {
			...
			go func() {
				...
				labelSelectors := make(map[string]string)
				restrictedScheduleBackup, err = CreateScheduleBackupWithValidation(ctx, schBackupName, SourceClusterName, srcClusterUid, backupLocationName, backupLocationUID, restrictedScheduledAppContexts, labelSelectors, BackupOrgID, preRuleName, preRuleUid, postRuleName, postRuleUid, periodicPolicyName, schPolicyUid, false)
			...
			go func() {
				...
				labelSelectors := make(map[string]string)
				baselineScheduleBackup, err = CreateScheduleBackupWithValidation(ctx, schBackupName, SourceClusterName, srcClusterUid, backupLocationName, backupLocationUID, baselineScheduledAppContexts, labelSelectors, BackupOrgID, preRuleName, preRuleUid, postRuleName, postRuleUid, periodicPolicyName, schPolicyUid, false)
