...
var _ = BeforeSuite(func() {
	...
	rValues := reflect.ValueOf(env)
	for i := 0; i < rValues.NumField(); i++ {
		Expect(rValues.Field(i).Bool()).To(BeTrue())
	}
	...
	context := context.WithValue(context.Background(), pds.ContextAPIKeys, map[string]pds.APIKey{"ApiKeyAuth": {Key: pds_cluster_sa_token, Prefix: "Bearer"}})
...
var _ = Describe("{PDSTestListAccountsTenantsProjects}", func() {
	It("has to list accounts, tenants and projects.", func() {
		Step("Get the list of accounts and verify ", func() {
			...
			Expect(strings.Compare("Portworx", accountName)).To(BeTrue())
		})
		...
		Step("Get the list of projects to belong to the detault tenant.", func() {
			...
			Expect(strings.Compare("Default", projectName)).To(BeTrue())
		})
	})
})
