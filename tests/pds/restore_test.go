...
// TODO Enable this test when feature flag will be enabled.
var _ = Describe("{PerformRestoreToDifferentClusters}", func() {
	bkpTargetName = bkpTargetName + pdsbkp.RandString(8)
	JustBeforeEach(func() {
		StartTorpedoTest("PerformRestoreToSameCluster", "Perform multiple restore within same cluster.", pdsLabels, 0)
		bkpClient, err = pdsbkp.InitializePdsBackup()
	...
	It("Perform multiple restore across different clusters", func() {
		var deploymentsToBeCleaned []*pds.ModelsDeployment
		stepLog := "Deploy data service and take adhoc backup, deleting the data service should not delete the backups."
		Step(stepLog, func() {
			log.InfoD(stepLog)
			...
			for _, ds := range params.DataServiceToTest {
				...
				Step(stepLog, func() {
					log.InfoD(stepLog)
					log.InfoD("Setup restore client for multiple clusters.")
					restoreTargets, err := restoreBkp.GetRestoreTCContext()
					log.FailOnError(err, "Error while setting up the restore clients. Err:  %v", err)
				...
				Step("Delete Deployments", func() {
					for _, dsInstance := range deploymentsToBeCleaned {
