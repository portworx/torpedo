...
func ValidateRestore(ctx context1.Context, restoreName string, orgID string, expectedRestoredAppContexts []*scheduler.Context, resourceTypesFilter []string) error {
	log.Infof("the number of exepectedRestoredAppContexts is [%d]", len(expectedRestoredAppContexts))
	...
	specloop:
		...
		for _, appContext := range expectedRestoredAppContexts {
			log.Infof("appContext Appkey and Namespace while collecting volumes [%s] [%s == %s]", appContext.App.Key, appContext.ScheduleOptions.Namespace, expectedRestoredAppContextNamespace)
		...
		log.Infof("actualVolumeMap: [%#v]", actualVolumeMap)
		log.Infof("apparentlyRestoredVolumes: [%#v]", apparentlyRestoredVolumes)
		...
		for _, restoredVolInfo := range apparentlyRestoredVolumes {
			log.Infof("Source volume is %v", restoredVolInfo.SourceVolume)
