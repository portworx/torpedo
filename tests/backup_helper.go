...
// RunCMDInKubevirtVM runs command for a VM in a namespace
func RunCMDInKubevirtVM(namespace string, cmd []string, ctx context1.Context) ([]string, error) {
	if vm, ok := NamespaceAppWithDataMap[namespace]; ok {
		if vm[0].GetApplicationType() == appType.Kubevirt {
			return vm[0].ExecuteCommand(cmd, ctx)
		}
	}

	return []string{}, fmt.Errorf("No kubevirt VM found in the namespace")
