...
// FindProvisionerToSnapshotClassByNamespace finds all volumes in the specified namespace, retrieves their provisioners, and maps them to their corresponding VolumeSnapshotClass.
func FindProvisionerToSnapshotClassByNamespace(namespace string) (map[string]string, error) {
	...
	// Get the list of volumes in the specified namespace
	pvcList, err := k8sCore.GetPersistentVolumeClaims(namespace, nil)
	if err != nil {
		return nil, err
	}
	...
	for _, volume := range pvcList.Items {
		scProvisioner, err := k8sCore.GetStorageProvisionerForPVC(&volume)
		if err != nil {
			return nil, err
		}
		for _, snapshotClass := range snapshotClasses.Items {
			if snapshotClass.Driver == scProvisioner {
				provisionerToSnapshotClassMap[scProvisioner] = snapshotClass.Name
			}
		}
	}
...
func UpdateDriverWithEnvVariable(envVariables map[string]string) error {
	...
	log.InfoD("Sleeping for 5 minutes for PX cluster to stable after deletion of pods")
