...
// GetAllBackupsFromCluster returns all the backups from a cluster
func GetAllBackupsFromCluster(ctx context1.Context, clusterName string, clusterUid string) ([]string, error) {
	backupNames := make([]string, 0)
	backupDriver := Inst().Backup
...
func ShareCluster(ctx context1.Context, clusterName string, clusterUid string, users []string, groups []string, shareClusterBackups bool) (*api.ShareClusterResponse, error) {
	backupDriver := Inst().Backup
...
func ShareClusterWithValidation(ctx context1.Context, clusterName string, clusterUid string, users []string, groups []string, shareClusterBackups bool) (*api.ShareClusterResponse, error) {
	...
	err = ValidateShareCluster(ctx, clusterName, clusterUid, users, groups)
	if err != nil {
		return nil, err
	}

	if shareClusterBackups {
		err = ValidateShareClusterBackup(ctx, clusterName, clusterUid, users, groups)
		if err != nil {
			return nil, err
		}
	}
...
func UnShareCluster(ctx context1.Context, clusterName string, clusterUid string, users []string, groups []string) (*api.UnShareClusterResponse, error) {
	backupDriver := Inst().Backup
...
func UnShareClusterWithValidation(ctx context1.Context, clusterName string, clusterUid string, users []string, groups []string) (*api.UnShareClusterResponse, error) {
	...
	err = ValidateUnShareCluster(ctx, clusterName, clusterUid, users, groups)
	if err != nil {
		return nil, err
	}
	return resp, nil
...
func ValidateShareCluster(ctx context1.Context, clusterName string, clusterUid string, users []string, groups []string) error {
	backupDriver := Inst().Backup
	userIds := []string{}
	groupIds := []string{}
	...
	for _, group := range groups {
		if !IsPresent(groupIds, group) {
			return fmt.Errorf("Group with id [%s] is not present in the list of collaborators for cluster [%s]", group, clusterName)
		}
	}
	...
	if !AreStringSlicesEqual(groupIds, groups) {
		return fmt.Errorf("Group list [%v] does not match the collaborators group list [%v] for cluster [%s]", groups, groupIds, clusterName)
	}

	// Expand the users list with members from groups
	for _, group := range groups {
		usersFromGroup, _ := backup.GetMembersOfGroupWithID(group)
...
func ValidateShareClusterBackup(ctx context1.Context, clusterName string, clusterUid string, users []string, groups []string) error {
	backupDriver := Inst().Backup
	clusterBackups, err := GetAllBackupsFromCluster(ctx, clusterName, clusterUid)
	if err != nil {
		return err
	}
	...
	for _, group := range groups {
		usersFromGroup, _ := backup.GetMembersOfGroupWithID(group)
		users = append(users, usersFromGroup...)
	}
...
func ValidateUnShareCluster(ctx context1.Context, clusterName string, clusterUid string, users []string, groups []string) error {
	backupDriver := Inst().Backup
	req := &api.ClusterInspectRequest{
		Name:  clusterName,
		OrgId: BackupOrgID,
	...
	for _, group := range groups {
		if collaboratorMap[group] {
			return fmt.Errorf("Group with id [%s] is present in the list of collaborators for cluster [%s]", group, clusterName)
		}
	}
