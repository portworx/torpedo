...
spec:
  ...
  - name: init-sysctl
    ...
    securityContext:
      privileged: true
    command: ["sh", "-c", "mkdir -p /mnt/testresults && chmod 777 /mnt/testresults/ && mkdir /dhruv && git clone https://github.com/portworx/torpedo.git /dhruv/"]
  containers:
  - name: torpedo
    image: ${TORPEDO_IMG}
    ...
    command: [ "go" ]
    args: [ "run", "/dhruv/torpedo/apiServer/pxone/apiserver.go"
    ]
    tty: true
