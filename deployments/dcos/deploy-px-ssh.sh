...
cat <<EOF > torpedo.json
{
    ...
    "run": {
        ...
        "cmd": "ginkgo $VERBOSE --trace --fail-fast $SKIP_ARG $FOCUS_ARG --slow-spec-threshold 10m bin/basic.test -- --spec-dir ../drivers/scheduler/dcos/specs --scheduler dcos --node-driver ssh",
        "env": {
