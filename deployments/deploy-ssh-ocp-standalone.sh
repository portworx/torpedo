...
NODE_NAME=$(hostname)
# Check if the node name was retrieved successfully
if [ -z "$NODE_NAME" ]; then
  echo "Error: Could not retrieve the hostname."
  exit 1
fi

kubectl label nodes $NODE_NAME torpedo-master=true
...
spec:
  ...
  - name: torpedo
    ...
    args:
      - |
        mkdir -p /mnt/testresults && chmod 777 /mnt/testresults
        find . -type f -name "*.yaml" -exec sed -i "s|http://kubevirt-disk-registry.pwx.dev.purestorage.com/more_images|http://$HOST_IP:8000/images|g" {} \;
        ginkgo --trace \
          --timeout=${TIMEOUT} \
          $FAIL_FAST \
