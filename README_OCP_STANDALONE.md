...
- Torpedo.tar
- debian-12-4.qcow2 (sharedv4) or fio-data-validation.qcow2 (For FADA-Raw)
- launch-torpedo.sh
...
NEW_TAG="localhost/torpedo:master_custom"
docker pull $IMAGE 
docker tag $IMAGE $NEW_TAG && docker save -o torpedo_master.tar $NEW_TAG
```
...
1. From a machine which has access to a master node, copy the provided artifacts to a directory on Master Node.
   ...
   export APP_IMAGE="debian-12-4.qcow2"
...
3. Edit the `launch-torpedo.sh` file and configure the values according to your environment
   ...
   - ColdAddNewDiskToKubevirtVM 
   - MultipleParallelLiveMigration 
   - LMAfterColdAddDiskToVM
   - LMBeforeColdAddDiskToVM
   - PxKillAfterColdAddDiskToVM
