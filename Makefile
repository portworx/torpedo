...
build-taas:
	...
	go build -ldflags "-X 'NODE_NAME=spec.nodeName' -X 'K8S_VENDOR=${K8S_VENDOR}' -X 'TORPEDO_SSH_USER=${TORPEDO_SSH_USER}' -X 'TORPEDO_SSH_PASSWORD=${TORPEDO_SSH_PASSWORD}' -X 'TORPEDO_SSH_KEY=${TORPEDO_SSH_KEY}' -X 'AZURE_TENANT_ID=${AZURE_TENANTID}' -X 'VOLUME_PROVIDER=${VOLUME_PROVIDER}' -X 'OBJECT_STORE_PROVIDER=${OBJECT_STORE_PROVIDER}' -X 'AZURE_CLIENT_ID=${AZURE_CLIENTID}' -X 'AZURE_CLIENT_SECRET=${AZURE_CLIENTSECRET}' -X 'AZURE_ACCOUNT_NAME=${AZURE_ACCOUNT_NAME}' -X 'SOURCE_RKE_TOKEN=${SOURCE_RKE_TOKEN}' -X 'DESTINATION_RKE_TOKEN=${DESTINATION_RKE_TOKEN}' -X 'AZURE_ACCOUNT_KEY=${AZURE_ACCOUNT_KEY}' -X 'AZURE_SUBSCRIPTION_ID=${AZURE_SUBSCRIPTION_ID}' -X 'AZURE_CLUSTER_NAME=${AZURE_CLUSTER_NAME}' -X 'AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID}' -X 'AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY}' -X 'AWS_REGION=${AWS_REGION}' -X 'KUBECONFIGS=${CLUSTER_CONFIGS}' -X 'S3_ENDPOINT=${S3_ENDPOINT}' -X 'S3_AWS_ACCESS_KEY_ID=${S3_AWS_ACCESS_KEY_ID}' -X 'S3_AWS_SECRET_ACCESS_KEY=${S3_AWS_SECRET_ACCESS_KEY}' -X 'S3_REGION=${S3_REGION}' -X 'BUCKET_NAME=${BUCKET_NAME}' -X 'LOCKED_BUCKET_NAME=${LOCKED_BUCKET_NAME}' -X 'S3_DISABLE_SSL=${S3_DISABLE_SSL}' -X 'DIAGS_BUCKET=${DIAGS_BUCKET}' -X 'PROVIDERS=${PROVIDERS}' -X 'CLUSTER_PROVIDER=${CLUSTER_PROVIDER}' -X 'INTERNAL_DOCKER_REGISTRY=${INTERNAL_DOCKER_REGISTRY}' -X 'IMAGE_PULL_SERVER=${IMAGE_PULL_SERVER}' -X 'IMAGE_PULL_USERNAME=${IMAGE_PULL_USERNAME}' -X 'IMAGE_PULL_PASSWORD=${IMAGE_PULL_PASSWORD}' -X 'VSPHERE_USER=${VSPHERE_USER}' -X 'VSPHERE_PWD=${VSPHERE_PWD}' -X 'VSPHERE_HOST_IP=${VSPHERE_HOST_IP}' -X 'IBMCLOUD_API_KEY=${IBMCLOUD_API_KEY}' -X 'CONTROL_PLANE_URL=${CONTROL_PLANE_URL}' -X 'DS_VERSION=${DS_VERSION}' -X 'DS_BUILD=${DS_BUILD}' -X 'NAMESPACE=${NAMESPACE}' -X 'NO_OF_NODES=${NO_OF_NODES}' -X 'DATA_SERVICE=${DATA_SERVICE}' -X 'DEPLOY_ALL_VERSIONS=${DEPLOY_ALL_VERSIONS}' -X 'DEPLOY_ALL_IMAGES=${DEPLOY_ALL_IMAGES}' -X 'DEPLOY_ALL_DATASERVICE=${DEPLOY_ALL_DATASERVICE}' -X 'GCP_PROJECT_ID=${GCP_PROJECT_ID}' -X 'PDS_USERNAME=${PDS_USERNAME}' -X 'PDS_PASSWORD=${PDS_PASSWORD}' -X 'PDS_CLIENT_SECRET=${PDS_CLIENT_SECRET}' -X 'PDS_CLIENT_ID=${PDS_CLIENT_ID}' -X 'PDS_PARAM_CM=${PDS_PARAM_CM}' -X 'PDS_ISSUER_URL=${PDS_ISSUER_URL}' -X 'CLUSTER_TYPE=${CLUSTER_TYPE}' -X 'TARGET_KUBECONFIG=${TARGET_KUBECONFIG}' -X 'TARGET_CLUSTER_NAME=${TARGET_CLUSTER_NAME}' -X 'PX_ORACLE_user_ocid=${PX_ORACLE_user_ocid}' -X 'PX_ORACLE_fingerprint=${PX_ORACLE_fingerprint}' -X 'PX_ORACLE_private_key_path=${ORACLE_API_KEY}' -X 'INSTANCE_GROUP=${INSTANCE_GROUP}' -X 'LOGGLY_API_TOKEN=${LOGGLY_API_TOKEN}' -X 'PODMETRIC_METERING_INTERVAL_MINUTES=${PODMETRIC_METERING_INTERVAL_MINUTES}' -X 'TARGET_PXBACKUP_VERSION=${TARGET_PXBACKUP_VERSION}' -X 'TARGET_STORK_VERSION=${TARGET_STORK_VERSION}' -X 'PX_BACKUP_HELM_REPO_BRANCH=${PX_BACKUP_HELM_REPO_BRANCH}' -X 'BACKUP_TYPE=${BACKUP_TYPE}' -X 'NFS_SERVER_ADDR=${NFS_SERVER_ADDR}' -X 'NFS_SUB_PATH=${NFS_SUB_PATH}' -X 'NFS_MOUNT_OPTION=${NFS_MOUNT_OPTION}' -X 'NFS_PATH=${NFS_PATH}' -X 'SKIP_PX_OPERATOR_UPGRADE=${SKIP_PX_OPERATOR_UPGRADE}' -X 'VOLUME_SNAPSHOT_CLASS=${VOLUME_SNAPSHOT_CLASS}' -X 'S3_SSE_TYPE=${S3_SSE_TYPE}' -X 'S3_POLICY_SID=${S3_POLICY_SID}' -X 'S3_ENCRYPTION_POLICY=${S3_ENCRYPTION_POLICY}' " -o $(TAAS_BUILD_DIR)
